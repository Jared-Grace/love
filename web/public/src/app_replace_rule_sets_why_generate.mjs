...
export async function app_replace_rule_sets_why_generate() {
  ...
  async function lambda(rule_set) {
    ...
    let r = await openai_responses_cache(
      'You will receive the JSON of the replacement rules of a grammar. Please provide a sentence or two describing what the grammar demonstrates, and why. Answer using JSON: { "result": "description_HTML" }',
      json,
    );
    log(app_replace_rule_sets_why_generate.name, {
      r,
