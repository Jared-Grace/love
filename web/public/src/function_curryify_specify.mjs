...
export async function function_curryify_specify(f_name, positions_comma) {
  let positions_ = text_split_comma(positions_comma);
  let args_get = function_curryify_specify_args_get_curried_right(positions_);
  let r2 = function_curryify_specify_name_get_curried_right(positions_);
  let r = await function_curryify_generic(f_name, r2, args_get);
  return r;
}
