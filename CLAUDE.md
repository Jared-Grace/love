...
**Ask which documented atoms nobody has ever run: `atoms_unexampled`.** A row in either table above is a promise made to every Claude at once, and until 2026-07-28 nothing checked it — 16 of the 56 atoms named there had never been executed by anything, so following the instructions was as likely to land on an unexercised unit as on one the corpus proves every run. `atoms_unexampled_gate_run` (in `q`) now ratchets that list against `data/atoms_unexampled_baseline.json`: a newly documented atom with no example fails, and an entry that *has* since been exampled fails too, so writing one is always followed by `atoms_unexampled_baseline_write` shrinking the file. The baseline holds one long-term entry, `js_selects_functionize`, and it is **not** unkeepable — but the reason it is still there is narrower than "it creates a file". **A multi-file example is a solved shape already:** `kind: "files"` takes `before` and `after` as lists of `{name, source}`, materializes them into a temp folder, runs the command, and compares the whole folder back — eight examples use it, and `example_copy_adds_file` is one that *adds* a file. So neither the format nor the sandbox is missing.
  ...
  What is missing is that the file-creating half resolves **against the ambient repo rather than against the folder it is run in**, in three separate places: `function_name_to_path_relative` builds the path from `functions_path()`, `function_new_declaration_from` combines it with `user_repo_get()`, and `function_unalias_exists_not_assert_json` refuses a name that exists **anywhere in the real repo** — which a sandbox copy cannot escape. That is why the seven commands that do have `files` examples each go through a hermetic `js_identifier*_dir` core taking the folder as its first argument: `example_files_command_lambda` maps a command name to one of those, and functionize has none. **So the work is a `_dir` core for it, not a change to the example machinery** — and note `js_outside_move_generic` already takes its file-creating step as a parameter for exactly this reason, so the seam is open. Threading a root through the other three is the real cost, and it is the same change as a virtual-write mode; do not start it as a side errand.

  When it does land, **show the `after` as tabs, not as a chooser, and do not write a count-based switch between the two:** this transform edits one file and creates one, so the answer is always exactly two. `app_a_list_chooser` is a filter-as-you-type list built for a directory, and a dropdown costs two clicks to read the second file where tabs cost one — with no third file for the extra machinery to ever pay for. **That list is a standing work queue: pick one, write its example, shrink the baseline.**
