...
**A value containing a dot cannot reach a transform at all.** Every granted runner splits its joined argument on commas *and* dots, so `0.85em` arrives as `0` and `85em` and the transform refuses on argument count. That rules out font sizes, versions and file names — the values most likely to be duplicated constants. The way through is not to type the value: **`function_copy <getter> <new_shared_name>` carries the literal across**, then `js_find_string_starting_with` + `js_statement_delete` takes the copied prose out and `js_block_prose_add` writes the shared one.
