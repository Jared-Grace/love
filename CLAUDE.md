...
**Ask which documented atoms nobody has ever run: `atoms_unexampled`.** A row in either table above is a promise made to every Claude at once, and until 2026-07-28 nothing checked it — 16 of the 56 atoms named there had never been executed by anything, so following the instructions was as likely to land on an unexercised unit as on one the corpus proves every run. `atoms_unexampled_gate_run` (in `q`) now ratchets that list against `data/atoms_unexampled_baseline.json`: a newly documented atom with no example fails, and an entry that *has* since been exampled fails too, so writing one is always followed by `atoms_unexampled_baseline_write` shrinking the file. The baseline holds one long-term entry, and it is **not** unkeepable: `js_selects_functionize` publishes a new file, so its result is a *set* of files rather than one before-and-after — and it wrote into the source tree instead of a sandbox because `examples_gate_run` runs on the node side, where `file_overwrite_uncached` takes its disk branch. The other branch of that same function already writes into an IndexedDB-backed simulated filesystem when it runs in a browser, versioned per path, which is exactly the sandbox this needs. So the two missing pieces are named ones — a way to select the virtual branch off the browser, and an example whose `after` is several files — not a property of the transform. **Show that `after` as tabs, not as a chooser, and do not write a count-based switch between the two:** this transform edits one file and creates one, so the answer is always exactly two, and `app_a_list_chooser` is a filter-as-you-type list built for a directory. A dropdown costs two clicks to read the second file; tabs cost one, and there is no third file for the extra machinery to ever pay for. **That list is a standing work queue: pick one, write its example, shrink the baseline.**
