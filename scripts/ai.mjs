...
(async () => {
  ...
  'printed as both ends with the middle counted and the whole thing filed under a name, because Claude is handed this printing rather than asking for it and was cutting it by hand twelve thousand times a fortnight. say love_lines=all for the whole thing, or love_lines=<number> for a different amount'
  ...
  let { log_console_json_capped } = await import('../js/log_console_json_capped.mjs')
  let result = await function_run_from_process_argv_full_name()
  await log_console_json_capped(result)
})();
