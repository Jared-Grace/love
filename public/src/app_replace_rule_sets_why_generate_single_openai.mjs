...
export async function app_replace_rule_sets_why_generate_single_openai(
  ...
  let r = await openai_responses_cache(
    'You will receive the JSON of the replacement rules of a grammar. Please describe what the replacement rules demonstrate, and why. Answer using JSON: { "result": "description" }',
    json,
  );
  log(app_replace_rule_sets_why_generate_single_openai.name, {
    r,
