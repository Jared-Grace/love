...
export async function app_replace_rule_sets_why_generate() {
  ...
  async function lambda(rule_set) {
    ...
    let r = await openai_responses_cache(
      "You will receive the JSON of an unrestricted grammar (although it could be more restrictive like a context-free grammar). Please provide a sentence or two describing what the grammar demonstrates, and why. Please explain as simply as possible. Assume the audience is brand new to computer programming.",
      json,
    );
    log(app_replace_rule_sets_why_generate.name, {
      r,
