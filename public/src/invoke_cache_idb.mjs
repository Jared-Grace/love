...
export async function invoke_cache_idb(fn, args) {
  marker("1");
  ...
  let cached_exists = function lambda3(key) {
    let exists = storage_local_exists(invoke_cache_idb, key);
    return exists;
  };
  let cached_get = async function lambda2(key) {
    let value = await storage_local_get(invoke_cache_idb, key);
    return value;
  };
  let cache_save = function lambda4(key, value) {
    let v2 = storage_local_set_exists_not(invoke_cache_idb, key, value);
    return v2;
  };
