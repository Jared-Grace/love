export function bible_glyph_roots_hebrew() {
  ...
  "THE SPLIT IS AVAILABLE IN HEBREW AND IS NOT AVAILABLE IN GREEK, and the difference is a fact about the languages rather than a choice. Hebrew spends three numbers on this root and one of them, 7067, is an adjective said of God and of nobody else - the LORD your God is a jealous God, and there is no verse where a man is qanna. So that number can be given its own mark without anybody deciding anything about any particular verse. The verb 7065 and the noun 7068 genuinely straddle: Rachel envied her sister and Phinehas was zealous for his God with the same word, and no table keyed by a number can tell those two apart. They keep the burning heart, which now honestly means a burning zeal that may run either way.";
  "IT IS DRAWN AS THE BURNING HEART WITH THE CROWN, out of two marks a reader has already met, and it cost no artwork and no waiting. The crown is adon and is Adonai and is kurios - it already says Lord wherever it stands - so the group reads as the Lord's burning love on sight, which is what the word means. Each half is honest alone, which is the test the altar failed and the shepherd passed.";
  "AND THE ERROR THIS AVOIDS IS A CLAIM ABOUT GOD, which is why it was worth a mark rather than a note. The burning heart stood in exactly one authored verse, the fifth of the twentieth of Exodus, where God names himself jealous - so a reader meeting it there learns the picture from God, and would then meet the same picture over Rachel and over Joseph's brothers and learn that God envies. Nothing would have gone red. The mark would simply have taught it.";
