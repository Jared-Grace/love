...
export function app_code_lesson_expression_choose_order_parentheses() {
  ...
  ("parentheses round the || of a line that also holds an && , taken a press at a time: false && (true || true) or (true || false) && true, choose the || , choose what it comes to, then choose the && and what that comes to");
  ...
  ("The parentheses themselves are old - they were met among numbers, where they beat the * - so nothing about what they mean is new. What is new is that they can be put round these operators too, which is the whole of what one screen should ask.");
  ("Nothing has to refuse the wrong press. The && cannot go first because the side the parentheses are on is not a value yet - it is the || , still waiting - so the rule is read off the line rather than enforced against the learner, which is the same reading every lesson in this run has asked for.");
  ("The parentheses fall on either side of the && from question to question, which is what makes the lesson about the marks rather than about a place on the line. Always at the same end, every answer would sit where the last one did and a learner could press it right without ever reading them.");
