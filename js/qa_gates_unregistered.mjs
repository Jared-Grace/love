...
export async function qa_gates_unregistered() {
  ...
  ("Which names to count is a rule about spelling, and that is worth admitting rather than hiding, because a rule about spelling is the exact shape of blindness this gate exists among. Every gate found here ends in the same two words, so ending in them is what is asked, and a check named some other way is invisible to this precisely as a writer of one fixed file was invisible to the reading of rewrites.");
  ("That is not a worry about the future. Sixteen of the checks the run already asks are named otherwise - some end in the word for an assertion, some in the word for a check - and the first run of this said so by accident, because the run came back holding more members than there are functions named the gate way. Those sixteen are safe by already being in the run. What is still out of reach is a check named that way and never registered, and this cannot see one.");
  ("Widening the rule to those endings was tried on paper and refused. The word for an assertion ends the name of hundreds of ordinary helpers here, so asking for it would bury the answer in functions that were never gates, and an answer nobody reads is worse than a narrower one that is read. So the honest state is that this covers the gate-named family completely and says nothing at all about the rest, and closing that needs a reading of what a check IS rather than of how it is spelled.");
