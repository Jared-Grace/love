...
export function js_code_span_cuttable_is(code, from, to) {
  ...
  ("Written for a corpus to ask. A case is a small function spelled out in full and two line numbers, and the whole of the turning of that into a run and the lines on either side of it lives here rather than in the gate - so the gate says only which cases there are and what each one should answer.");
