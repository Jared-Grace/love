...
export function app_code_lesson_expression_choose_order_not_tree_of_code_generic(
  ...
  ("the shape behind a line one of the not lessons printed, worked out again from the line itself: the ! holds the whole of what stands inside its parentheses, and what that inside is made of is the caller's to say.");
  ...
  ("Reading a line of these lessons' own making is safe because they make only one kind: a ! , an opening parenthesis, one word, an operator, one more word, and a closing parenthesis. The parentheses are always there, because a ! is worked out before either kind of operator and so a shape that says otherwise can only be printed with them.");
  ...
  let opening = text_combine(bang, left_parenthesis);
