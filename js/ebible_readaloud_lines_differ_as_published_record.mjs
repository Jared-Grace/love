...
export async function ebible_readaloud_lines_differ_as_published_record(
  ...
  ("The proving is the command. A chapter whose lines fall short of the verses its page marks looks identical whether nobody ever published the rest or the download stopped early, and no amount of staring at the file we hold will separate those two. Deleting what we hold and asking for it again does: if words come back, the copy was at fault; if the same file comes back, the words are not there to be had. So the verdict is not something a person asserts here, it is something this command goes and finds out.");
