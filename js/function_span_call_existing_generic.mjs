...
export async function function_span_call_existing_generic(
  f_name,
  address_from,
  address_to,
  f_name_call,
  ...
  ("Points a run of lines inside one function at a function that already writes those lines out, leaving the run as a call to it, and puts everything back untouched if the two are not the same work - with the caller saying which reader finds the two ends.");
  ("The whole of the folding, held once. Which reader finds the ends is the only thing its callers ever disagree about, exactly as it is the only thing the two cutters under it disagree about, and a second copy of the cut-read-and-put-back would be a second place for a fix to be missing from.");
