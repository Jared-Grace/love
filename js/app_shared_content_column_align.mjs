...
export function app_shared_content_column_align(element) {
  ...
  "the twin next door pads a full-width box so its children land in the column; this takes the element itself in, by holding the same length out of each side as a margin and narrowing the width by both. Auto side margins would centre it too, but only once it has a width of its own; asking the same length the padding asks keeps the two from drifting apart by even a pixel.";
