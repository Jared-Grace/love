...
export async function song_wordings(references) {
  arguments_assert(arguments, 1);
  ("$plain references");
  ("Every passage a song rests on, each against the wordings that are really on offer for it - each set of words once, and the translations that use exactly those words.");
  ...
  ("IT IS HANDED THE PASSAGES RATHER THAN GOING AND FINDING THEM, WHICH IS THE WHOLE OF WHAT MAKES IT SERVE A SECOND SONG. The first song to need this asked its own passages for itself, so every line of the reading was written against that one song and a second song could only have copied it. Every song already says what it rests on, in the same shape, so taking that list as an argument costs the caller one word and costs this nothing at all.");
