...
import { bible_glyph_chapter_rosetta_lines_files_found } from "./bible_glyph_chapter_rosetta_lines_files_found.mjs";
...
export async function bible_glyph_chapter_rosetta_lines_prose_gate_run() {
  ...
  ("A CHAPTER WITH NO BAND FILE AT ALL IS NOT THIS GATE'S FAULT TO REPORT, and it is already somebody's: ",
    ...
    " sends for every chapter's bands and fails on the one that will not come. Counting the absent ones here and saying so beside the verdict is enough, and it keeps two gates from naming the same repair twice. Which chapters have a file and which have none is asked of ",
    fn_name("bible_glyph_chapter_rosetta_lines_files_found"),
    " rather than worked out here, because that is a question about the shape of the repo and this is a gate about what is written inside a file.");
