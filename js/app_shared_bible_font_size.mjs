...
export function app_shared_bible_font_size() {
  ...
  "Nothing is written back. This page offers no way to change the size, so a size written from here would be a page they passed through quietly deciding what the bible opens at next time - and that holds for the migration too, which is left for the bible reader itself to do the next time it is opened.";
  arguments_assert(arguments, 0);
  let app_fn_name = fn_name("app_bible");
  let key = app_shared_font_size_key_factor();
  let chosen = storage_local_name_get(app_fn_name, key);
  ...
  let pixels = storage_local_name_get(app_fn_name, key_pixels);
