...
export async function app_receipts_purchases_pull(folder_code) {
  ...
  "The date, time, price, description, notes and colour are read from what storage says about the details file, not from inside it: a page opened at an address the store does not know may ask about a file but not read it, and a phone reaching this machine by its number is exactly such a page.";
  ...
  for (let id of object_property_names(remote)) {
    ...
    let purchase = {
      ...
      notes: property_get_or(custom, "notes", ""),
      color: property_get_or(custom, "color", "blue"),
      photos,
    };
