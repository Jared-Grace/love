...
import { list_size_max_skip } from "./list_size_max_skip.mjs";
...
export async function app_a_function_select(context, f_name) {
  ...
  function lambda(h) {
    ...
    let max = 100;
    ("a history shorter than the cap is the normal case: keep all of it, and drop the oldest only once it grows past the cap");
    let result = list_size_max_skip(h, max);
    return result;
  }
