...
export function app_code_lesson_expression_choose_order_and_before_or() {
  ...
  ("It stands on the two lessons that first showed the marks, Or and And, where each was worked between a plain true and a plain false. Neither of those says anything about the two of them meeting, and until they meet a learner has no way to know that they are not equals.");
  ("It used to sit after the run that puts a comparison either side of an && or an ||, and it was written up as the reason those lessons exist. It was moved ahead of them on 2026-09-10 because its lines carry two operators and theirs carry three, and this course puts fewer operators first. Nothing is missing after the move: the line here is plain trues and falses, so the only thing it ever needed was the pair of lessons that taught the marks.");
