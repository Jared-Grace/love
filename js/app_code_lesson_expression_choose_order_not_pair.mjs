...
export function app_code_lesson_expression_choose_order_not_pair() {
  ...
  ("Both of its parents are already met. A ! in front of a bracketed thing was the lesson two before it, and brackets round an && or an || were the lesson after that. Nothing here is a new mark, and nothing here is a new rule - the one new thing is that the thing under the ! is two things joined rather than one comparison.");
