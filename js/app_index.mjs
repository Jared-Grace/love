...
export async function app_index(context) {
  ...
  ("the working links show only on a machine on this same network");
  ...
  ("about sits here with the working links rather than among the apps, because it is the same kind of thing they are: a way in for whoever is working, shown only on this network, and worth one tap instead of the four it takes to reach the same paragraph through an app's settings.");
