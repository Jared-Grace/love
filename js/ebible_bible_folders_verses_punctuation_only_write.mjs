...
export async function ebible_bible_folders_verses_punctuation_only_write() {
  ...
  "IT OVERWRITES RATHER THAN WRITES FRESH, and that is the whole of why the first run of this died. A fresh write refuses a file that is already there, so keeping the answer safe after every hit and refusing to tread on an earlier answer are the same thing asked twice with opposite answers; the second hit threw and took the run with it after thirty translations. The answer here is the whole measurement rather than one translation's share of it, so treading on the earlier copy is the point.";
