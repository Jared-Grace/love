...
export function js_builtin_calls_usable(ast) {
  ...
  "A file that hands work to a browser is left alone entirely. A function sent to a page is sent as text, so the import written for it here never arrives, and the call it was rewritten into lands there as a name nothing answers to - a failure that shows up as a browser test going wrong and can be traced back to nothing.";
