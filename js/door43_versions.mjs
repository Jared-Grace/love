...
export function door43_versions() {
  ...
  ("Each entry names its language the way eBible names one, in the three-letter code and in the language's own writing. That is not decoration: every list here that asks what languages are already covered joins a folder to a language through an eBible copyright page, and a bible from this catalogue has no such page. Spelling the language here is what lets those joins fall back on this list instead of dropping the entry and reporting a covered language as missing.");
  let am = {
    bible_folder: "am_ulb",
    tag: "v7.2",
