...
export function bible_verses_marked_is(bible_folder) {
  ...
  "The archive says where a verse begins in two places that can disagree, so a bible from there is read twice and the two readings laid against each other. usfm says it once, and a Sword module says it once. There is nothing there to check, and reaching for the check anyway would mean fetching the same bible a second time from a place it did not come from - which for the Berean would fetch the older printing and lay it against the newer one as though a difference were a fault.";
  "Asked as one question about the text rather than as a list of catalogues, because that is what the roads below actually want to know. They do not care who published a bible; they care whether the words came already marked. Three sources here ship marked text and the archive does not, and a further one that did would answer yes with nothing below edited.";
  ...
  let either = or(elsewhere, published);
  let marked = or(either, packaged);
  return marked;
}
