...
export function gloss_explain_roots_named_word_given(word, explain, known) {
  "Every root a gloss explanation names, read the four ways the reader handed the sentence alone reads them plus a fifth that only a reader holding the word can read at all, with the one ambiguous wording of the four made to either spell its answer inside the word or name something a vocabulary has met.";
  ...
  "A third mark exists and is deliberately not used, written down here so the next reader does not have to find it again. The sentences that really name a root go on to describe the building - with -a on the end for a command, with gipa- in front and its sound shifted inside the word - and the ones giving an English meaning simply stop at the meaning. That is stronger evidence than spelling, because it is the writer saying which of the two things the sentence is doing rather than a reader inferring it. It is not used because the vocabulary already leaves only five sightings wrong across the whole store, and a fifth wording read off a hunch is precisely what went wrong four times running here. Handing an empty vocabulary is meaningful rather than broken. It gives the reading spelling alone would give, so the two ways of doing this are one function asked with different second chances rather than two functions to keep in step. That is deliberate: the choice between them is which set to pass, not which function to call.";
  ("The fifth wording is asked last and is the reason this reader is worth having beyond the one wrong class it fixes. An explanation may name its root before it names anything else - Buhat is to do. Gi- tells it from the side of the deed - and a reader given only that sentence cannot tell the root it opens on from the word it is about, so all four of the other wordings call it silence. Measured over the store it is 6560 sightings across 803 roots that nothing was reading. ",
    fn_name("gloss_root_named_reversed_is"),
    " holds the five marks that separate it from a repeated headword, a quoted affix and half a two word entry, and it is asked after the other four rather than before because those are the writer saying what they are doing and this is a reader inferring it from where a quotation mark fell.");
  ...
  function opening_kept_is(root) {
    let names_root = gloss_root_named_reversed_is(word, root);
    return names_root;
  }
  let opening = gloss_explain_roots_opening_named(explain);
  let opening_kept = list_filter(opening, opening_kept_is);
  let r = list_map(opening_kept, text_punctuation_edges_removed);
  return r;
}
