...
export function app_code_lesson_expression_worked_card_two_operators(
  ...
  ("The last row names the answer rather than writing the whole line out again. The whole line already stands at the top of the card, three rows up, so repeating it says nothing the reader cannot see - and on a line carrying brackets it is long enough to wrap on a phone, which puts the value the row exists to give at the end of a second line.");
