...
export function app_code_lesson_expression_choose_order_sides_settled_tree(
  ...
  ("The one in the middle is handed in rather than drawn here, so a caller wanting the same line twice with the parentheses moved gets the same three truths both times and the second line reads as the first one turned round.");
  ("How the parts are hung together is handed in as well, because that is the other thing the lessons using this differ by: one lets the operators decide the shape and the other puts parentheses round the or.");
