...
export function js_rewrite_then_read_pairs(ast) {
  ...
  ("THE WORD IT SPELLS IS TAKEN AS THE THING IT REWRITES, which is wider than the truth and is meant to be. Following the word from the marker into the argument of the rewriting call would mean tracing a local through the body, and a name spelled for some other reason costs only a pairing that the reach question then throws away. Missing the real one would cost the whole point.");
  ...
  let targets = js_fn_name_literals(ast, f_name);
  let pairs = [];
