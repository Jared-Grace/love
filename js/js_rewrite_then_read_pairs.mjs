...
import { js_rewrite_fixed_sites } from "./js_rewrite_fixed_sites.mjs";
...
export function js_rewrite_then_read_pairs(ast, writers) {
  ...
  ("A REWRITE IS ALSO ASKED FOR WITH NO NAME AT THE CALL SITE AT ALL, and that shape had to be handed in because it cannot be read here. A writer that always writes the same file is called by its own name and empty brackets, and the only thing in this tree that would give it away is a name it never spells. So the writers arrive already worked out, and a caller told nothing about them is read exactly as it was before - which is what the landing command for a picture Bible chapter was, on the day it wrote a chapter into the list and left both chains without it.");
  ...
  let writer_names = list_map_property(writers, "f_name");
  let rewriters = list_concat(seams, writer_names);
  let rewriting = list_intersect(imports, rewriters);
  ...
  let spelled = js_rewrite_targets(ast);
  let fixed = js_rewrite_fixed_sites(ast, writers);
  let sites = list_concat(spelled, fixed);
