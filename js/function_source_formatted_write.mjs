...
export async function function_source_formatted_write(f_name, contents) {
  ...
  ("IT ONLY EVER MAKES A FILE THAT IS NOT THERE YET, and a caller wanting to replace one has to reach for the overwriting writer beside it. The layer underneath asserts the file is absent before it writes a byte, so a name the repo already answers to throws here rather than being quietly replaced.");
  ...
  ("The three steps between working out the path and telling the index are held one name down, shared with the overwriting twin, and the only thing said here is which writer decides what happens to a file that is already there.");
