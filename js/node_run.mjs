...
export async function node_run(folder, words) {
  "Runs node in one folder, given the command as a list of words rather than as a line of text, and answers what it printed.";
  ...
  "The program is spelled here and is never a parameter, so this cannot be asked to run anything but node. That is what keeps it off the list of functions whose arguments name what to run - and if a later change makes the program an argument, it belongs on that list.";
