...
export function app_code_lesson_expression_parentheses_worked_card({
  ...
  ("one worked card for the lessons that teach the parentheses: three true or false words joined by an && and an || , with the marks round one pair or the other, carried all the way to its value");
  ...
  let inside_first = ternary(parentheses_left, left, middle);
  let inside_second = ternary(parentheses_left, middle, right);
  let outside = ternary(parentheses_left, right, left);
  ...
  let parenthesised = js_code_wrap_parenthesis(pair);
  let whole_left = app_code_operator_code(parenthesised, outer_symbol, right_word);
  let whole_right = app_code_operator_code(left_word, outer_symbol, parenthesised);
  let whole = ternary(parentheses_left, whole_left, whole_right);
  ...
  let rest = ternary(parentheses_left, rest_left, rest_right);
