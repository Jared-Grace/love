...
export function bible_usfm_chapter_lines(usfm, chapter_number) {
  ...
  ("One chapter of a book written in usfm, as lines of usfm ready to be read - still marked up, but put back together, cut down to the chapter asked for, with every mark standing on the line it belongs to, and with the printing's own division subtitles taken out.");
  ("It is the four tidyings every reader of a chapter has to do before it can read anything at all, and none of them is about what the reader then wants. A reader laying the chapter out as writing and a reader cutting a passage out of it both need exactly this and differ only afterwards, so doing it in each of them would be the same four steps written twice, in the same order, with nothing to say if one of them ever changed.");
  ...
  ("Last, a line the printing sets as a psalm ascription but does not count as scripture is taken out, and that one is asked of the whole book rather than of the chapter, which is why the lines of the whole book are still in hand here and nowhere below.");
  ...
  let out = bible_usfm_lines_division_subtitles_dropped(usfm_lines, lines);
  return out;
}
