...
export async function bible_glyph_chapters_verse_marks_offenders_word_draw(
  offenders,
) {
  "Draws the missing mark on every named verse where the interlinear and the English agree on one single word, and hands back the verses it refused to settle.";
  "A READING NAMES A CHAPTER, A VERSE AND A MARK, AND STOPS THERE. It counts how many times the original seats a picture against how many times the verse draws it, so it can say a verse is one mark short; what it can never say is which English word the missing mark belongs on, because counting compares totals and never positions. That last step was being taken by hand, one verse at a time, and a step taken by hand over a list is a command that has not been written yet.";
  "IT TAKES THE LIST RATHER THAN ASKING FOR IT, and that is the whole reason this is its own function (2026-09-18). The decision below never once looks at where an offender came from - it needs a chapter, a verse, a glyph, how many seats and how many were drawn, and nothing else. While it sat inside the caller that asks the shrink-only underdrawn reading, a second reading with a different bound could not reach it, and the only way to widen the reach was to widen that reading - which moves a gate that is ratcheted against a baseline. Splitting the taker from the asker costs one function and buys every later reading this judgment for free.";
