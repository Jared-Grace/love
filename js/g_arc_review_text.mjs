...
export function g_arc_review_text(arc, passages) {
  ...
  let lines = [];
  for (let name of ["occupation", "trouble", "summary"]) {
  ...
  for (let conversation of conversations) {
    ...
    let item2 = text_combine_multiple(["CONVERSATION ", conversation_number]);
    list_add(lines, item2);
    let catch_up = property_get(conversation, "catch_up");
    let b = text_empty_is(catch_up);
    let caught_up = not(b);
    if (caught_up) {
      let item3 = text_combine_multiple(["catch_up: ", catch_up]);
    ...
    for (let turn of turns) {
      ...
      let changed = not(b2);
      if (changed) {
        let item4 = text_combine_multiple(["  opener: ", opener]);
