...
export function bless_vehicles_drive(world, container_map) {
  ...
  ("finishes that slide at once instead: at most one square, once per celebration.");
  ("That excuse was once written as 'on a street the player is not looking at, because the");
  ("camera has gone to the house that just lit up'. It was true of a celebration and FALSE of");
  ("a crossing, and a camera journey is how both of them are made. Stopping at a kerb brings");
  ("the camera in close on the road on purpose, because the car being waited for is the whole");
  ("of what there is to watch - so the one journey where the player is staring straight at the");
  ("traffic was the one that froze it, twice, going in and coming out. The hold is now taken");
  ("for the single frame that changes a layout rather than for the length of a journey, which");
  ("is what makes the sentence above true again.");
  ("An excuse of the form 'nobody can see this' names a place the reader has to go and check,");
  ("and it is only ever as good as the widest caller. This one was written by a caller who was");
  ("right about itself.");
