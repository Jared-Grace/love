...
export function app_shared_contact_overlay() {
  "an in-app contact screen: a full-screen panel over the current app, so the reader never leaves the PWA (each app is its own narrow-scope standalone PWA, so opening another page would eject them to the browser). Sends the message to the developer's inbox, then thanks them. Close returns to the app.";
