...
export function app_shared_bible_screen_open(context, back) {
  ...
  "NOTHING IS SET ON THE PAGE ROOT, and the centring is the reason that rule had to be widened. Emptying the page clears what is in it and leaves the styles on it, so a screen that centred the root centred every screen drawn after it for the rest of the visit - and the app that showed it was the picture Bible, whose verses came back from the settings screen centred with no way to read from the code that the settings screen was what had done it. A style put on a fresh box goes when the box goes.";
  arguments_assert(arguments, 2);
  let page = html_clear_context(context);
  let screen = html_div(page);
  html_centered(screen);
  html_page_padding_x(screen);
  let content = html_div(screen);
