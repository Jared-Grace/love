...
export function js_statement_work_is(node) {
  ...
  ("Four kinds are statements and are not work. A block is a container, so counting it counts the same run twice, once as the walls and once as what is inside them. A string on its own is how this repo writes a comment. A marker call is left for a reader in the same way. So is a paragraph a bracket and a comma turned into a pair, which is a comment wearing a shape no reading that looks for a lone string can see.");
