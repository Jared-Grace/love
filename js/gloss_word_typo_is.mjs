...
export function gloss_word_typo_is(written, explained) {
  ...
  "Short words are refused however close they look, and that is the whole of what keeps this safe. Two edits over three letters is most of the word, so the pair are simply different words that happen to stand near each other, and correcting one to the other would put a spelling nobody wrote under an explanation about something else.";
  let distance = text_edit_distance(written, explained);
  let near = less_than_equal(distance, 2);
  let size_written = text_size(written);
  let size_explained = text_size(explained);
  let longest = list_max([size_written, size_explained]);
  let long_enough = greater_than_equal(longest, 5);
