...
export function harmony_bass_place_score(chord, bass_pitch) {
  ...
  "root and third sit close together on purpose because a bass note that is the third of one chord is the root of another and only the music around them decides which so the chooser should come out unsure and say so rather than pick by a wide margin it has no grounds for";
