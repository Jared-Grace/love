...
export function bible_usfm_interlinear_words_apart_allowed() {
  ...
  ("The ten chapters where the Berean read off the usfm shelf and the Berean printed in the interlinear tables do not say the same words, with exactly which words each side holds over the other, and beside each one the reason somebody opened the verse and decided the reading was not at fault.");
  ("★ EVERY ENTRY WAS OPENED AT THE VERSE THE SWEEP NAMED, ON BOTH SHELVES, WHICH IS THE ONLY THING THAT MAKES THIS AN ALLOWANCE RATHER THAN A SHRUG. A list written straight off a run says nothing except that the run happened, and a check resting on it passes because it was told to. What stands here is the reading of ten verses on two shelves, and three of the first reasons written down were wrong and were corrected by looking.");
  ("★ NOT ONE OF THE TEN IS THE READING HANDING OVER A WRONG WORD ANY MORE, AND THAT IS WHAT CHANGED. There were thirteen, and five of them were the publisher's own marked-up file missing a space or carrying three stray letters, so a person copying that chapter was given a word no dictionary holds. Those five are now put right on the way in, one place at a time, each against a second published edition of the same translation, and the list of them is kept apart. What is left here is the tables' own faults and the two editions honestly differing, which is a thing to be recorded and not a thing to be mended.");
  ...
  ("THEY FALL INTO SIX CAUSES AND NOT ONE. Three are stray page markup left inside the interlinear tables, whose letters read as words on that side. Two are three stray letters the tables carry that the shelf is now mended of, so the sweep has turned round and is pointing at them. Two are the two editions wording a clause differently. One is the reading keeping the square-bracketed words the printing supplies, which the tables drop on purpose. One is a clause the printing supplies where the Hebrew has nothing and the tables leave out. One is a line the printing sets as a note under the last verse.");
  ("NOT ONE OF THE TEN IS THE READING ADDING OR LOSING A LINE OF ITS OWN, which is the fault this was built after - a book-division subtitle written with the psalm ascription mark was being handed to whoever copied Psalm 107, and it opened the passage with the words Psalms 107-150. Every psalm now agrees word for word except one, and that one is stray markup on the tables' side.");
