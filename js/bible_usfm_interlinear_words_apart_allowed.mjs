...
export function bible_usfm_interlinear_words_apart_allowed() {
  ...
  ("The eight chapters where the Berean read off the usfm shelf and the Berean printed in the interlinear tables do not say the same words, with exactly which words each side holds over the other, and beside each one the reason somebody opened the verse and decided the reading was not at fault.");
  ...
  ("THEY FALL INTO FIVE CAUSES AND NOT ONE. Three are stray page markup left inside the interlinear tables, whose letters read as words on that side. One is three stray letters the tables carry that the shelf is now mended of, so the sweep has turned round and is pointing at them. There were two; the other, she named vvv him Ben-oni in Genesis thirty five, stopped standing apart once the tables' reading learned to drop a filler word sharing a row with English, and its line was taken out. Two are the two editions wording a clause differently. One is the reading keeping the square-bracketed words the printing supplies, which the tables drop on purpose. One is a clause the printing supplies where the Hebrew has nothing and the tables leave out.");
  ("NOT ONE OF THE EIGHT IS THE READING ADDING OR LOSING A LINE OF ITS OWN, which is the fault this was built after - a book-division subtitle written with the psalm ascription mark was being handed to whoever copied Psalm 107, and it opened the passage with the words Psalms 107-150. Every psalm now agrees word for word except one, and that one is stray markup on the tables' side.");
