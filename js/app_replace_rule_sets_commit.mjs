...
import { git_folder_commit_file_path } from "./git_folder_commit_file_path.mjs";
import { git_folder_commit_tree_run } from "./git_folder_commit_tree_run.mjs";
...
export async function app_replace_rule_sets_commit(commit) {
  ...
  let file_path = await git_folder_commit_file_path(folder, commit, file_name);
  ...
  let rule_sets = await git_folder_commit_tree_run(
    folder,
    commit,
    lambda$tree_folder,
  );
