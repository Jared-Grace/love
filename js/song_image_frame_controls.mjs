...
export function song_image_frame_controls(column, state, on_change) {
  ...
  html_style_set(placement, "padding", "6px 12px");
  html_style_set(placement, "cursor", "pointer");
  function flip_click() {
    state.flip = not(state.flip);
    on_change();
  }
  let flip = html_button(controls, "swap sides", flip_click);
  html_style_set(flip, "padding", "6px 12px");
  html_style_set(flip, "cursor", "pointer");
  html_style_set(flip, "opacity", equal(state.placement, "half") ? "1" : "0.4");
  return controls;
}
