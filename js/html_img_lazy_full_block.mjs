...
export function html_img_lazy_full_block(picture) {
  ...
  "AS WIDE AS THE PAGE ALLOWS, ON ITS OWN LINE, WHICH IS THE STARTING POINT AND NOT THE LAST WORD. Where the picture is what the reader came for, that is the whole answer: these are single shapes and a reader whose eyes no longer read small things needs the size. Where the picture sits beside something else that is being read, the caller puts a ceiling over this and the width becomes as wide as the ceiling allows - which is what the music page does, because there the hymn is what is being read and a full-width drawing between every two lines pushes the song off the screen.";
  "What the picture is of, what shape its corners are, and how large it may finally be drawn are left to the caller - they differ picture by picture, and these four do not.";
