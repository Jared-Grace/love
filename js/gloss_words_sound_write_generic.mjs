...
export async function gloss_words_sound_write_generic(words, sound_fn, voice) {
  ...
  ("★ THE WORKING FOLDER IS FILED UNDER THE VOICE THIS CALL WAS ASKED FOR, WHICH IS ONLY TRUE IF NOTHING ELSE IS SPEAKING AT THE SAME TIME. What a killed run leaves behind carries no record of who spoke it, so a second run started while the first is still going would file the first one's leftovers under the second one's voice. Record one voice at a time.");
  ...
  ("★ EVERY WAY A LONE WORD DIFFERS FROM A CHAPTER IS ASKED FOR HERE, RATHER THAN LEFT TO THE ENGINE'S OWN SETTINGS, because the engine is asked for both jobs and only the caller knows which one it is. There are four, and each keeps its reasoning next to its value: the speed in ",
    ...
    ", who says it in ",
    fn_name("gloss_word_sound_voices"),
  ...
  let filed = await gloss_words_sound_pending_file(pending, folder, voice);
  await folder_delete(pending);
  let r = {
    voice,
    words: list_size(words),
