...
import { app_code_expression_written_shapes_untaught } from "./app_code_expression_written_shapes_untaught.mjs";
import { list_concat_multiple } from "./list_concat_multiple.mjs";
export function app_code_expression_written_shapes() {
  arguments_assert(arguments, 0);
  ("every shape to hold a printed line against: the arithmetic ones, the ones that come out true or false, and the ones built with the two operators the app can work out that no lesson teaches");
  ...
  ("The untaught part is what closes the gap between the two. The operators a shape MAY hold and the operators any lesson does hold were never the same set, and the difference was where both printing faults lived.");
  let numbers = app_code_expression_written_shapes_numbers();
  let truths = app_code_expression_written_shapes_truths();
  let untaught = app_code_expression_written_shapes_untaught();
  let parts = [numbers, truths, untaught];
