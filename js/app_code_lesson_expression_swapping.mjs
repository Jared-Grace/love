...
export function app_code_lesson_expression_swapping() {
  "the next step after both-sides-arithmetic: put the SAME two numbers on each side, but swapped around the operator - a op b === b op a. The idea is commutativity framed honestly as CAN-always-swap vs CANNOT-always-swap: + and * survive a swap for ANY two numbers (always true), while -, /, %, <, >, <= and >= survive only when the two numbers are equal (a no-op), so for two DIFFERENT numbers they flip and it is false. That is why the quiz uses distinct pairs - equal operands can't tell the operators apart, because f(a, a) === f(a, a) is vacuously true for ANY binary operator (swapping equal args is a no-op). ** is left out: 2 ** 4 === 4 ** 2 is 16 === 16, the one distinct pair where swapping exponent does not flip. This feeds the chained-comparison pitfall (2 < 5 < 3) later. Answer is the code's own true/false value, correct by construction. Comparisons parse cleanly without parentheses because < and > bind tighter than ===.";
