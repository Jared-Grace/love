...
export async function bible_glyph_chapter_verse_text_write(
  ...
  "A SHORT VERSE IS STORED ON ONE LINE AND A LONG ONE OVER MANY, AND BOTH HAD TO BE UNDERSTOOD. The formatter puts a word list on a single line whenever it fits, so a verse of four words reads words colon bracket and closes on that same line. The first version of this walked forward from the line after the word list looking for the closing bracket, which for a short verse is a bracket belonging to some later verse - so writing the thirteenth of the twentieth of Exodus deleted the fourteenth, fifteenth and sixteenth, and the commandments against adultery and stealing went with them. The two shapes are told apart before anything is cut.";
  ...
  list_add(written, "        ],");
  let opens = text_trim(lines[start + 1]);
  let is_words = text_starts_with(opens, "words: [");
  if (not(is_words)) {
    error({
      hint: "the line after the verse number does not open the verse's word list, so the chapter file is not shaped the way this expects",
      f_path,
      verse_number,
      opens,
    });
  }
  let one_line = text_ends_with(opens, "],");
  let head = lines.slice(0, start + 1);
  let tail = [];
  if (one_line) {
    tail = lines.slice(start + 2);
  }
  if (not(one_line)) {
    let closer = -1;
    let index = start + 2;
