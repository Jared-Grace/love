...
export async function git_hook_commit_msg_write() {
  ...
  "A FOLDER THAT IS NOT A REPOSITORY IS PASSED OVER RATHER THAN FAILED ON. The list is every folder sitting beside this one, and whether each is a checkout is not something the list knows; writing into a git folder that is not there would throw and take the whole install down with it, over a folder nobody was ever going to commit in.";
  ...
  let all = await repos_paths_memory_map_unordered(each_folder);
  let r = {
    written: all,
  };
  return r;
