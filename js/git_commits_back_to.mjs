...
export async function git_commits_back_to(folder, floor) {
  ...
  let named = await git_folder_run(folder, ["rev-parse", floor]);
  let floor_full = text_trim(named);
  let based = await git_folder_run(folder, ["merge-base", floor, "HEAD"]);
