...
export function html_code_loading_splash() {
  "the static loading splash baked into the app HTML body so a refresh paints the dark spinner INSTANTLY (before the bundle even downloads) instead of a white flash. self-contained: inline styles plus the shared keyframes, no external CSS or JS. carries the id app-loading so the shared boot can drop it once the app has loaded. shows the SAME spinner and message as the runtime overlay, single-sourced, so the dark-to-dark, spinner-to-spinner handoff is seamless; that overlay lives on the document element (surviving body clears) and takes over the moment scripts boot, this only covers the gap before that";
