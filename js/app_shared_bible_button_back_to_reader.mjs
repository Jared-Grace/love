...
export async function app_shared_bible_button_back_to_reader(parent, context) {
  ...
  "the chapter is taken from what was remembered and the verses from the link, because those are the two words that keep their truth for different lengths of time. the chapter in the link is overwritten the moment a book is tapped; the verses in it are written by the verse picker, which hands the reader home rather than leaving them here, so they still say what was picked before any of this began - and they say it more recently than the remembering does, which was taken when the reader was last drawn. the remembering answers for them only when the link says nothing at all, which is how a reader shown its own default verse still gets named by it";
