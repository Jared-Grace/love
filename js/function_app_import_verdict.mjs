...
export async function function_app_import_verdict(imported) {
  ...
  ("Callers are not the only evidence of belonging, and where they are absent the callees answer instead. A check reaches into the app it checks and nothing in the app calls it back, so counting callers alone reads it as a lie and asks for a rename - which was tried on five of them and made things worse rather than better, because each check carries one or two reaches of its own, so four reported faults became nine and the record could not even hold them. What the name says and what the unit uses agreed all along; it was the reading that was wrong.");
  ("So a name with no callers of its own that reaches into its own app is answered separately, and the answer is that nothing about the name should move. Whatever is really wrong sits in the importer instead - most often a list of gates belonging to no app naming a gate that does.");
