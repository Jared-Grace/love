...
export async function lyric_video_bible_document_pictures_times_write(
  ...
  for (let index = 0; less_than(index, count); index += 1) {
    ...
    let opening = 0;
    let own = lines[picture.line].start;
    picture.start = first ? opening : own;
    let after = index + 1;
    let last = equal(after, count);
    let closing = document.duration;
    let next = last ? closing : lines[pictures[after].line].start;
    picture.end = next;
