...
export function app_code_lesson_expression_parentheses_both_sides_card_flat_line(
  ...
  ("the same line without the parentheses, solved all the way through one replacement at a time, and then what that leaves us needing the parentheses for");
  ("The card opens with the question rather than the answer. Here we need the ( and ) stated the conclusion first, which is a forward reference on a card whose whole job is to earn it - the reader is asked to accept that the parentheses are needed, and only then shown why. Asked as a question, the three blocks read in the order they are true in: what if they were not there, here is what the line comes to, so that is what they are for.");
