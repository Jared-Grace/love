...
export async function git_history_messages_strip_rehearse() {
  ...
  ("★ THE PRICE IS EVERY COMMIT NAME FROM THE FIRST CHANGED ONE ONWARDS, AND THAT IS WHY THIS IS A PASSENGER AND NEVER A REASON. Fourteen messages carrying a folder layout are worth nothing like the cost of orphaning every commit name anybody has written down, and taking a word out of a log that has already been sent to a public address does not take it back - whoever cloned in between has it, and the addresses hold it until they are made to forget. So this is built to be ready, to be run inside a sitting that was going to rewrite the history anyway, and never to be the thing that opens one.");
  ...
  let offending_live = await git_folder_messages_text_commits(folder, prefix);
