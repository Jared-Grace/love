...
export async function git_history_messages_strip_rehearse() {
  ...
  ("Two texts are taken out and not one, and which two and in what order is ",
    fn_name("git_history_folder_strip_texts"),
    "'s to say rather than this one's. The last of them is the wider, so it is the one the counting asks for.");
  ...
  let texts = await git_history_folder_strip_texts(folder);
  let home = list_last(texts);
