...
export function app_code_lesson_quiz_token_select_counts_set(
  ...
  ("THE TIMES SIGN IS PART OF WHAT IS SHOWN, and it is not decoration. A small raised two after a seven is how mathematics writes seven squared, and this is a course about writing expressions - the one reader who must not misread it is the only reader it is for. Times two cannot be read as a power of anything.");
  ...
  function each_button(b, index) {
    ...
    app_code_lesson_quiz_token_select_count_badge_style(badge);
  }
