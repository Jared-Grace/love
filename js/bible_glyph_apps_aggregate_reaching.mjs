...
export async function bible_glyph_apps_aggregate_reaching() {
  ...
  ("THE THREE NAMED FUNCTIONS EACH HOLD THE WHOLE PICTURE BIBLE. One holds all twenty five chapters, one holds the written-out Rosetta bands for all twenty five, and one holds the whole Tagalog. They are written that way on purpose, because everything on this side of the machine - the writers, the surveys, the gates that read across chapters - genuinely wants all of them at once and there is nothing to be gained by making a program that is about all twenty five ask for them one at a time.");
