...
export async function lyric_video_psalms_documents_time(version) {
  ...
  ("★ EVERY SINGING IS HEARD, NOT ONE PER CHAPTER, BECAUSE EACH ARRANGEMENT PUTS THE LINES SOMEWHERE ELSE. Hearing only the earliest recording of a chapter left the others holding the flat spread they were drafted with and no command able to reach them. Each is asked for by its chapter and its take together, which is the pair that names one recording.");
  ...
  for (let song of songs) {
    let args = [version, song.chapter, song.take];
