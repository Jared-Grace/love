...
export async function gloss_word_sound_spoken_move(
  ...
  "★ THE VOICE IS HANDED IN RATHER THAN READ OFF THE RECORDING, BECAUSE NOTHING IN A RECORDING SAYS WHO MADE IT. A run speaks the whole batch in one voice and the caller is the one who asked for that voice, so the caller is the only side that knows. Filed under the wrong voice a recording is not lost and nothing goes red - the reader simply hears one person while the cycle believes it is playing another.";
  "The folder it goes into is the voice's own, and the caller has made sure that folder exists before the first of these runs. It is not made here, because these run all at once and the last thing wanted is several hundred of them each deciding whether to create the same folder.";
  ...
  let key_path = gloss_word_sound_path(word, voice);
  let sound_to = path_join([folder, key_path]);
  let already = await file_exists(sound_to);
  if (already) {
    return;
  }
  await file_move(sound_from, sound_to);
  list_add(filed, key_path);
}
