...
export async function data_identifiers_search_any(ids_comma) {
  ...
  let ids = text_split_comma_dot_trim(ids_comma);
  let found = {};
  for (let id of ids) {
    async function search_one() {
      let callers_one = await data_identifiers_search(id);
      return callers_one;
    }
