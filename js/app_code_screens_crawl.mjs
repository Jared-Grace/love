...
export async function app_code_screens_crawl(url_prefix) {
  "the durable regression crawler: walk EVERY code lesson (examples + every quiz kind via Next) in a headless browser, capturing each screen and collecting any javascript errors. url_prefix is the served code.html, e.g. http://localhost:8080/love/public/dev/code.html . Returns a compact summary of the mechanical failures (horizontal overflow, blank screens, page errors) - the deterministic half of the check; the captured text feeds the separate make-sense judge. Reuses the same playwright harness as the app_replace e2e tests";
