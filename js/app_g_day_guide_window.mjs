...
export function app_g_day_guide_window(player, tile, viewport, grid, content) {
  ...
  let centre_x = add(grid_left, multiply(add(player_x, 0.5), tile));
  let centre_y = add(grid_top, multiply(add(player_y, 0.5), tile));
  let wanted_x = subtract(centre_x, divide(view_width, 2));
  let wanted_y = subtract(centre_y, divide(view_height, 2));
  ...
  let min_x = divide_ceil(negative(origin_x), tile);
  let min_y = divide_ceil(negative(origin_y), tile);
  let past_right = divide_floor(subtract(view_width, origin_x), tile);
  let max_x = subtract(past_right, 1);
  let above_bar = subtract(subtract(view_height, origin_y), bar_height);
