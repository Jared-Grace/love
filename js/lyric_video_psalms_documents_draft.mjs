...
export async function lyric_video_psalms_documents_draft(version) {
  ...
  ("★ EACH SINGING IS DRAFTED SEPARATELY, BECAUSE THE TIMES OF ONE ARRANGEMENT FIT NO OTHER. The renditions of a chapter differ in length, some of them by a third, so a line that lands at forty seconds in one lands nowhere near it in another; one document shared between them would be right for whichever was drafted first and wrong for the rest. Where the second and later singings are written is worked out from the take the recording's own name carries, so the first singing keeps the address it already had and nobody's corrected times are stranded under a name nothing reads.");
