...
export async function bible_audio_verses_manifest_write_all() {
  ...
  "★ THE FOUR ANSWERS ADD UP TO EVERY RECORDING, AND THAT IS CHECKED BY CONSTRUCTION RATHER THAN HOPED FOR. Every recording lands in exactly one of clean, stale, cut some other way, and could not be asked, because each is reached by a return that leaves no path to a second one. A sweep whose parts do not sum to its total has looked at a category it never named.";
