...
import { file_name_js_is } from "./file_name_js_is.mjs";
...
("regardless of read order or import spacing (",
  fn_name("js_format_trim"),
  " = the real write-time pass).");
("Only a file whose name says it holds code is formatted; anything else - a page, a piece of");
("data - is compared as it was written, because the formatter reads JavaScript and would");
("refuse the rest.");
export async function example_files_canonical(files) {
  async function to_block(f) {
    ...
    let code = file_name_js_is(name);
    let formatted = code ? await js_format_trim(source) : source;
