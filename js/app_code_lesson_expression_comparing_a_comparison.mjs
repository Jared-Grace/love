...
export function app_code_lesson_expression_comparing_a_comparison() {
  ...
  "Taught as replacing rather than as evaluating: a comparison works out to a true or false, so it can be replaced by that true or false, and the line that is left is one the learner already knows how to read. Order is not raised at all: every line here holds one comparison and one true or false, so there is only ever one place a replacement could go, and replacing lands on the right value on its own. Choosing WHICH part to replace, and why the choice matters, is a lesson of its own.";
