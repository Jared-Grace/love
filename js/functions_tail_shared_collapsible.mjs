...
export async function functions_tail_shared_collapsible(shared_name) {
  arguments_assert(arguments, 1);
  ("Every function in the repo that ends with a copy of the named function's body, sorted into the ones whose copy could be swapped for a call to it and the ones whose copy could not, each of those carrying the reason.");
  ("The mirror at the other end of the function of the same reading made at openings. It matters more than that one rather than less: endings are where this repo repeats itself, because functions are told apart by what they go and fetch and agree about what to do with it once they have it.");
  ("This reads and reports and changes nothing, which is the whole reason it exists apart from the swap. The refusals are the part worth reading. Each one is a function that really does end with the same work and still cannot take the call, so each is either a hazard the swap would have walked into or a rule drawn more tightly than it needed to be, and there is no way to tell those apart without looking.");
  ("The named function has to be a run of work handing back one name that run made, or there is nothing a copy of its body could be swapped for, and it has to read every name it was handed, or a call could be written with nothing to put in the gap.");
  ("Its own name is skipped, since a function trivially ends with a copy of itself.");
