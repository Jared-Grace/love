...
export async function functions_gate_run_unwired() {
  ...
  "A gate is a function whose name ends in the two words the whole-repo gate's own members end in, and it is run by being a member of one of the two lists and by nothing else. So a gate that was written, tested by hand once, and never added is not a weak gate - it is a gate that has never run, and it reads exactly like the ones that do: same name, same shape, green when you call it.";
  ...
  "There are two lists and there always were - one asked inside a frozen copy, one asked out here of this machine - and this read only the first of them. Every gate correctly listed in the second was therefore reported as never run, and the fix offered was to add it to a list where it cannot work or to write down an excuse for standing outside a list it is not outside of. Both would have been false. A detector that names one roster has to be reread each time a second one appears, because its silence about the other is indistinguishable from there being no other.";
  ...
  "Both lists are read off the file rather than off the loaded one, because a command that lists a gate and then asks this again would otherwise be answered from before its own edit, and hand its own finished work back as still to do.";
