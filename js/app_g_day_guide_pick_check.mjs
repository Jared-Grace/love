...
export function app_g_day_guide_pick_check() {
  ...
  assert(gold !== null, "guide should return a gold tile for an off-screen target");
  const in_window = gold.y >= 5 && gold.y <= 11 && gold.x >= 0 && gold.x <= 11;
  assert(in_window, "gold tile must be inside the visible window");
  assert(gold.y < player.y, "gold tile must lead toward the target (upward)");
  return {
    gold: [gold.x, gold.y],
  };
}
