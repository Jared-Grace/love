...
export function js_statements_span_cuttable_is(statements, tail) {
  ...
  ("The cut refuses on exactly these three readings, so a run this lets through is a run the cut will take. All three refusals live next door with the reasons written out: a jump that leaves the run has nowhere to land, a name the run writes to but did not bring into being would be written on a copy, and and a name a function inside the run holds would become two names spelled the same way once the lines behind write to it.");
