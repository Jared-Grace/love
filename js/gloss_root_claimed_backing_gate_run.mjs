...
export function gloss_root_claimed_backing_gate_run() {
  ...
  "The pairs that must come back root guard the other direction. Every one of them was standing in the queue as a fault until the claim was compared with the given root, so a reading that stopped doing that would not fail anywhere except in what a person is asked to read.";
  ...
  while (less_than(i, size)) {
    ...
    let read = gloss_root_claimed_backing(root, root_chain, claimed);
