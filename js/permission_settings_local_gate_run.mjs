...
export async function permission_settings_local_gate_run() {
  ...
  ("Which of the two is open depends on what the rule names. A rule naming a dispatcher function has somewhere to go: the shared list is generated from function names, so naming it there moves the approval under everything that watches. A rule naming a fetched site has somewhere to go as well: the shared list is not made of function names alone, and a site is written into it by ", fn_name("permission_rules_other_domain_add"), " and rendered beside them, so the approval stands where a check can read it. That was not true when this was written and the sentence here said it could not be done; it is true now. A rule naming anything else - a path to read, a command of its own - still has no place in that list, and for those the only honest answer is out of the local file, and whether an approval a person granted should be taken away again is theirs to say and not this gate's.");
