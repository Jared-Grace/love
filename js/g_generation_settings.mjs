...
export function g_generation_settings() {
  ...
  "What a turn COUNTS is one opener and one verse match, which are the same thing rather than two. A generated conversation is a list of turns and each turn holds both halves - the concern the npc voices and the verses to choose between - so the opener menu offers one button per turn still unplayed, and choosing an opener is choosing WHICH turn to play. Eight turns is eight openers and eight matches, not eight matches on top of eight openers.";
  "Asking for discernment costs nothing here. The prayer is a button rendered inside a turn whenever it offers more than one choice, so it may be prayed on every turn of a conversation and the conversation is still the length it was drawn at. That is why it is not in the setup price either - it is not on the way to a conversation and it is not a turn of one.";
  "So a turn and a match are ONE unit under two names, matches when a day is being budgeted and turns when a conversation is. Nothing converts between them because there is nothing to convert.";
  "The low end is a setting because getting into a conversation costs a fixed price in taps, and a conversation holding one or two turns spends more of itself on the approach than on the Scripture it exists to reach. It also fixes the most conversations a day can hold, which is why that number is worked out rather than written down.";
  ("That price is COUNTED now, in ",
    fn_name("g_conversation_setup_actions"),
    ", and it comes to five. The reason for a floor was written down long before the price was, and a number can only be checked against a price that has been counted.");
  ("Six against five is a conversation at the floor buying six turns for five taps - over break-even, which is what the floor is for. It went to eight for a while against a price of SEVEN, and seven was a miscount: the opener and the prayer for discernment were counted as approach, when a turn renders its own choices and its own discernment button. Charging a turn's own cost to the doorway made the doorway look dearer than it is, and the floor was raised to clear a price that was never there.");
  ("Eight was measured before it was withdrawn, and the measurement stands even though the reason for it did not. Over twenty scheduled plants it took days holding four people from forty-four to fifteen and days holding two from seventy-two to ninety-eight. That is the cost of the floor rather than an argument against six: raising it buys nothing here and spends variety, because at six the shortest conversation is already worth walking over for.");
  ...
  ("The day range is a BOUND on the live path and a DESCRIPTION on the other one, and it is worth knowing which is being read. The generator that runs cuts a plant when the sermon lines it has taken come to more days than the ceiling, so there a plant cannot go over; measured over one seed it made seventeen plants in three hundred and eight days, none of them short. The scheduler that lays conversations into days derives the length instead of bounding it, and there a run of long arcs simply runs long.");
  ("So the two disagree, and after the arcs were lengthened to thirty-six they disagree by more: twenty scheduled plants came out seventeen to twenty-four days, three of them past the ceiling. Nothing is broken by that, because nothing on the live path calls the scheduler - but a measurement taken from it is a measurement of a model the game does not use, and this range is not the place to reconcile them. Arc length moves the scheduler's days and cannot move the live one's, which come from preaching.");
  ...
  ("An arc's TURNS are drawn directly rather than counted out in conversations, because a conversation is itself a draw and multiplying two draws together says nothing either of them meant. Thirty-six is three conversations at the usual length - long enough to hear and believe and be answered once, short enough that a room holds a dozen of them.");
  ("It was thirty, and thirty put plants of seventeen and eighteen people past the sixteen a room may hold. The generator that runs fills a plant from the pool until the turns come out near what the preaching pays for, so the only two ways to spend that budget on fewer people are to stop taking them and waste the rest, or to make each one worth more turns. Lengthening the arcs spends every turn and buys depth with them; capping the count would have thrown them away.");
  ("Measured over a hundred and thirty-six plants across eight seeds: at thirty the sizes ran thirteen to eighteen, and at thirty-six they run twelve to sixteen and never once reach seventeen. The plants themselves did not move - how many there are and how many days each runs come from the sermon lines, not from the arcs.");
