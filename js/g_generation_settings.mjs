...
export function g_generation_settings() {
  ...
  "The CAST and the ROOM are two different counts and they are named apart. The cast is how many arcs get written for a chapter-group, drawn once and met by every player. The room is how many of those people one save groups together, which ramps from six. They were one set of numbers called plant_npcs, and that name made a false claim: it read as the size a plant may be, so it said a plant is never smaller than ten when a first plant is always six.";
  "The two maximums both hold sixteen and are still written twice. They are the same number today and they mean different things - the most arcs worth writing for a group, and the biggest a room may grow on a lucky roll - so either one can move without dragging the other with it.";
  ...
  let r = {
    ...
    plant_cast_minimum: 10,
    plant_cast_mean: 13,
    plant_cast_maximum: 16,
