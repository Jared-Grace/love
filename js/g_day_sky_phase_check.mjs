...
export function g_day_sky_phase_check() {
  ...
  assert_message(equal(g_day_sky_phase(0), 5), "fraction 0 → 6 AM sunrise (5)");
  let end = g_day_sky_phase(1);
  assert_message(
    greater_than(end, 9),
    "fraction 1 → 7 PM, past sunset into dusk (> 9)",
  );
  assert_message(less_than(end, 10), "fraction 1 → short of full night (< 10)");
  let mid = g_day_sky_phase(0.5);
  assert_message(greater_than(mid, 7), "fraction 0.5 → after noon (> 7)");
  assert_message(
    less_than(mid, 8),
    "fraction 0.5 → before afternoon peak (< 8)",
  );
  return {
    ok: true,
  };
