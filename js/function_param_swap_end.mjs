import { arguments_assert } from "./arguments_assert.mjs";
import { list_index_last_second } from "./list_index_last_second.mjs";
import { list_index_last } from "./list_index_last.mjs";
import { property_get } from "./property_get.mjs";
import { js_function_declaration_params_ast_get } from "./js_function_declaration_params_ast_get.mjs";
import { list_swap_at } from "./list_swap_at.mjs";
import { function_params_new_generic } from "./function_params_new_generic.mjs";
export async function function_param_swap_end(f_name) {
  arguments_assert(arguments, 1);
  await function_params_new_generic(
    function_transform_current_lambda,
    on_call,
    f_name,
  );
  function on_call(args) {
    let index_a = list_index_last(args);
    let index_b = list_index_last_second(args);
    list_swap_at(args, index_a, index_b);
  }
  function function_transform_current_lambda(ast) {
    let r = js_function_declaration_params_ast_get(ast);
    let params = property_get(r, "params");
    let index_a = list_index_last(params);
    let index_b = list_index_last_second(params);
    list_swap_at(params, index_a, index_b);
  }
}
