...
export async function functions_free_names_scoped_timed() {
  ...
  ("That last sentence stops being true once the folders are unchanging, and the two numbers then mean different things. Where each name is looked for only once and kept, the asking here pays for the looking and the reading below gets it free - so the finding figure is the whole cost of finding and the reading figure holds no finding at all. Where the folder is live, both askings pay, so the reading figure contains one finding and what was read is the difference between them. Subtract in the live folder; do not subtract in the frozen copy.");
