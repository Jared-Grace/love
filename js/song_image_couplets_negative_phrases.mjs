...
export function song_image_couplets_negative_phrases() {
  ...
  "THE TRAILING SPACE ON AND NOT AND ON BUT NOT IS LOAD-BEARING and must not be tidied away. Without it the word nothing is matched as and not, because and not is a prefix of and nothing - and that is not a near miss but the commonest negative in this file, so two couplets were reported for four rounds on a substring accident. Both of them said a region holds nothing else, which is the one negative shape measured here to be obeyed every time it is asked, so the detector was pointing at the two clauses least in need of it.";
