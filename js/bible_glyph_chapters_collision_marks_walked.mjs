  ...
  for (let chapter of chapters) {
    ...
    for (let row of rows) {
      ...
      for (let word of row.words) {
        ...
        previous_strong = closes ? "" : word.strong;
    ...
    for (let verse of parsed.verses) {
      ...
      let asked = property_get_or_null(wanted_by_verse, verse_number);
      let unasked = null_is(asked);
      let wanted = {};
      ...
      for (let glyph of object_property_names(drawn)) {
        ...
        let left = property_get_or_null(wanted, glyph);
        let unwanted = null_is(left);
        if (unwanted) {
          left = 0;
        }
        let distinct = [];
