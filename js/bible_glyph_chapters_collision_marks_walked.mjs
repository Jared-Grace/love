...
export async function bible_glyph_chapters_collision_marks_walked() {
  ...
  "PRESENCE DECIDES FIRST AND ORDER IS ASKED SECOND, ONLY WHERE THE TWO SIDES COUNT THE SAME. Presence answers on its own wherever the verse names one root of the pair, and a word the verse does not contain cannot be the one that was drawn. Where the verse names both, presence has nothing further to say, and one more question is worth asking before a person is sent for: if the chapter draws that picture exactly as many times as the interlinear WANTS MARKS FOR, the marks and the words pair off one for one, and each mark takes the root of the word standing in its place.";
  "HOW MANY MARKS THE WORDS WANT IS NOT ALWAYS HOW MANY WORDS THERE ARE. One word wanting one mark is the rule and the Greek emphatic double negative is the exception: two negatives that together mean certainly not, which a reader meeting two negation marks in a row cancels back into its own opposite. So the count the marks are held against is asked for rather than assumed, and the asking lives in its own reading.";
  ...
  for (let chapter of chapters) {
    ...
    for (let verse of parsed.verses) {
      ...
      let seen = property_get_or_null(numbers_by_verse, verse_number);
      let unseen = null_is(seen);
      let numbered = {};
      let counted = not(unseen);
      if (counted) {
        numbered = seen;
      }
      for (let glyph of object_property_names(drawn)) {
        ...
        let strongs = property_get_or_null(numbered, glyph);
        let unnumbered = null_is(strongs);
        if (unnumbered) {
          strongs = [];
        }
        let distinct = [];
