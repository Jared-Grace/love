...
export async function lyric_video_document_pictures_model_bench(
  ...
  "$plain model";
  "$plain size";
  "Draws every scene an authored lyric video document names a second time on a different model at one named size, into a folder of that run's own, so the two sets can be laid side by side.";
  "THE PLAIN RUN OF A MODEL, WHICH IS WHAT ITS FOLDER CARRYING NOTHING BUT THE MODEL AND THE SIZE MEANS. It asks for the picture with the scene's own wording and nothing else, so it is the thing every other run of that model is compared against - and that is why it is the one that takes the model's bare name.";
