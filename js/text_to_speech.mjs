...
export async function text_to_speech(args) {
  ...
  "★ THE WHOLE LINE IS READ, AND A READER THAT HUNTS FOR THE BRACKETS IN IT MUST NOT BE USED. One was, and it silently cut the report short: it takes the earlier of the last closing curly brace and the last closing square bracket, so a report whose final two characters are a square bracket then a curly brace loses the brace and will not parse. This report ends exactly that way, because its list of chapters is the last field. The line printed is already the whole answer, so there is nothing in it to find.";
