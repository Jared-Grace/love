...
export function app_g_day_guide_tile(g, player, target, div_map) {
  "the GOLD guide tile toward the discerned person — the THIN DOM wrapper: read the live viewport (scroll offset + client size + tile px), turn it into the FULLY-visible tile WINDOW, and delegate the path/geometry to app_g_day_guide_pick (pure, testable). the window is fully-in-view (ceil / floor-1) AND stops above the bottom bar (barH), so the chosen movable tile is never clipped by an edge or hidden under the bar. BESPOKE (querySelector / getBoundingClientRect / Math), do NOT auto-canonicalize";
