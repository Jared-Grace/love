...
export function bible_sentence_end_marks_findings_sorted(
  recorded,
  property_name,
  shipped,
) {
  "Sort what the measuring left behind into the kinds a person can act on: never measured, measured and no longer shipped, never answered for, empty in storage, read and empty, and read and finishing nothing.";
  "It is the half of the findings that asks nothing about the list of languages that write no marks. Every kind here is read off the record and the shipping list alone, so the naming question can be asked afterwards over a set already sorted rather than by subtracting one big list from another and offering everything that falls out the same single repair.";
