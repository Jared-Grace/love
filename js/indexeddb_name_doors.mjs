...
export async function indexeddb_name_doors() {
  ...
  "Narrowed to the functions whose names carry the browser database's own word, because the words looked for are ordinary ones and asking them of the whole repo would take every function that happens to call something a name.";
  "The word is asked for anywhere in the name rather than at the front. Four functions that receive a store sit outside the front of the name and were invisible while it was asked for there - a cache wearing the name of what it caches, with the store it keeps things in as its last argument. A word written at a call to one of those is on somebody's disk with nothing watching it.";
  ...
  for (let f_name of f_names) {
    let ours = text_includes(f_name, word);
    if (not(ours)) {
      continue;
    }
