...
export function app_code_note_name_colors() {
  ...
  ("THE FIRST ONE IS AS BRIGHT AS IT CAN BE MADE, AND THE GREEN IS WHAT STOPS IT. It was lifted twice at a reader's asking and has about six of lightness left before it stands level with the green - and orange against green is the exact pair a red-green reader has no hue for, so the little that is left between them is the whole of what that reader has. An orange lighter than this is also a paler one: orange runs out of colour as it climbs where green does not, which is why green holds the top of the three and orange sits under it.");
  ("The three sit about thirteen and about six of lightness apart, where two is already tellable. They are not held above the note's grey: the darkest of them is about as light as that grey, and stays legible because its hue is nothing like grey and because the marks around it say what it is even to a reader who sees no colour here at all.");
