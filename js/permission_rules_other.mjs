...
export function permission_rules_other() {
  ...
  "a git verb belongs here only when no argument to it can write. That is what keeps three neighbours out. Naming the subcommand alone is not enough - the listing of worktrees is spelled out because the verb above it makes and removes them, and a rule naming the verb would hand over both. Making an archive is refused for the same reason a sort is: it prints, until an argument tells it to write a file instead. And a stream editor is the plainest case, since the flag that edits in place is one letter away from the flag that reads, so a rule wide enough to read is wide enough to overwrite anything the repo can reach.";
