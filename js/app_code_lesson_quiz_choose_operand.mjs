...
export function app_code_lesson_quiz_choose_operand(
  ...
  "an answer quiz whose buttons are the NUMBERS in the question itself (for '14 / 4' the choices are 14 and 4) - so the other operand is the decoy and the learner must pick the one in the asked role (the dividend), not just spot the only number on screen. The across-item multiple choice can never set this up because it skips same-question items, so it could only ever offer other divisions' numbers as distractors. Needs the question built with real spaces (not js_code_binary_spaced_nb's non-breaking ones) so the tokens split apart";
  ...
  let operators = ["/", "*", "-", "+", "===", "!==", "%", "**"];
  let tokens = text_split_space(quiz_question_text);
  function is_operand(token) {
    let is_operator = list_includes(operators, token);
    return not(is_operator);
  }
  let choices = list_filter(tokens, is_operand);
  ...
  function each_button(choice) {
    ...
    async function on_click() {
      ...
      if (eq) {
        ...
        "a wrong pick dims just this choice and leaves the other live, and on_wrong marks the attempt so the review requeues it";
        on_wrong();
