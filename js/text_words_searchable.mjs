...
export function text_words_searchable(text) {
  ...
  "Capitals are put down to small letters for the same reason the marks come off: a word is the same word at the start of a sentence as in the middle of one. It does nothing at all to a script that has no capitals, which is every script this was first written for, and it is what lets the same layout be used on a translation written in the Latin alphabet.";
  ...
  let small = text_lower_to(plain);
  let pattern = text_words_any_script_pattern();
  let words = small.match(pattern);
