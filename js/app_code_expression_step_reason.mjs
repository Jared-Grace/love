...
export function app_code_expression_step_reason(symbol, others) {
  ...
  function strong_is(other) {
    "whether one of the other operators is solved before the others too";
    let held = list_includes(strong_symbols, other);
    return held;
  }
  ...
  let outranks = and(strong, not(any_strong));
  if (outranks) {
    ...
    for (let other of others) {
      ...
      list_add(parts, other);
      first = false;
    }
    return parts;
  }
  let other = list_first(others);
  let equals = ["The ", symbol, " and the ", other, " are solved left to right"];
  return equals;
}
