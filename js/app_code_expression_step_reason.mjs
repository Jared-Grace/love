...
export function app_code_expression_step_reason(symbol, others) {
  ...
  ("Three cases and no more, whatever the line holds: something ranked below this operator still on it means the rank decides; nothing below means the two are equals and the position decides; nothing left at all means there was never a choice, and that case answers with no row.");
  ("THE LAST OPERATOR ON A LINE IS OWED NO REASON. This used to answer that the operator was the only one left, which is a true thing about the line and is not a reason - there is no other part it was chosen over, so there is nothing to explain. Read on the screen it was worse than merely empty: every other row on the walk names a rule the learner can carry to the next line, so a row in the same place saying only what is on the line reads as though it were one of those, and a learner looks for the rule in it. It is also the one row that could never be wrong and could never be useful, which is exactly the shape of a row that should not be there. An empty answer, and the walk that asked draws nothing.");
  ...
  if (none_left) {
    ("NOTHING IS SAID WHEN THERE WAS NEVER A CHOICE. An empty row, and the caller draws no row at all.");
    let alone = [];
    return alone;
  }
