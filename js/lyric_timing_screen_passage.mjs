...
export function lyric_timing_screen_passage(parent, on_settled) {
  ...
  ("$plain parent");
  ("$plain on_settled");
  ("The part of the screen saying which recording is being timed and how late the person taps: the translation, the passage itself, the lag to take off afterwards, and the row that measures that lag.");
  ...
  ("Redrawing the row is handed on with the boxes, because the song somebody chooses settles the same question the row asks and the row has to be able to say so. It is passed rather than reached for, so that the one place the row is drawn stays the one place that knows how it is drawn.");
  ...
  let passage_show = lyric_timing_screen_choose(
    parent,
    version_input,
    chosen,
    on_settled,
