...
export async function bible_glyph_chapters_verse_marks_underdrawn_gate_run() {
  ...
  "A ROW WITH NO ENGLISH WORD OF ITS OWN IS NOT COUNTED, and that exclusion is a third of everything this used to say. Hebrew doubles a verb for emphasis - he shall surely die is muth muth - and the interlinear glosses the second half with an ellipsis, because English has no separate word for it. Counting it would demand a second picture for a word the English sentence never got, so the seated side skips any row whose gloss holds no letter and no digit. That took a hundred and one findings down to thirty-eight without a single judgment call.";
  "Measured against the record rather than against nought, because thirty-eight stood when this was written and clearing them is reading rather than code. Most are function words - the and, the not, the said - where one English word honestly carries two in the original; a few are the name, the son, the wicked one, and those are words left behind. The record holds all of them so that today's chapter cannot add a thirty-ninth, and it may only ever shrink.";
