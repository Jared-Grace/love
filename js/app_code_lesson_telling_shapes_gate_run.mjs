...
export async function app_code_lesson_telling_shapes_gate_run() {
  ...
  ("The finer of the two checks that hold a lesson's telling against its own question bank. The other asks whether an operator was ever shown; this one asks whether a line with more than one operator in it was ever shown written the way the questions write it - bracketed, or flat.");
  ("This is the check the reported fault needed, and the one the coarser check could not make. A person read lesson eighty-nine, saw false !== (3 === 3) explained and 2 !== 2 === false asked, and said so in those words. Both lines write === and !== and nothing else, so a check over symbols passes them both; the whole of what the learner is stuck on is a bracket the telling never took off.");
  ("Zero is the ratchet and there is no baseline file. Measured 2026-08-28 over all thirty-two lessons: none. A baseline would only record a debt that does not exist.");
  ("Proved to fire before it was believed, because a check that names nothing says the same word whether nothing is wrong or nothing was looked at. Handed the two lines the reader reported it answers bracketed and flat, and handed 3 === 5 it answers with nothing, one operator having no order to argue about. The both-sides parentheses lesson keeps its flat line in a card of its own: read on its own the lesson file shows bracketed and the card shows flat, and a version of the split that stopped short of following the call into that card named the lesson - which is the whole thing this gate says when it goes red.");
  ...
  let hint =
    "a lesson of the code app asks about a line written bracketed or flat when its own telling only ever showed the other one - the lesson is named above, beside what it asks and what it shows";
