...
export async function app_code_lesson_telling_shapes_gate_run() {
  ...
  ("Four marks make up the shape, and any one of them going unshown fails the lesson. Whether a line with more than one operator in it stands parenthesised or flat. Whether it writes one operator over and over or mixes two of them. Which end a true or a false sits at. What kinds of thing stand either side of the operators - a number, a value, a name. They are separate faults and the report names whichever it found, because a learner shown false on the left and asked about false on the right is stuck on the move, not on the parentheses.");
  ("This is the check the reported fault needed, and the one the coarser check could not make. A person read lesson eighty-nine, saw false !== (3 === 3) explained and 2 !== 2 === false asked, and named both differences in those words: the parentheses had gone, and the false had crossed to the other side. Both lines write === and !== and nothing else, so a check over symbols passes them both.");
  ...
  for (let report of offenders) {
    ...
    console.log("NEVER SHOWN WRITTEN THAT WAY  " + said);
  }
  ...
  let hint =
    "a lesson of the code app asks about a line written a way its own telling never showed - parenthesised against flat, one operator repeated against two mixed, a true or false at one end against the other, or a kind of operand never put there before. the lesson is named above, beside what it asks and what it shows";
