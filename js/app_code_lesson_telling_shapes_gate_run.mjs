...
export async function app_code_lesson_telling_shapes_gate_run() {
  ...
  ("Zero is the ratchet and there is no baseline file. Measured 2026-08-28 over all thirty-two lessons, with all three marks in: none. A baseline would only record a debt that does not exist.");
  ("Proved to fire before it was believed, because a check that names nothing says the same word whether nothing is wrong or nothing was looked at. Each mark was shown a pair of lines differing in that mark alone. Brackets: false !== (3 === 3) against 2 !== 2 === false. Side: (3 === 3) !== false against the same line reversed. Kind: 3 === 5 against divisor === 0, which the other two marks call identical. Handed === is not the same as ==, a sentence about code rather than a line of it, all three answer with nothing.");
  ...
  let hint =
    "a lesson of the code app asks about a line written a way its own telling never showed - bracketed against flat, a true or false at one end against the other, or a kind of operand never put there before. the lesson is named above, beside what it asks and what it shows";
