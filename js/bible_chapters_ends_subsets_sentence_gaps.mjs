...
export function bible_chapters_ends_subsets_sentence_gaps(chapters_ends, size) {
  ...
  "Each reader is counted and then let go. Keeping them all and pouring them together at the end is the same answer, but every group of four out of thirty-eight is seventy-three thousand readers holding twenty-one million gaps between them - and holding them is what turned under a minute of counting into an hour of it.";
  arguments_assert(arguments, 2);
  let first = list_first(chapters_ends);
  let indexes = list_indexes(first);
  let subsets = list_subsets_size(indexes, size);
  let measured = bible_sentence_gaps_tally_empty();
  let furthest_each = [];
  for (let subset of subsets) {
    ...
    bible_sentence_gaps_tally_add(measured, joined);
    let gaps = property_get(joined, "gaps");
    let furthest = list_max_or_null(gaps);
    list_add(furthest_each, furthest);
  }
  let tally = property_get(measured, "tally");
  let most = tally_most_or_null(tally);
  property_set(measured, "most", most);
