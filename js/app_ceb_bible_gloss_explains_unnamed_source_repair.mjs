...
export async function app_ceb_bible_gloss_explains_unnamed_source_repair() {
  ...
  "The longer wording of a pair is listed before the shorter one it contains, so the shorter one cannot eat the longer one's opening words and leave a half sentence behind.";
  ...
  let wordings = [
    ["The analysis gives its root here as", "The root behind it here is"],
    ["The analysis gives its root as", "The root behind it is"],
    ["The analysis builds it on", "It is built on"],
    ["Its root is given here as", "Its root here is"],
    ["Its root here is given as", "Its root here is"],
    ["Its root is given as", "Its root is"],
    [
      "read the analysis for each rather than assuming one",
      "read each shape as the dictionary gives it",
    ],
    [
      "read each analysis as it is given",
      "read each shape as the dictionary gives it",
    ],
    [
      "read each analysis as it stands",
      "read each shape as the dictionary gives it",
    ],
  ];
  let done = [];
  for (let wording of wordings) {
    let one = await gloss_chapters_explains_text_replace(
      fn,
      wording[0],
      wording[1],
    );
