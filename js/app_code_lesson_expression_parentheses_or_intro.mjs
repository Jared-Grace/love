...
export function app_code_lesson_expression_parentheses_or_intro(root, context) {
  ...
  ("Each worked line is one the parentheses change. Without them the same three words come to true, and with them they come to false, so a learner who reads past the marks gets the card wrong in front of themselves. A line that came to the same thing either way would be a card that never had to be read.");
  ("Both sides are worked rather than one of them being drawn, which is what the two lessons that taught parentheses do: each says the marks may stand at either end and then works a line with them at the other end, in the same card. Drawn instead, half the visits would show the sentence with nothing beside it that moves - and half the questions ask for the side that visit never showed.");
  ("The second working is the first one turned round: the same three words, in the other order, with the parentheses at the other end. Different words would leave a reader finding which of two things had changed before they could see that only one had.");
