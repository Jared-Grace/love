...
export function app_g_bless_world_new() {
  ...
  ("A fresh world to walk about in - ground, a row of buildings with a pavement in front of");
  ("them, a player standing on it, and a crowd living along it.");
  ...
  ("Only the block and the crowd are this game's own, because only those differ: those");
  ...
  ("The order is the whole of it. The block is decided first because everything after it is");
  ("measured from where it landed; it is built into the ground next; the reachable land is");
  ("asked for only THEN, because building changed what the ground is - walls where there was");
  ("grass, path where there was water - and a list made earlier would still be calling those");
  ("squares what they used to be, so the player would be refused a walk down a street they");
  ("can see and offered one through a wall they cannot.");
