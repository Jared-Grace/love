...
export function app_shared_game_crowd_part(g, path, still) {
  ...
  "The way itself is kept clear of everybody who COMES TO REST, so nobody steps out of the walker's path and straight back into it further along. It may still be passed through, because a tile of it that somebody is standing on is a person this same loop will ask to move in their turn. Where everybody is standing is kept up to date as the openings are made, so two people are never sent to one tile.";
  ...
  "THE ROAD IS NOT GROUND ANYBODY IS SHOVED ONTO. Making room for the player is the one moment a person is moved without choosing to, so a road counted as room is a person put in front of the traffic by somebody else's journey - and the cars come past at once. It is taken out of the ground the opening is looked for in rather than checked at the end, so a road is neither rested in nor shuffled through, and a crowd on a pavement opens along the pavement or does not open at all.";
  "A world with no roads in it - a garden, a hillside - hands in none, and every square of its land stays room, exactly as it always was.";
  ...
  let roads = property_get_or(g, "roads", {});
  let land = g_coordinates_land(coordinates);
  let footway = g_coordinates_outside_index(land, roads);
  let land_index = g_coordinates_index(footway);
  ...
  let still_index = g_coordinates_index(held);
