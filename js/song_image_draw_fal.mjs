...
export async function song_image_draw_fal(number_text) {
  ...
  "it is a second function rather than a switch on the first because the road is not a setting - it is which company drew the picture, which is worth being able to read off the name of what was run months later, and a couplet folder holding attempts from both is a folder nobody can tell apart afterwards.";
