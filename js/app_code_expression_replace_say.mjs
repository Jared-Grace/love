...
export function app_code_expression_replace_say(note, solved_code, value_text) {
  ...
  ("Said with === rather than becomes, because that is how the track has written what a line comes to since the very first lesson that printed an answer, and the learner is looking at the blue block the same words are about.");
  ...
  let chosen_pieces = [];
  function chosen(span) {
    "style a piece as the piece being worked out and remember it, in the one pass that makes it";
    app_code_expression_chosen_code_nowrap(span);
    list_add(chosen_pieces, span);
  }
  let cycles = [noop, html_style_code_dark_nowrap, noop, chosen, noop, chosen];
