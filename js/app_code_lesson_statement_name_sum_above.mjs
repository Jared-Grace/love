...
export function app_code_lesson_statement_name_sum_above(root, context) {
  ...
  ("Two things a learner already has, put next to each other, and nothing else. A sum inside console.log was the arithmetic lessons; a name holding a value was the screens just before this one. Neither is taught again here - what is new is only that the second may stand inside the first.");
  ...
  ("The second box's line shows both ways in it rather than naming only the new one, and the old way is said first inside that line as well as first on the screen. Both are code, so both are chipped. It was once one long sentence with the two sums in brackets; it is now said a line at a time, each way followed by what it adds with, because the one long sentence asked a learner to hold both ways and the difference between them at once.");
  ...
  html_div_cycle_code(box_names, ["Suppose we give names to two numbers:"]);
  ...
  ("The two ways of adding are said one after the other, each followed by a line saying what it adds with - the numbers themselves, then the names of those numbers - so the one difference between them is said in words and not left to be spotted in the code.");
  html_div_cycle_code(box_names, [
    "If we want to add the numbers together, we could write: ",
    numbers_sum,
  ]);
  html_div_cycle_code(box_names, ["This adds the two numbers using numbers"]);
  html_div_cycle_code(box_names, [
    "Instead of writing ",
    numbers_sum,
    ", we could also write ",
  ...
  html_div_cycle_code(box_names, [
    "This adds the two numbers together using the names of those numbers",
  ]);
