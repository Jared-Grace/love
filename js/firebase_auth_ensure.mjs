...
export async function firebase_auth_ensure(container) {
  ...
  let user = await new Promise(function lambda2(resolve) {
    function lambda(u) {
      unsubscribe();
      resolve(u);
    }
    let unsubscribe = onAuthStateChanged(auth, lambda);
  });
  if (user) {
    return user;
  }
  let r = new Promise(function lambda3(resolve) {
    ...
    async function login() {
      let value = html_value_get(email);
      let value2 = html_value_get(password);
      let v = await signInWithEmailAndPassword(auth, value, value2);
      html_clear(container);
      resolve(v.user);
    }
