...
export function app_code_lesson_expression_not_comparison() {
  "the step from ! on another ! (already learned) to ! on a whole comparison: !(3 === 5). One new idea, and it is the parentheses - a ! applies only to what is next to it, so the comparison has to be gathered into one thing before the ! can apply to all of it.";
  ...
  "The comparisons vary over < > === !== - every comparison the learner already owns - so the only thing being learned is the parenthesis. One screen shows an inner comparison that is true and one that is false, so neither the inner nor the outer value can be guessed from habit.";
  function wrapped_wanted(want) {
    "a parenthesised comparison that works out to want, drawn again until it does";
