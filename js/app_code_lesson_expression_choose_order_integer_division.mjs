...
export function app_code_lesson_expression_choose_order_integer_division() {
  ...
  ("It stands IN FRONT OF the lesson that asks this line all at once, because that is the order everywhere else in this run: the whole part is pressed apart on one screen and answered whole on the next, and so is the remainder. A learner who is asked for the value of a line before ever taking one apart has been handed the answer to find with no way of finding it, and the screen that presses it apart afterwards arrives as a remedy rather than as the teaching.");
