  ...
  let cases = [
    {
      ...
      line: text_combine_multiple([
        "/bin/bash -c eval 'while pgrep -f \"timeout 900 node scripts/ai.mjs ",
        fn_name("qa_gate_run"),
        '" >/dev/null; do sleep 10; done; echo "GATE FINISHED"\'',
      ]),
      sleep_loop: true,
    ...
    },
    {
      ...
      line: text_combine_multiple([
        "/bin/bash -c eval 'until [ ! -e /tmp/tasks/bh2yh3csk.output ] || ! pgrep -f \"node scripts/ai.mjs ",
        fn_name("qa_gate_run"),
        "\" >/dev/null; do sleep 10; done; echo DONE'",
      ]),
      sleep_loop: true,
    ...
    },
    {
      ...
      line: text_combine_multiple([
        '/bin/bash -c eval \'prev=""; while true; do cur=$(node scripts/ai.mjs ',
        fn_name("g_verify_loop_check_line"),
        '); if [ "$cur" != "$prev" ]; then prev="$cur"; fi; sleep 3; done\'',
      ]),
      sleep_loop: true,
    ...
    },
    {
      ...
      line: text_combine_multiple([
        "/bin/bash -c eval 'node scripts/ai.mjs ",
        fn_name("qa_app_commit_deployable"),
        " g 2aba7dba2b 2>&1 | tail -30'",
      ]),
      sleep_loop: false,
