...
export function app_shared_bible_open_hash_languages(
  ...
  ("A LINK NAMES ITS LANGUAGES BY THEIR SHORT CODES AND THE READER IS OPENED WITH THE LANGUAGES THEMSELVES, so the codes are looked up on the way rather than handed on as they are. A code naming no language this repo still carries is dropped in that lookup, which is what lets a link written before a bible was retired go on opening instead of failing.");
