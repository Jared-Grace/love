...
export function app_code_lesson_expression_choose_order_or_questions() {
  ...
  ("The asking is the same asking as every other press-both-sides lesson - one line a screen, true and false taking turns, the shape read back off the writing - so it is asked for rather than written out here. All that differs is the maker handed in, which is the whole of what this lesson is.");
  ("Reading a line back works unchanged because an || line is written the same seven words as a line with a comparison in the middle: a number, an operator, a number, the middle operator, and the same three again. Nothing is parenthesised either way, because || is weaker than every comparison and so each side gathers itself.");
