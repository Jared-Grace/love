...
export function bible_search_built_path() {
  ...
  "IT STANDS IN A FOLDER OF ITS OWN, and the folder is the whole point: a word is kept under its own spelling with nothing else in the name, so any name straight inside the words folder is a name some word can have. This one first went in as built.json and BUILT IS A WORD OF THE BIBLE - the mark read back as that word's own index, so it could never differ from itself, which is the one thing a mark is for. A word cannot hold a slash, so a folder is what no word can reach into.";
  let folder = bible_search_folder();
  let path = list_join_slash_forward([folder, "mark", "built.json"]);
  return path;
}
