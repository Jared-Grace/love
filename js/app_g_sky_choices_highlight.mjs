...
export async function app_g_sky_choices_highlight() {
  "restyle the #sky choice pills so the one matching the CURRENT sky (g_time_current_index) is active/selected — called on render, after a jump, and after each walk-step, so the you-are-here marker always tracks the live sky. reads the pills stashed by app_g_sky_choices; a no-op when the panel isn't up (the real game), so the demo-only step path can call it blindly";
  let up = global_function_property_exists(app_g_sky_choices_highlight, "pills");
  if (not(up)) {
    return;
  }
  let pills = global_function_property_get(app_g_sky_choices_highlight, "pills");
