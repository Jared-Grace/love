...
export async function gloss_write_chapter_file_generic(
  ...
  "Save a whole chapter's authored word explanations into a named gloss store from one JSON file, keyed by the verses each passage covers, and take the file away once it is stored.";
  "$plain chapter_code";
  ...
  "★ IT TAKES ITS OWN INPUT AWAY, AND THE REASON IS THE HUMAN'S ATTENTION RATHER THAN TIDINESS. The file has been read and everything in it stored, so what is left on disk is a copy of the store; every caller was already removing it, and removing it was costing a click. A removal chained onto a call cannot inherit that call's approval - a chain is never grantable, whatever is in it - so the four hundred runs measured over one week were four hundred interruptions bought by one line that the function could run for itself. Done here, the call is a single approved command again and nobody is asked.";
  "IT IS TAKEN AWAY ONLY AFTER EVERY PASSAGE HAS BEEN STORED, so a passage named by verses the chapter does not divide itself at throws with the author's file still sitting there to correct. A removal is the one change that leaves nothing behind to read afterwards, so it must never be the thing that runs when something has gone wrong.";
