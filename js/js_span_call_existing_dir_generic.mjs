...
export async function js_span_call_existing_dir_generic(
  dir,
  f_name,
  address_from,
  address_to,
  ...
  ("Points a run of lines in one file of a flat folder at a function that already lives in the file next door, leaving the run as a call to it, with the caller saying which reader picks the two ends. The hermetic core of the verb that retires a run written out by hand in favour of a name that was already there.");
  ...
  ("Which reader finds those ends is handed in rather than chosen here, because that one word is the whole difference between the climbing verb and the one that reaches inside a loop. Held once, a fix to the cutting reaches both; written twice, a fix made in one of them is silently missing from the other.");
  ("The reader arrives as a function rather than as its name. Everything else in this file is hermetic - the folder handed in is the only thing opened - and looking a name up would reach the repo, which is exactly what an example running in a sandbox has no right to do.");
  ...
  function lambda_select(address) {
    let found = select_statement(ast, address);
    return found;
  }
