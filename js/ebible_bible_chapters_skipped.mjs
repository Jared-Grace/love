...
export async function ebible_bible_chapters_skipped(bible_folder) {
  ...
  ("Read from what the bible says it carries rather than from the pages that arrived, because the two can disagree and only the first is an editorial choice. englxxup lists Proverbs 30 in its own index and publishes no page for it, which is a broken link and is being written to as one; it is not a bible that decided to leave the chapter out.");
