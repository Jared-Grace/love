...
export function lyric_timing_lag_said_cases() {
  ...
  ("THE LAGS ARE PICKED FOR WHAT THEY DO TO THE ROUNDING. A quarter of a second and a bit must come back as a quarter, and a hair under a fifth must come back as a fifth - so a version that printed the raw number is refused by three of these and one that kept it to one place by three others, which sounds thorough and is the opposite - neither is refused by the hair-under-a-fifth case alone, and for a while nothing here asked the rounding question sharply enough for that case to be the only one answering. A version that cuts the number off at two places rather than going to the nearest is what finally did it, and that case is now the only one refusing it.");
  ("THE RUN NOBODY ANSWERED IS THE ONE CASE HERE THAT REFUSES NOTHING BY ITS WORDS, and it is kept for what it refuses by not falling over. There is no lag at all in it, and a version that turned the lag into words before deciding whether it had one would throw rather than answer - which the gate reads as a failure just as surely as a wrong sentence. It is the only case where the lag is not a number, so it is the only one that can make that version fall over - but it is not the only one that refuses it, because that version also words the lag with a rounding of its own, and the hair-under-a-fifth case refuses it for that instead.");
  ...
  ("THE WRONG VERSIONS ARE NO LONGER A PARAGRAPH ABOUT THE PAST. They are written out in data/given/red_proofs/",
    fn_name("lyric_timing_lag_said"),
    ".mjs, and ",
    fn_name("red_proofs_gate_run"),
    " asks every one of them of every case here on each run - so a case removed later, or a version that stops being wrong, is said out loud rather than leaving this account describing a check nobody is doing any more. Everything above records what was learnt; the file is the check itself.");
  let told_quarter =
  ...
  let cases = [
    ...
    },
    {
      ...
      count: 10,
      said: ask_seven,
      why: "One press below the line, with a perfectly reasonable lag sitting there unused. It refuses a version deciding on whether there is a lag at all rather than on how much of the run stands behind it, though the half-answered short run refuses that one too. What only this case refuses is a version drawing the line at seven tenths instead of three quarters, which is the nearest wrong place to draw it and the reason seven of ten was picked over six.",
    },
    {
      measured: {
        lag: null,
    ...
    },
    {
      ...
      count: 4,
      said: told_three,
      why: "Three quarters of a four-sound run, which is the line exactly - this is the case sitting on it, and a version drawn at the same place but wanting strictly more than three quarters goes red here and nowhere else. The version that had the line written as a number of presses rather than as a share of the run does not go red here, because one missed sound out of four is few by either reading; it goes red on the half-answered short run below.",
    },
    {
      measured: {
        lag: 0.3,
