...
export function app_g_player_path_choose(g, player, to) {
  ...
  "The player's OWN LINE is the last thing it will go through, and stays standing for this question while any other way exists at all. A line is passed by backing it down its own trail, in order, which is a different move entirely - walking through it would trade places with a follower and leave the line in an order it was never walked in. Strangers stand aside or trade places; the line reverses. Each way past belongs to what is in the way.";
