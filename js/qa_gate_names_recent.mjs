...
export async function qa_gate_names_recent(f_names) {
  ...
  "Asked somewhere with no history, it answers that nothing is recent rather than throwing - the same as its neighbour, and for the same reason: this is reached while a red gate is being reported, and a throw there loses every complaint after the first.";
