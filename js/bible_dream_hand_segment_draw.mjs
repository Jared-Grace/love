import { bible_dream_hand_path_keep_is } from "./bible_dream_hand_path_keep_is.mjs";
...
export function bible_dream_hand_segment_draw(state, points) {
  ...
  "A setting the stroke cannot carry starts a new stroke at the point the old one ended at, so nothing is skipped and the two touch. Only there do two pieces of the hand's line ever overlap, and only there is it right that they do: the two are honestly different marks.";
  let start = points[1];
  let end = points[2];
  let strength = end.strength;
  let b = bible_dream_hand_path_keep_is(state, strength);
  if (not(b)) {
