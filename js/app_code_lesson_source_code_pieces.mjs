...
export function app_code_lesson_source_code_pieces(source) {
  ...
  ("The finding is written apart from what is then read off it, because more than one reading wants the same pieces - what operators a card spells, and whether the line it spells is parenthesised - and a second copy of this split would be a second place for the length to drift.");
