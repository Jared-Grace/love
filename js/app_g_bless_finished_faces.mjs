...
export async function app_g_bless_finished_faces(
  ...
  ("The lights go up before the camera has arrived, not after. The quiet gold mark lands on");
  ("a face the moment the prayer does, so a celebration that waits for the camera lets the");
  ("player see the answer and then watch the screen slide about before anything is made of");
  ("it. Started together, the journey and the lighting are the same moment.");
