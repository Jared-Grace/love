...
export function app_code_lesson_expression_choose_order_not_pair_intro(parent) {
  ...
  ("Three sentences and no more: a ! can stand in front of a whole && or ||, the parentheses are what say it holds all of it, and so what is inside them is the part that goes first. The last of the three is the one the lesson is for, and it follows from the two above it rather than being announced on its own.");
