...
export function app_code_lesson_quiz_token_select_variations_spelled(
  ...
  ("EVERY CHECK HERE IS THE SAME CHECK ASKED ABOUT A DIFFERENT SPELLING. Everything that built the pool went by value, and by value all of these are the question's own line - which is exactly why nothing above stopped them. A lesson is not about the value though, it is about the writing, so what is compared here is how the question was written against how the answer was: which tiles, how much each parenthesis gathered, whether the join wrote the same word twice.");
  ...
  ("An arrangement may not leave out a token the question wrote. Everything above widens the pool - a sign that reads both ways, a call whose arguments come in any order, another way of saying the same value - and one of those ways is to say it with fewer symbols. Re-printing a line from its tree drops a parenthesis the value does not depend on, so (3 === 5) === false came back as 3 === 5 === false as well, and a lesson whose whole subject is the parentheses accepted an answer with no parentheses in it. The learner is told they are right without having done the thing.");
  ("Measured across every line the course can hand out, this drops something in twenty-seven of seven hundred and sixty-one, and every one of them is a lesson losing an answer that omitted its own subject: the four parenthesis lessons, and the statement lesson, which was accepting console.log(5) without the semicolon it had just taught.");
  ...
  ("Parentheses have to gather as much as the question's parentheses gathered. Leaving them out is only the loud half of the same fault; the quiet half keeps every tile and moves the parentheses somewhere they change nothing, so (3 === 5) === false was also accepting (3) === 5 === false and (3 === 5 === false). Those are the same line by value - which is exactly why every check that goes by value calls them right - and they are not the same answer to a lesson whose subject is which part the parentheses are around.");
  ("This is asked of the whole pool rather than of the half it came from, because both halves can produce it: the free rearrangement of tiles is where these came from today, and a swap that re-prints from the tree is free to parenthesis a different side tomorrow.");
  ("How much each parenthesis gathered is asked, and not which tiles it gathered. Rearranging the tiles is the whole of what the learner is doing, so a parenthesis holding a different comparison is not a parenthesis that moved - (3 !== 2) !== (8 === 6) and (8 !== 6) === (2 !== 3) put a comparison inside each parenthesis either way. Whether the rearranged line still says what the question said is asked separately, by the reading the caller does above.");
  ("A genuine swap survives, because the amounts are compared as a set and not by where they fall - (4 < 5) === (5 < 4) swapped over still gathers three tiles on each side. A question with no parentheses is unaffected: it has no parenthesis button, so nothing parenthesised was ever buildable.");
  let asked_groups = js_tokens_parenthesis_group_sizes(asked);
  function parentheses_alike_is(variation) {
    let groups = js_tokens_parenthesis_group_sizes(variation);
    let alike = lists_equal_pair(groups, asked_groups);
    return alike;
  }
  kept = list_filter(kept, parentheses_alike_is);
  ("A line that writes the same word either side of its join must go on writing it. Two comparisons that share an end are saying something about the thing they share, and the swap that reads both ways carries that shared word to the outside - so a middle written twice comes back as two different numbers meeting at the join. The same line by value, and no longer the line the lesson is about. This is the parenthesis rule standing in another place: same value, different answer to the question actually asked.");
