...
export function app_code_lesson_expression_choose_order_both_sides_above_generic(
  ...
  ("The line and the closing sentence are both handed in, because the lessons built on this run differ only in what stands in the middle of the line and in the one sentence that says so. Everything between - why the two sides go first, why either may go first, and the working out itself - is said the same way about either kind of line, and is read off the line rather than written out.");
  ("What the two sides ARE is handed in as well, in two words, because the run has to name their kind twice and the shape cannot read it off the line: the sides of a line under a comparison are arithmetic, and the sides of a line under && are comparisons. Everything else on the screen is the same telling, so naming the kind is what keeps one run serving both rather than a second copy being written to change two sentences.");
  ...
  ("Why they go first is said as strength - arithmetic is worked out before a comparison, and a comparison before an && - and not as brackets, because there are no brackets on this line and there is nothing on it to point at. Brackets are how the lesson further on says the same thing about a line whose three operators are all comparisons and so all of one strength.");
  ...
  html_div_cycle_code(run, [sides_same]);
  html_div_cycle_code(run, [
    "So ",
    left_code,
    " and ",
