...
export async function ebible_text_to_speech_chapters(
  ...
  "★ THE MODEL IS READ OFF DISK ONCE PER WORKER HERE AND ONCE PER CHAPTER THE OTHER WAY, WHICH IS THE WHOLE REASON THIS EXISTS. Speaking a book chapter by chapter starts a fresh engine every time, and the engine is a third of a gigabyte of weights; a book of fifty chapters paid that fifty times and spoke on one core while thirteen others waited. Handing the list over once lets the workers load once and then simply take the next chapter, and measured that moves the whole Bible from about two hundred and fifty three hours to about a hundred and sixteen.";
