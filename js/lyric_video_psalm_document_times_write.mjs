...
export async function lyric_video_psalm_document_times_write(
  ...
  "★ THE SINGING IS SAID BY ITS MARK, WHICH IS THE SAME WORD THE DOCUMENT AND THE VIDEO ARE ALREADY NAMED BY. It used to be the number the recording's file name carries, and that number does not identify a recording: the thing that downloaded these songs numbered the compressed copies and the lossless copies as two separate runs, so one number can name two different performances. The mark comes out of the whole set of recordings of the passage, is what the drafting run already reported, and can be read straight off the document sitting in the repo - so the word to type here is a word already written down somewhere. The plain singing has no mark, and an empty word is what asks for it.";
  ...
  function chapter_key_of(song) {
    let key_song = song.chapter + " (" + song.mark + ")";
    return key_song;
  }
  let keys = list_map(songs, chapter_key_of);
  let key_wanted = number_chapter + " (" + mark + ")";
  let at = keys.indexOf(key_wanted);
  if (equal(at, -1)) {
    let unsung = lyric_video_document_times_unheard(
      number_chapter,
      "no whole singing of this chapter under this mark is on this machine",
    );
    return unsung;
  }
