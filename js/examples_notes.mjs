...
export function examples_notes() {
  ...
  let notes = {
    ...
    example_wrap_copy_refuses_a_taken_name:
      "the refusal for the one wrapping verb never told the name it writes - it works the twin's name out, so the file it would write over is the very one somebody wrote by hand for the same purpose",
    example_param_delete_refuses_a_name_it_never_binds:
