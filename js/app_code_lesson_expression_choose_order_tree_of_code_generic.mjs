...
export function app_code_lesson_expression_choose_order_tree_of_code_generic(
  ...
  "Any parentheses are taken out before the words are counted rather than being read as part of them. They say nothing this reading needs - in the one kind of line these lessons write, parentheses can only ever be round the last two words, and the lesson that hands in the parenthesised hanging is the one that put them there. Left in, they would be stuck to the words either side of them and every word would have to be cleaned of them one at a time. A line with none is handed back unchanged, so the lesson that never parentheses pays nothing for this.";
  ...
  "HOW THE PARTS HANG TOGETHER IS THE ONLY THING THE TWO LESSONS DIFFER BY: one lets the operators decide the shape and the other puts the last two inside parentheses. Everything above was written twice and said the same thing twice.";
  arguments_assert(arguments, 2);
  let left_parenthesis = js_code_parenthesis_left();
  let right_parenthesis = js_code_parenthesis_right();
  let parentheses = [left_parenthesis, right_parenthesis];
  let nothing = text_empty();
  let bare = text_replace_multiple_to(code, parentheses, nothing);
