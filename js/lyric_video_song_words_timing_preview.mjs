...
export async function lyric_video_song_words_timing_preview() {
  ...
  ("★ THE WORD IS LIT FROM EXACTLY WHERE IT IS PLACED, EXACTLY AS THE VIDEO LIGHTS IT. The video used to turn a word red a little early and let it go a little late so a small mistake was forgiven; that forgiveness hid the very mistake being looked for here, so once words were placed here by ear the video was made exact as well. When a word is done is one rule for both, ",
    fn_name("lyric_video_word_done"),
    ".");
