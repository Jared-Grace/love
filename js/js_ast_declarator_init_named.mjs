...
export function js_ast_declarator_init_named(ast, name) {
  ...
  "The first line binding the name in that one scope is the answer, and the lines inside functions standing within it are not looked at. It used to look at every line in the file and take the first, on the ground that the shadowing gates refuse a name bound twice. That ground fails twice over: two side-by-side scopes may bind one name and are allowed to, deliberately, because neither can see the other; and a name bound only inside a nested helper has no outer line to compete with, because the function around it may be handing back a parameter or an import.";
  "It fails a third time for a plainer reason, and that is the one that actually bit. A gate judges what has been committed; a reader like this one walks the folder as it stands, where hiding the gate really would refuse can be sitting in front of it unjudged. A gate returned its record under the usual name, a helper inside it bound an empty list to that name a few lines above, and this handed back the empty list - ordinary hiding, which the shadowing gates do refuse and had not yet been asked about.";
