...
export function app_g_dev_index_tree_check() {
  ...
  assert(equal(study.hash, "study"), "top-level route is a leaf with its hash");
  let conversation = tree.children["conversation"];
  assert(equal(conversation.hash, null), "conversation is a category, no hash");
  let unbeliever = conversation.children["unbeliever"];
  assert(equal(unbeliever.hash, "unbeliever"), "unbeliever carries its route hash");
  let gospel = unbeliever.children["gospel_share"];
  assert(equal(gospel.hash, "gospel_share"), "unbeliever also holds gospel_share as a child");
  return {
    ok: true,
  };
}
