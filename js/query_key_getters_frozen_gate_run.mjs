...
export async function query_key_getters_frozen_gate_run() {
  ...
  "Moving a word out of the sites and into a function is only half of what keeps a saved link working. The word can still be reworded inside that function, and then every link somebody saved spelling the old one opens on nothing - and there is no repair, because the link is on a disk nobody here can reach.";
  "Freezing is the other half, and it is the half nothing reminds you of: the sites are clean, the gate watching for written-out words is green, and the word is published unwatched all the same.";
  ...
  list_empty_is_assert_json(unfrozen, {
    hint: text_combine_multiple([
      "a word is being written into the query part of a page address by a function nothing has frozen, so rewording it there would quietly break every link already saved with it. Name that function to ",
      f_name2,
