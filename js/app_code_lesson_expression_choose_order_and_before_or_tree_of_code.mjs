...
export function app_code_lesson_expression_choose_order_and_before_or_tree_of_code(
  ...
  ("These lines carry no parentheses, so the second word is the first operator and nothing has to be cleaned off it before it can be looked at. Taking the rest of the line apart is the same work as on the parentheses lesson, so it is said once, next door.");
