...
export function app_code_quiz_tile_arrangements_or_null(code) {
  ...
  ("The brackets, the commas and the names are the movable tiles' opposite: the learner is never handed one, so a layout that moved one is not something they could have built. Holding those still is not a rule about kinds, it is the row of tiles the screen actually gives them.");
