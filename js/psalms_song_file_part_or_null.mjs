...
export function psalms_song_file_part_or_null(file_name) {
  ...
  ("★ A STANZA'S NAME MAY BE SEVERAL WORDS, WHICH IS THE DIFFERENCE BETWEEN READING FOUR SONGS AND PASSING OVER THEM. One Hebrew letter carries two sounds and the Berean text heads its stanza with both names, so those recordings arrived called Sin and Shin; a reading that wanted one unbroken run of letters did not match the name at all, and four songs of Psalm 119 were invisible to every command in this repo with nothing anywhere saying so. The words are joined by the same two marks that join every other part of these names.");
  ("★ THE WORDS AFTER THE FIRST ARE NOT KEPT AS THEIR OWN PIECE OF THE ANSWER, BECAUSE THE PIECES ARE COUNTED AND NOT NAMED. Which take a file is comes back from the fourth piece of the reading, so a piece added anywhere before it would silently hand back a chapter's worth of songs all numbered wrong - the same shape of fault as an argument inserted before the ones already being passed. What is wanted here is only that the words be allowed, not that they be told apart, and a piece that keeps nothing keeps the count where it was.");
