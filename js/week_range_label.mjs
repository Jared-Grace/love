...
export function week_range_label(range) {
  ...
  let start = clock_label(range.start);
  let end = clock_label(range.end + 1);
  let label = text_combine_multiple([range.day, " ", start, " – ", end]);
  return label;
}
