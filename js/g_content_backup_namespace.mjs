...
import { g_content_backup_file_changed } from "./g_content_backup_file_changed.mjs";
import { list_map_limited_async } from "./list_map_limited_async.mjs";
export async function g_content_backup_namespace(namespace, generations) {
  ...
  async function lambda(storage_path) {
    let path = await g_content_backup_file_changed(storage_path, generations);
    return path;
  }
  let paths = await list_map_limited_async(storage_paths, lambda, at_once);
  return paths;
}
