...
import { qa_promoted_commits_named } from "./qa_promoted_commits_named.mjs";
import { qa_commit_named_said_distances } from "./qa_commit_named_said_distances.mjs";
...
import { qa_commit_named_said_forget } from "./qa_commit_named_said_forget.mjs";
...
export async function qa_commit_named_said_expire() {
  ...
  "What this holds on to for itself is the ORDER: which judgements are the nearest few is the one question that cannot be settled while walking, so the measuring, the sorting and the letting go are three steps rather than one loop, and each of the outer two is named.";
  arguments_assert(arguments, 0);
  let remembered = await qa_commit_named_all();
  let head = await git_head_commit();
  let ceiling = qa_commit_named_behind_ceiling();
  let standing = await qa_promoted_commits_named();
  ...
  let measured = await qa_commit_named_said_distances(remembered, head);
  let distances = property_get(measured, "distances");
  let spoken = property_get(measured, "spoken");
  ...
  let forgotten = qa_commit_named_said_forget(
    remembered,
    distances,
    protected_commits,
    ceiling,
  );
  let expired = property_get(forgotten, "expired");
  let kept = property_get(forgotten, "kept");
