...
export async function qa_commit_named_said_expire() {
  ...
  "The two keepings hand over to one another, and that is why both are needed. The freshest judgement is protected from the moment it is filed until a deployment starting from it writes its first note; that note then protects the same commit for as long as the pieces built out of it are waiting, by which time something fresher has usually taken the first protection over.";
  "The keeping is bounded by how many apps there are rather than by how far back anybody looks, so it cannot grow the file the way the distance rule was written to stop: one commit an app, several apps usually name the same one, and one more for the freshest.";
  ...
  ("How far back each spoken judgement stands is worked out once, before anything is let go, because the nearest of them cannot be known until all of them have been measured.");
  let distances = {};
  let nearest = null;
  let nearest_behind = null;
  for (let commit of object_property_names(remembered)) {
    ...
    let first = null_is(nearest_behind);
    let right = less_than(behind, nearest_behind);
    let closer = or(first, right);
    if (closer) {
      nearest = commit;
      nearest_behind = behind;
    }
  }
  let b = null_is(nearest);
  let spoken_any = not(b);
  if (spoken_any) {
    list_add(standing, nearest);
  }
  let expired = [];
  let kept = {};
  for (let commit of object_property_names(remembered)) {
