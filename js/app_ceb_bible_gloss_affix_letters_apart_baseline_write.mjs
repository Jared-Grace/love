...
import { app_ceb_bible_gloss_affix_letters_apart_baseline_growth_assert } from "./app_ceb_bible_gloss_affix_letters_apart_baseline_growth_assert.mjs";
...
export async function app_ceb_bible_gloss_affix_letters_apart_baseline_write() {
  "Rewrite the record of the Cebuano explanations quoting letters no cut of their word holds, from what the store holds right now. For seeding it once, and for banking a repair - never for blessing a fresh invention, which is the one thing the gate exists to refuse.";
  ...
  let known = await app_ceb_bible_gloss_affix_letters_apart();
  await app_ceb_bible_gloss_affix_letters_apart_baseline_growth_assert(known);
