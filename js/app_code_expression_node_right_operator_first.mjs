...
export function app_code_expression_node_right_operator_first(
  ...
  ("Parentheses are never written here, and whether the printed line carries any is not the same answer for every caller: an inner operator that is the stronger of the two needs none, and one that is weaker can only mean what it says with parentheses round it. Both come out right because the helper that prints the line reads the need off the shape instead of being told, which is what lets this one builder serve a lesson that wants parentheses and a lesson that must not have them.");
