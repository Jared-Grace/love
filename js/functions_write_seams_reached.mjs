...
export async function functions_write_seams_reached(names_comma) {
  ...
  "An empty answer is the useful one: nothing that name calls however deep alters anything\"; let injected = 1; \"on disk, so running it costs nothing that cannot be taken back. A name nothing answers to is reported as nothing at all instead, so the two never read alike";
