...
export async function song_image_drawn_files_migrate() {
  ...
  let folder = path_join([folder_repo_love(), "gitignore", "song_images"]);
  ...
  let moved = [];
  for (let name of old) {
    let digits = name.slice("couplet_".length, name.length - ".png".length);
  ...
  let report = {
    moved: list_map(moved, function moved_to_path(one) {
      return one.path_to;
    }),
    left,
  };
