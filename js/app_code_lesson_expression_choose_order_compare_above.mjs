...
export function app_code_lesson_expression_choose_order_compare_above(
  ...
  ("BOTH SHAPES ARE WORKED, because the card below asks about both. The bank hands out a line with the comparison on the right and a line with it on the left, turn about, and the two are read by different rules: on the right the comparison is printed inside parentheses and they decide, on the left there are none and the ordinary rule for two operators decides. Measured 2026-08-27: only the parenthesised line was worked, so a learner met a line like 4 === 6 !== true having read a rule about a mark that line does not carry.");
  ("The parenthesised line is worked FIRST, for the reason the first question of the bank puts that shape first: the part that may be solved first is then not the leftmost thing on the line, so a learner following the run learns to read the line rather than the position. Reversing the two would teach position on the first line they ever see and take it away on the second.");
