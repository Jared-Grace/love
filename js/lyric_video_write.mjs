...
import { lyric_video_audio_fade_seconds } from "./lyric_video_audio_fade_seconds.mjs";
...
export async function lyric_video_write(
  ...
  "★ THE SONG IS BROUGHT UP RATHER THAN SWITCHED ON. The first sample of a recording arrives at whatever loudness it was recorded at, and a listener hears that as a bang before they hear any music. A brief rise at the very front removes it. It is asked for as its own instruction on the sound rather than being built into the picture instruction beside it, because that instruction is a graph of frames and the sound never enters it - the song is handed straight from the file it was opened from to the finished video.";
