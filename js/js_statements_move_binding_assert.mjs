...
export function js_statements_move_binding_assert(
  ...
  if (down_is) {
    let bound = js_statements_declared_names_direct(statements_moved);
    let read = js_statements_referenced_names(statements_crossed);
    list_intersect_empty_is_assert_json(bound, read, {
  ...
  let read = js_statements_referenced_names(statements_moved);
  let bound = js_statements_declared_names_direct(statements_crossed);
  list_intersect_empty_is_assert_json(read, bound, {
