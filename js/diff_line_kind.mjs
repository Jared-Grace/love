import { js_line_name_alone_is } from "./js_line_name_alone_is.mjs";
...
export function diff_line_kind(line) {
  "What one changed line of a difference is made of: an import, a written-out comment, a value chosen, a name or number standing alone that could be either, or actual code";
  "The five are worth telling apart because only one of them is a gap. Imports are already written by the canonicalizing pass, and a comment in this repo is a bare piece of text standing on its own line, which no transform was ever going to compose. What is left is the code, and that is where a missing command would have helped";
