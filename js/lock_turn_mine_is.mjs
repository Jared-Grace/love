...
export async function lock_turn_mine_is(ticket_path) {
  "$plain ticket_path";
  ...
  ("It is told one path rather than a folder and a name, because the two must agree and there is nowhere for them to disagree if there is only one of them. The folder is simply the path with the last part taken off");
  let waiting = await path_dirname(ticket_path);
  let ticket = path_base(ticket_path);
  await path_touch(ticket_path);
  let names = await folder_read_files(waiting);
  let now = date_now_milliseconds();
  let live = [];
  for (let name of names) {
    let path = path_join([waiting, name]);
    let touched = await path_modified_ms(path);
