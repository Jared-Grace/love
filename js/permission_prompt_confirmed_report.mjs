import { permission_prompt_confirmed_keys } from "./permission_prompt_confirmed_keys.mjs";
...
export async function permission_prompt_confirmed_report(
  ...
  let keys = permission_prompt_confirmed_keys();
  ...
  let keep = 20;
  let gathered = await permission_prompt_confirmed_rows(
    key,
    days,
    seconds_minimum,
    keep,
  );
  let rows = property_get(gathered, "rows");
  ...
  let summary = {
    ...
    waits_scanned: property_get(gathered, "scanned"),
    proved: property_get(gathered, "proved"),
    counted: property_get(gathered, "counted"),
  };
  return summary;
}
