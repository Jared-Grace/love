...
export function app_code_lesson_expression_choose_order_both_sides_above(root) {
  ...
  ("The same three-part shape as the step-at-a-time lessons before it - recall, run, hinge - because a learner arriving here has read that shape on every screen of this kind behind them. A run laid out a new way would be read as a new thing to learn, when the only new thing on this screen is that one line holds both kinds of operator.");
  ...
  app_code_lesson_expression_choose_order_both_sides_intro(change_card);
}
