...
export function bible_dream_passage_steps_needed() {
  ...
  "What may not happen is filling the gap in. Somebody will want to show what she saw, and no passage says - so an image presented as her dream is invented at exactly the point the text chose to withhold, however scriptural the image itself is.";
  "That is narrower than it first looked, and the passage does have an errand in it. The dream, the suffering, the cause and the message she sent are all given; only the vision is not. So it needs showing and it needs word after all, and it is empty here because this asks what the DREAM held rather than what the passage hands over.";
