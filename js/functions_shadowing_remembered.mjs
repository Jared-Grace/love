...
export async function functions_shadowing_remembered() {
  ...
  ("Keeping it is safe here because the three that ask never write a file. What could change the answer under them is a peer editing the repo mid-run - and then the three used to disagree with each other, one file at a time, which is worse than the three agreeing about one moment. The ones that DO write are deliberately left asking the plain question: ",
  ...
  let held = global_function_property_exists(
    functions_shadowing_remembered,
    key,
  );
  if (held) {
