...
import { lyric_video_picture_motion_text } from "./lyric_video_picture_motion_text.mjs";
...
export function lyric_video_filter_text(
  ...
  ("★ THE SLOW ZOOM AND PAN LAID ON EVERY PICTURE DOES CUT ITS EDGES, and that was asked for: the human watched still pictures move and wanted more of it. Fitting still decides the shape; the moving is what trims, and only by as much as it zooms.");
  ...
  ("★ THE FADE IS WHY A PICTURE IS REPEATED AT THE RATE OF THE VIDEO, AND WHY THE ONE BEFORE IT IS HELD ON PAST ITS END. A fade has something different to say on every frame, so a picture stepping once a second would flicker rather than fade. The picture is fitted once and then moved, so every one of those frames is a new frame rather than a copy.");
  ...
  function picture_steps(picture, index) {
    ...
    let moving = lyric_video_picture_motion_text(
      picture,
      width,
      height,
      repeats,
    ...
    let fit = source + "setsar=1" + lit + rising + fitted;
