...
export async function functions_oversize_shortened_rescues() {
  ...
  "THE RESCUES ARE NOT ALL TAKEABLE AND THE COUNT CANNOT KNOW IT. Eighteen of these places were worked through by hand on 2026-08-24 and eight of them cut. Of the ten that did not, seven stopped the command outright - four because a function inside the run holds a name the lines below go on using, one because the run reads a name only the lines below bring into being, and two because the word addressed had gone since the list was read - and three were cut and put straight back, because every line of the run only carried a name from one side of it to the other and the piece that came out held no work at all. Since 2026-08-25 the reading behind this one asks the whole of the cut's question rather than the name half of it alone, so the five that stopped the command are gone: five of the places offered afterwards were worked by hand and not one threw. What is left is the other refusal, which is about worth rather than about a silent wrong - four of those five came out and went straight back in, because the piece held no work at all. That test is asked of the finished function after it has been written, so this reading cannot ask it, and the count still says nothing about whether a place is worth taking.";
