...
export function app_code_lesson_expression_parentheses_both_sides() {
  ...
  function refill() {
    "two examples a screen, one true and one false. Every line here is the same shape - a wrapped comparison on each side - so the only thing that changes from line to line is the answer, and two lines show both answers. Four lines said those same two things twice, with different numbers.";
