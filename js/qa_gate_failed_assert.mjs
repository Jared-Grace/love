...
export function qa_gate_failed_assert(told, here, asked_ms, blamed_ms) {
  ...
  ("The words wrapped around the names are asked for rather than spelled here. This sentence is taken apart again by the reader that pulls the names back out of it, so the wording is an agreement between two places rather than a decoration on one - and spelled at both ends it can be half changed, after which the reader hands back names nothing answers to and no gate says a word about it.");
  ...
  if (red) {
    ...
    throw new Error(
      qa_gates_run_failed_prefix() + said + qa_gates_run_failed_suffix(),
    );
  }
}
