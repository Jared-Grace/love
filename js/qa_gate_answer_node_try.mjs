...
export function qa_gate_answer_node_try(ast) {
  ...
  "Only the returns written in the gate's own body are its own. A gate with a helper standing inside it returns twice over, and a walk over the whole file cannot tell which return belongs to the gate - it would take whichever came last in the text, which is the helper's whenever a helper is written after the last line of the gate. The step from the returned name to what fills it is scoped the same way, and for the same reason.";
  arguments_assert(arguments, 1);
  let returns = js_list_type_nodes_outermost_function(ast, "ReturnStatement");
