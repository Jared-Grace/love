...
export async function bible_glyph_artwork_download() {
  ...
  "A MISS IS NOT PROOF OF A WRONG NAME. Measured: a run of ninety-odd requests reported one picture missing whose name and address were both exactly right, and the very next run fetched it. So a name that appears once in the report is worth running again before it is investigated, and only a name that misses twice is a name to correct.";
