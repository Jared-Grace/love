...
export async function app_emoji_bible_chapter_body(
  ...
  ("THERE ARE TWO WAYS TO ASK FOR A READING AND ONE WAY OF DRAWING ONE. The whole key stacks every band under the verse; the English on its own is that one band. Either answer needs the verses matched against the interlinear, so either answer takes the same road, and what the reader asked for is passed on rather than decided again at the bottom.");
