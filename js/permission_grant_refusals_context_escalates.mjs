...
export async function permission_grant_refusals_context_escalates(
  denied,
  unaliased,
  refusals,
  openers,
  ...
  ("The same argument condition as above, and for the same reason rather than by imitation. The two rules that already stand are argument-free regenerators - one renders the settings file from the granted-names list, the other renders that list from itself - so what they write is fixed by committed source, and a grant covering every argument they are ever handed covers nothing they would not do anyway. Changing what they write means editing the source first, which is a visible edit and not something a standing approval buys. What the condition cannot see is an argument-free function that adds rules from something outside the source. One was written since - the restorer of departed grants takes nothing and reads which approvals to put back off the settings file - and this check passed it clean, exactly as predicted. It is refused now by the reason above rather than by this one: it says in its own prose that it must never be handed a standing approval. That is the answer to the whole class, because what makes such a function unsafe is where it reads from, which no reading of its parameters can reach.");
