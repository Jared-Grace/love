...
export async function qa_snapshot_ensure_named(copy_name, commit) {
  ...
  if (made) {
    ...
    ("The putting-back is inside the retrying rather than in front of it, so every attempt gets a copy that was tidied a moment ago instead of one that was tidied once at the start. Everybody here shares this copy, so the thing that leaves a file behind in it is a peer running gates, and a peer can do that between the tidying and the move as easily as before either. Tidied once at the start, the same refusal then comes back on all five attempts, each of them looking at a folder nothing has touched since the last one - measured 2026-08-26, five identical refusals naming one file left behind, and the way through was a person typing the tidying out by hand. Inside, an attempt can only fail on something that arrived during that attempt, which is the collision the retrying was written for.");
