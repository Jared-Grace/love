...
export async function bible_glyph_chapter_bands_write(chapter_code) {
  ...
  ("Writes every band a newly authored picture Bible chapter needs, and names the one that has to be named: the Rosetta lines, their entry in the register, and every reveal language refetched.");
  ...
  ("AND IT MUST RUN AFTER THAT AND NOT BEFORE, because every language writer finds its own set by reading the list of chapters. Run early they write every band except the new one, report a number one short, and leave exactly the failure they exist to prevent.");
