...
export async function baseline_writers_growth_unguarded() {
  ...
  "what counts as refusing lives next door, handed a plain list of names so a corpus can hand it sets nobody has written. this sweep comes back empty on a healthy repo, so on its own it could never tell a working judgment from a broken one - and the first draft of that judgment was in fact broken";
