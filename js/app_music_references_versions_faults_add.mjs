...
export async function app_music_references_versions_faults_add(
  ...
  ("THE PASSAGES ARE ASKED OF EACH BIBLE TOGETHER RATHER THAN ONE AT A TIME, and that is the whole reason this is a name of its own. The reader underneath answers a list of passages several at a time, and asking it for one passage twenty-two times over turns that into twenty-two waits one after another. Measured 2026-08-26 the gate above took three and a half minutes, of which thirteen seconds were spent computing and the rest was spent waiting for one chapter at a time to come down.");
  ("Twenty-two passages are seven bibles here, so seven askings answer what twenty-two were asking, and each of the seven waits on its own passages together instead of in turn.");
