...
export async function functions_builtin_calls_gate_run() {
  "QA gate: no file still calls a built-in method the repo keeps a name for. The normalize pass writes those names, so a file that has been through it has none left.";
  "It stands at zero rather than at a line already drawn. Every file carrying one was moved on the day the built-in joined the list, so there is nothing here being lived with and nothing to grandfather - which is the only moment a gate like this can be added without the baseline immediately becoming the thing everybody reads instead. A built-in is added to the pairings the same way: move every file first, then let the gate hold the line.";
