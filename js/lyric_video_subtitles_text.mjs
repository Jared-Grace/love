...
export function lyric_video_subtitles_text(document) {
  "$plain document";
  "The whole subtitle file of a lyric video, written out of one authored document: the reference that stands at the foot of every frame, how long the song runs, how big the frame is, how big the two lettering sizes are, and the lines with the moment each is sung.";
  ...
  "BOTH LETTERING SIZES ARE ASKED FOR, AND THE SMALLER ONE USED TO BE WORKED OUT AS A SHARE OF THE LARGER. Tying them together says the two are one decision, and they are not: the words are sized to be read across a room from a phone lying on a table, while the reference is sized to be legible without competing with them. Making the words bigger through a fixed share drags the reference up with them into exactly that competition, and the only way out was to change a number that no document could see.";
  ...
  let reference_size = document.reference_font_size;
  let styles_format =
  ...
  let reference_style =
    "Style: Ref,Noto Sans," +
    reference_size +
    ",&H00969696,&H00969696,&H00000000,&H00000000,0,0,0,0,100,100,2,0,1,0,0,2,80,80,110,1";
  let reference_event =
