import { ebible_firebase_upload_path } from "../../love/js/ebible_firebase_upload_path.mjs";
import { app_supper } from "../../love/js/app_supper.mjs";
import { app_prefix_without_fn } from "../../love/js/app_prefix_without_fn.mjs";
export function app_supper_verses_get_upload_destination(ebible_folder) {
  let without = app_prefix_without_fn(app_supper);
  let destination = ebible_firebase_upload_path(ebible_folder, without);
  return destination;
}
