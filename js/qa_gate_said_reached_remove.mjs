...
export function qa_gate_said_reached_remove(said) {
  ...
  while (more) {
    ...
    let newline_found = after.indexOf(newline);
    let quote_at = quote_found < 0 ? length : quote_found;
    let newline_at = newline_found < 0 ? length : newline_found;
    let end = quote_at < newline_at ? quote_at : newline_at;
