...
export function app_code_lesson_statement_name_sum_number_pairs() {
  ...
  ("Three lessons ask the same four sums and differ only in where the total goes on its way out - straight into console.log, into a third name, or back into the first name - so the numbers are picked here once and all three read them from here.");
