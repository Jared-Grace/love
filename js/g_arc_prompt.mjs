...
export function g_arc_prompt(
  ...
  let lines = [
    ...
    "A conversation uses each opener at most once, and the turns under one opener sit together: the player opens a topic once, and the person carries it until it is done, before the player opens another.",
    "The person is the one who hands the topic back. When they have said all they have to say under an opener, on that last turn they ask the player to bring something else up - and that is what lets the player choose the next opener. Write it into what the person says, so the next opener does not arrive out of nowhere.",
    "Not every opener ends that way. The last opener of a conversation ends the conversation instead, so there the person takes their leave rather than asking for another topic.",
    ...
    "No wrong answers. Every passage you name must be the right one - just name its book and chapter together, and its verse numbers.",
    "",
