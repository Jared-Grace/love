...
export function app_code_lesson_expression_not_pair_intro(root) {
  ...
  ("How far a ! reaches, and why what it reaches past has to be wrapped, is NOT built up again here. The lesson that put a ! around a comparison built it a piece at a time, from what the ! would apply to without the parentheses through to the finished line, and a learner arriving here has read it. Built a second time it would be a near-copy of a card they know, which reads as a new thing to learn and is not one.");
  ("So the first card says only the two things that are actually new - that what stands inside can be a whole && or ||, and that the parentheses still say the ! holds all of it - and the second card shows the working rather than telling any more rules.");
  ...
  app_code_container_light_blue_cycle_code_multiple(root, [
    ...
    ["The parentheses say the ", symbol, " holds all of it"],
  ]);
