...
export async function example_new_from_temp(
  ...
  ("IT MUST PASS BEFORE IT IS KEPT, which no hand-written example ever promised. An example whose after does not match what the transform really writes is worse than no example: it is a claim about the repo that the corpus itself contradicts, and until the gate is next run nothing says so. So it is run where it was drafted, before anything at all is written: an example that fails is taken straight back out again, and one that fails so badly it cannot even be run leaves nothing behind to clear away.");
