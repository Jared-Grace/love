...
export function app_en_learn_bible_gloss_urdu_adverb_conjunction_explains() {
  ...
  "‘because’ was widened for a different reason, and the reason is worth keeping because it is a gate rather than a reading. A shared label saying that the word joins two statements and gives the reason stands over ‘because’, ‘for’, ‘since’, ‘that’ and ‘For’, and a label may only be written over where every word carrying it already says everything the label says. ‘for’ and ‘since’ said both halves. ‘because’ said only the reason, so one word out of five blocked a hundred and thirty-two entries from being mended - and the blockage was correct, because those readers were being told something ‘because’ does that this table had never said. The line was short, not hard. Filling it in released the label, which is the second time a refused label has turned out to be pointing at the thinnest entry in this table rather than at anything wrong with itself.";
  "'then' was widened the same way 'there' was, and the half that was left out of it matters more than the half that went in. Its entry told the time job and nothing else, while the store had twenty-five sightings teaching that the word sounds like 'than' and is a different word - the same lookalike shape as 'their', found the same way, by ranking the store under the word and reading what the entries say. That went in.";
  "What did not go in was the sense of consequence, and the reason is a lesson about reading a store. Seventeen sightings do say the word carries the first thing into the second, which looks like a second job going unsaid - but the biggest of those wordings is shown for 'So' and 'Therefore' as well, so it is a sentence about consequence in general rather than a discovery about this word. Set against that, 'after that' already gives a beginner everything narrative consequence needs, and a third sense would turn a two-sentence entry into a grammar lesson. A stored wording is evidence about a word only where it is not equally about several.";
  ...
  let r = {
    ...
    then: "یہ لفظ وقت کا ظرف ہے، یعنی وقت بتاتا ہے: 'پِھر'، یعنی اُس کے بعد۔ اِسے 'than' سے مِلا نہ دیں: وہ اَور لفظ ہے، جو دو چِیزوں کا فرق بتانے کے لیٔے آتا ہے۔",
    ...
    because: "یہ لفظ دو باتوں کو جوڑتا ہے اَور وجہ بتاتا ہے: 'کیونکہ'۔",
    since:
