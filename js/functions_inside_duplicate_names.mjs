...
export async function functions_inside_duplicate_names() {
  ...
  ("A group every member of which is marked as alike on purpose is not named, because there is nothing here for a ratchet to hold: the shared run is the point of the mark, and a record that carried the group would be asking for the one collapse the mark exists to refuse.");
  ...
  let unmarked = await functions_duplicate_words_unmarked(once);
  return unmarked;
}
