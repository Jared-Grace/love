...
export function app_code_expression_equals_text(solved_code, value_text) {
  ...
  ("The piece is always gathered into parentheses, whatever it is. Left bare, a line like 5 === 8 === false asks the reader to take the two === in the order they are written, and nothing in this track has taught an order for operators of the same strength - order has only ever come from strength or from parentheses. Parenthesised, it needs neither.");
  ("Always rather than only where it is needed, because the reader is being taught one idea here and a parenthesis that comes and goes is a second one. A rule with no case in it is the rule they can carry to the next line they meet.");
