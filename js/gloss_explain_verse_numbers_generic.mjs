...
export function gloss_explain_verse_numbers_generic(
  ...
  "A SHUTTER IN FRONT OF THE WORD FOR VERSE MEANS THE WORD IS A NAME FOR A PLACE RATHER THAN A LABEL, AND THE READING NEVER OPENS. In verse three names a verse; the verse three times counts how often something happened in the verse being explained, a few verses two hands counts verses and then starts a fresh phrase, and this verse three times does the same. What separates them is the one word in front of the marker rather than anything behind the number, so that one word is looked at and the marker is shut before it can open.";
  "WHICH WORDS SHUT WAS READ OFF THE STORE RATHER THAN GUESSED. Of the four hundred and thirty places the original-language store put a number behind the word for verse on 2026-09-25, three hundred and sixty-three had a word of place in front of it - in, of, from, as, to, at - and every one of those was a real claim. Nineteen had the in front, and all nineteen named nothing: the verse two back, the verse three times, the verse one saying rather than two. Five had that in front, and all five were real claims - the land that verse one told us of - so that is deliberately left out, and a case here holds it so it cannot quietly be added.";
  "Only words of unstated count are shut, and only the definite article among the pointing words. A few verses, several verses, some verses: none of those can be followed by a verse label, because nobody writes a few verses three and four - they write verses three and four. A stated count can be: both verses eight and eleven is a real claim this store makes, and the two verses three and four is sayable, so both, all, two and last are left out on purpose. Leaving a count unread costs one row in a list a person reads; reading a real claim as a count would drop it in silence, which is the dearer mistake of the two.";
  "Only the one word in front of the marker is looked at, which leaves the tenth verse two lines back uncovered - the shutter is there but it is two words back, behind an ordinal. That was met once on 2026-09-25 and is written into the cases as it stands rather than being reached for with a second step, because every extra step back is another chance to shut a marker that was naming something.";
