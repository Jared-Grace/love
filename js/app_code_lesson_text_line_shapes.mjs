...
export function app_code_lesson_text_line_shapes(text) {
  ...
  ("every way a piece of text is SHAPED as a line of code, named. Whether it is parenthesised or flat, whether it writes one operator over and over or mixes two, which end a true or a false sits at, and what kinds of thing stand either side of the operators.");
  ...
  ("Each reading keeps its own idea of how much line it needs. Parentheses and the mix say nothing until there are two operators to order; a side and a kind are already there with one. A threshold shared out of tidiness would have been a claim that the grains are the same kind of thing, which they are not.");
  ...
  let found = [];
  let parenthesis_shape =
    app_code_lesson_text_line_parenthesis_shape_or_null(trimmed);
  let some = null_not_is(parenthesis_shape);
  if (some) {
    list_add(found, parenthesis_shape);
  }
