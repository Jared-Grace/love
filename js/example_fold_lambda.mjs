...
export function example_fold_lambda(x_name) {
  let f_name = fn_name("js_fold_all");
  `Adapt ${f_name} (two ASTs) to the examples-gate's one-file transform shape: bind pure fn x by name`;
  let f_name2 = fn_name("js_fold_all");
  `(read its real repo AST live) and return a lambda over F's AST. On refusal ${f_name2} returns null`;
