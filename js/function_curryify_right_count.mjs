import { integer_to } from "./integer_to.mjs";
import { function_curryify_right_count_args_get_curried_right } from "./function_curryify_right_count_args_get_curried_right.mjs";
import { function_name_combine } from "./function_name_combine.mjs";
import { arguments_assert } from "./arguments_assert.mjs";
import { function_curryify_right_name } from "./function_curryify_right_name.mjs";
import { function_curryify_generic_open } from "./function_curryify_generic_open.mjs";
export async function function_curryify_right_count(f_name, count) {
  arguments_assert(arguments, 2);
  count = integer_to(count);
  function lambda(unaliased) {
    let n = function_curryify_right_name(unaliased);
    let combined = function_name_combine(n, count);
    return combined;
  }
  let r = function_curryify_right_count_args_get_curried_right(count);
  let output = await function_curryify_generic_open(f_name, lambda, r);
  return output;
}
