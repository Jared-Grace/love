export function app_en_learn_bible_gloss_urdu_word_explains() {
  ...
  "‘this’ was widened within the hour of being written, and the widening came from the labels rather than from second thoughts. Written first as pointing at a near thing, it was then asked to stand over two hundred and forty places, and one of the labels waiting there spoke of a near *one* rather than a near thing - which in the Bible is usually a man. A wording that says ‘thing’ would have been written over every one of those and quietly narrowed them. So it says thing or person, and the first form is retired in the superseded list beside it.";
  ...
  let r = {
    ...
    this: "یہ لفظ کِسی پاس کی چیز یا پاس کے شخص کی طرف اِشارہ کرتا ہے، جیسے اُردُو میں ’یہ‘۔ دُور والے کے لیٔے ’that‘ آتا ہے۔",
