...
export async function module_web_latest_resolve(meta) {
  ...
  "IT EXISTS SO THAT THE CHECKED STAGE CAN BE SERVED BY ITS OWN NAME RATHER THAN BY BEING INSIDE SOMETHING ELSE. Until 2026-09-03 it was reachable only because it happened to sit under the published folder, which is served whole - so the address the checked build is opened at was an accident of where the folder was kept, and moving the folder anywhere else would have taken that address down without a word. It moved that day, out from under the published folder to stand beside it, and the address answered on through it because this had been written first. Asked for by name, it keeps answering wherever it is kept.";
