...
export function gloss_passage_words_misaligned(passage, words_read) {
  ...
  "The comma a word happened to sit in front of is taken off both sides before they are compared, because an explanation names the word and not the sentence it was standing in. What is left is compared exactly, accents and vowel points and all, since those are part of the word.";
  ...
  let written = list_map(words_written, text_punctuation_removed);
  let word_key = app_shared_gloss_bible_generate_generic_word();
  let explained = [];
  function entry_read(entry) {
    let word = property_get(entry, word_key);
    let words = text_split_space(word);
    let bares = list_map(words, text_punctuation_removed);
