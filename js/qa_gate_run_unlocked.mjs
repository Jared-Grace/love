...
export async function qa_gate_run_unlocked() {
  ...
  ("It is said before the throwing rather than after it, which is the whole of what makes it reachable. Put below, it was reached on a green run only - and this repo has been red for days on gates belonging to work several people have in flight, so a number written to be read on every run would in fact have been read on none of them. How long the parts took is printed twice for exactly this reason, once on the way through and once on the way out, and the second of those is the copy anybody has actually seen.");
