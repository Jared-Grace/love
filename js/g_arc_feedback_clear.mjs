...
export async function g_arc_feedback_clear(chapter_code, index) {
  "Drop the notes that were waiting on this pass against one person's arc, leaving the ones put off to the second pass and the notes against everybody else in the chapter where they are, and say how many were dropped.";
  "$plain chapter_code";
  ...
  "A NOTE PUT OFF TO THE SECOND PASS SURVIVES BEING CLEARED, and that is the whole of what makes putting one off different from withdrawing it. Revising a person clears their notes, so before this a note queued for later was destroyed by the very next unrelated revision of the same person - the queue emptied itself, silently, and the reviewer who queued it had no way of noticing. Only a note that was waiting on this pass is dropped here, because only such a note has been answered.";
  arguments_assert(arguments, 2);
  let person = number_from_text(index);
  function note_kept(note) {
    let left = property_get(note, "index");
    let theirs_not = not_equal(left, person);
    let deferred = g_arc_feedback_deferred_is(note);
    let keep = or(theirs_not, deferred);
    return keep;
  }
  let r = await g_arc_feedback_kept_write(chapter_code, note_kept);
  return r;
}
