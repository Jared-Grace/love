...
export async function public_chunks_orphaned_noted_not(f_paths) {
  ...
  ("WHERE THE PIECE SITS IS ASKED WITHOUT ASKING WHETHER IT MAY BE WRITTEN TO, AND THE DIFFERENCE IS THE WHOLE OF WHETHER THIS WORKS. The neighbouring reading that guards the write turns down every path inside a copy held in memory, on purpose: such a copy is thrown away and remade, so nothing is ever sent out of one and no write to it need be stopped. But the judging of a commit is run in exactly such a copy, and this is the reading the judging performs. Asked through the guard's reading, no piece anywhere was at the top of any published folder, nothing was ever found to be vouched for, and the file went on being named at the one place whose answer decides. Written the first way, this was measured working here and doing nothing at all where it counts - for a whole day, on the app it was written for.");
  ...
  for (let f_path of f_paths) {
    let root = await folder_public_root_is(f_path);
    if (root) {
