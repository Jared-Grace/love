...
export async function hash_key_doors_callers() {
  "Every function that calls a door onto the address of a page, kept beside the door it calls, as {caller, door}. Read-only.";
  "The pair rather than the name alone, because which door was called says which of its arguments the field name is, and the two doors do not agree on that today only by accident - one takes the field and the value, the other reads the address and writes it back.";
