...
export function gloss_root_unwalked_cause(known, folded_index, root) {
  "Why a dictionary took no step on from the root it gave: unasked where it has never seen the word, spelled_otherwise where it has it under another spelling and does take a step from there, unanalysed where it has the word and never took it apart, rootless where it took it apart and named no further root, and walked where it did take a step and there is no silence to explain.";
  "Four fifths of the disagreements gathered so far end with the dictionary saying nothing, and a pile that size is read as one thing and worked on as one thing. It is not one thing. One of these answers closes by asking the dictionary again, one closes without asking anybody anything, and two of them will never close at all - so a plan built on the count alone spends its effort on the part that cannot move.";
  ...
  "spelled_otherwise is the answer that costs nothing to act on, and it is only given where acting on it would actually get somewhere. The other spelling is looked up and then asked every question a directly held word is asked, because an entry reached for free that carries no breakdown of its own has moved nobody any nearer an answer. Answering on the reaching alone would have counted those as winnable, which is the same mistake as counting an unanswerable page with the unasked ones.";
  ...
  let entry = missing ? otherwise : held;
  let nowhere = null_is(entry);
  if (nowhere) {
    let unasked = "unasked";
    return unasked;
  }
  ...
  let named = property_get(entry, "root");
