...
export async function bible_glyph_chapters_verse_marks_underdrawn_word_draw() {
  ...
  "SO YOU MAY LOVE HIM WITH ALL YOUR HEART seats the covenant name twice and draws it once, and the leftover row is glossed Him, and the verse holds one plain Him. You must not defraud your neighbour or rob him is two negative clauses in the original, each seated on the negation mark, and English carries the second one on the word or - which is exactly what the leftover row is glossed. The same rule settles both, and it was the second of those that taught it.";
  ...
  for (let offender of offenders) {
    ...
    for (let word of row.words) {
      ...
      let item = text_letters_only(lowered);
      list_add(glosses, item);
    }
    ...
    for (let entry of plains) {
      ...
      let bare = text_letters_only(lowered2);
      let seated2 = list_includes(glosses, bare);
      let already = list_includes(shared, bare);
      let take = seated2 && not(already);
      if (take) {
        list_add(shared, bare);
      }
    }
    ...
    for (let entry of plains) {
      ...
      let bare2 = text_letters_only(lowered3);
      let same_word = equal(bare2, word_shared);
      if (same_word) {
    ...
    for (let gloss of glosses) {
      let same_gloss = equal(gloss, word_shared);
      if (same_gloss) {
