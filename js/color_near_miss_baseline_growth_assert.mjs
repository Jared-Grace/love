...
export async function color_near_miss_baseline_growth_assert(known) {
  ...
  let recorded = await color_near_miss_baseline_read();
  let added = list_difference(known, recorded);
  list_empty_is_assert_json(added, {
    hint: "these colours read as one colour now and did not before — reuse the existing colour rather than recording the near miss as known",
    added,
  });
}
