...
export function bible_usfm_versions() {
  ...
  ("THE FIFTEEN ENGLISH TEXTS BELOW THE FIRST FOUR WERE NOT PICKED ONE AT A TIME. They are every complete English translation eBible carries that nobody owns at all - derived from the licence pages by ",
    fn_name("bible_usfm_versions_english_public_domain"),
    " and fetched by ",
    fn_name("bible_usfm_versions_english_public_domain_download"),
    ", so the block can be regenerated rather than argued about. Public domain rather than merely shippable, because these are here to be sung out of: a song that repeats a word the passage says once is a new work made out of the translation, and a share-alike text would ask that the song be given away on the same terms as the text it was built from.");
  ("eBible packs the Berean as well, and it is not listed twice. It comes off the berean shelf, from its publisher own release, because one translation under two short words is two answers to the same question and nothing would say which was meant.");
  let versions = {
    bsb: {
