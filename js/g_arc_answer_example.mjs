...
export function g_arc_answer_example() {
  ...
  "The NESTING is the part only an example can carry. Every field is a plain string now, so no type has to be taught here: the turn names its passage by copying the citation out of the brackets it is already shown in, which is the same string on both sides of the call. It once split that citation into a book_chapter and a verse_numbers list of strings, and the list was the one thing an example had to demonstrate, because neither its bracketing nor its quoting could be inferred from a passage line reading [1,2].";
  ...
  "It is built as an object and printed by the same reader the profile goes through, so it cannot drift from the width the rest of the prompt's JSON is written at.";
  "THE CONTINUATION MARKS ARE THE ONE THING WRITTEN AFTERWARDS, and they are the reason this no longer prints as valid JSON. Two turns and one conversation are all the structure needs, and a shape shown closed reads as the shape asked for - so both lists end in an ellipsis saying the list goes on. JSON has no way to spell that, so each is carried through the printer as the string \"...\" - which puts it at exactly the indentation of the element above it, the part that would be guesswork if the line were spliced in by hand - and the quotes are taken off after.";
  let ellipsis = "...";
  let quoted = json_format_to(ellipsis);
  ...
  list_add(turns, ellipsis);
  property_set(held, "turns", turns);
  let conversations = [held, ellipsis];
  property_set(answer, "conversations", conversations);
  let printed = json_format_to(answer);
  let r = text_replace(printed, quoted, ellipsis);
  return r;
}
