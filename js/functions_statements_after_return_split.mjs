...
export async function functions_statements_after_return_split() {
  ...
  "What is left over after both marks is a tail of pure work with a real body above it. The two marks narrow rather than decide: run on 2026-08-17 they took seventeen names down to four, and reading those four by hand found one more kind neither mark can see - a utility with several returns down its length, where somebody moves the line up and down to choose which step runs. Nothing written on the page tells that apart from a forgotten tail, so the last four are still read before anything is taken out of them.";
  "The work above each remaining name travels out beside it, because a name with a single line above it is the one to read first.";
