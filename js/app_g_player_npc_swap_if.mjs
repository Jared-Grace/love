...
export function app_g_player_npc_swap_if(g, from, to) {
  ...
  "It is nearly always a STRANGER who is traded with. The way through is worked out with the player's own line left standing, so a follower is not somebody the player steps onto while there is any other way at all - a line is passed by backing it down its own trail, in the order it was walked. The one exception is a line with no trail left to back into, which would otherwise wall its own leader in for good: there the player passes the line the way they pass anybody, trading with each of them in turn, and the line is turned around into the order they end up standing in once the walk is over.";
