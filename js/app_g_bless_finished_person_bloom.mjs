...
export function app_g_bless_finished_person_bloom(div_map, person) {
  ...
  ("it, for the same reason the burst beside it does. That light is an empty box whose whole");
  ("job is to ride, so anything put in it rides, and the gold is free to come up on its own");
  ("clock underneath this rather than having to be shown before it.");
