...
export async function lyric_video_document_pictures_model_bench_attempts_generic(
  ...
  "$plain attempts";
  "$plain size";
  "Draws every scene an authored document names several times over on one model at one named size, each try into a file of its own, so that how often a model gets a scene right can be counted rather than guessed at.";
  ...
  "★ COUNTING TRIES IS ALSO WHAT PRICES A SIZE, WHICH IS WHY THE SIZE COMES THROUGH HERE AND NOT ONLY THROUGH THE RUN BELOW. What a cheap rung costs is its list price divided by the share of its pictures somebody would keep, and that share can only be had by drawing the same scenes several times at that rung and counting. A rung tested once tells you nothing about the divisor, which is the term that decides the answer.";
