...
export async function bible_audio_reading_changed_queue(
  ...
  "★ IT ASKS THE LATER OF THE TWO DOORS, AND THE OLDER ONE WOULD HAVE ANSWERED WRONG. The older door is where the reading stopped leaving an unknown name silent; the dictionary of names came three days after it. Luke 3 falls between them, so the older door calls its sound current while both of Pilate's names are spoken wrongly in it. Anything under the later door is under the earlier one too, so asking this one alone misses nothing.";
