...
export function key_getters_frozen_gate_run_generic(pairs, part) {
  ...
  "The way out was two steps for a while, and this named only the second. The list of frozen names is kept by hand, and the recorder only ever writes down what the names already on that list say today - so a function nobody has added yet is not missing a value, it is missing a mention, and running the recorder at it answers that it added nothing and leaves this exactly as red as it was.";
  "Somebody followed the old wording on 2026-08-14, got added nothing back, and had to read three functions to find out that the hand-kept list was the step nobody had mentioned. One command does both halves now, which is why the wording below names one thing and not a sequence.";
  "What the command cannot do is the part left standing in the hint: every entry in that list has a paragraph above it saying how the word escaped, and that is the part a reader needs and the part no rule can compose.";
