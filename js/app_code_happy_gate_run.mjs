...
export async function app_code_happy_gate_run() {
  ...
  "That name is the whole of what it accuses, and the walk's own account of what went wrong is a bystander - it is a sentence about a screen and it mentions whatever the screen mentions, so read as a list of names it would accuse strangers. So the app goes in the list and the account goes under the hint, which is dropped before the names are read.";
  "It hands back how many screens the walk actually reached. A verdict on its own cannot be told apart from a walk that pressed nothing: both say the same silent word. The count is what the walk touched rather than a number worked out from the offenders, which on every passing run is nothing at all.";
  ...
  async function walk(base) {
    ...
    reached = await app_code_happy_url_walked(url);
  }
  ...
  let trail = property_get(reached, "trail");
  let measured = trail.length;
  let r = {
    measured,
  };
  return r;
