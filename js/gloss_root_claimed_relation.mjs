...
export function gloss_root_claimed_relation(root, claimed) {
  ...
  "★ IT IS SOUND BETWEEN TWO ROOTS AND MISLEADING BETWEEN A WORD AND ITS ROOT, AND THE ANSWER LOOKS THE SAME EITHER WAY. Both names say root for a reason. Asked about a word and the root it was built from, apart comes back for sulugoon under sugo, kusgan under kusog, hukman under hukom and lubnganan under lubong, all of them correct, because Cebuano drops the vowel out of a root last syllable when a suffix goes on and that is what breaks the shared run. Measured over the whole gloss store it answered apart for 2311 of 42484 entries that way, and the top of that list is almost entirely good morphology. Between two roots there is no affix in the way and the answer means what it says.";
  "$plain root";
  "$plain claimed";
