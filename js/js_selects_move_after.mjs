...
export function js_selects_move_after(ast, selects) {
  ...
  let node_moved = list_first(selects);
  let node_target = list_last(selects);
  let f_moved = js_node_to_block(ast, node_moved);
  let f_target = js_node_to_block(ast, node_target);
  let body = property_get(f_moved, "body");
  let body_target = property_get(f_target, "body");
  let same = equal(body, body_target);
  assert_message(
    same,
    "The two chosen lines were expected to live in the same block. Would you like to check that both sit in the same scope?",
  );
  let item_moved = property_get(f_moved, "item");
  let item_target = property_get(f_target, "item");
  let index_from = property_get(f_moved, "index");
  let index_to = property_get(f_target, "index");
