...
export function html_scroll_centered_coordinates_element(
  container_e,
  tile_component,
) {
  arguments_assert(arguments, 2);
  ("$plain container_e");
  ("Which square of the grid is in the middle of the scrolling box right now, counted in squares and allowed a fraction of one.");
  ("The exact opposite of the sum that works out where the box has to stand for a named square to be in the middle, run backwards from where the box actually stands. It is written from the same three measurements as that one - half the box, where the middle of the first square sits, and how wide a square is drawn right now - so the two cannot drift apart into different ideas of where the middle is.");
