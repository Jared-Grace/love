...
export function app_g_map_pad_check() {
  ...
  const size = 2 + 2 * pad;
  assert(equal(padded.length, size), "padded has inner + 2*pad rows");
  assert(equal(padded[0].length, size), "padded has inner + 2*pad cols");
  assert(equal(padded[0][0], water), "top-left corner is water buffer");
  assert(equal(padded[size - 1][size - 1], water), "bottom-right corner is water buffer");
  assert(equal(padded[pad][pad], "a"), "core top-left preserved at [pad][pad]");
  assert(equal(padded[pad + 1][pad + 1], "d"), "core bottom-right preserved");
  assert(equal(padded[pad - 1][pad], water), "the tile just above the core is water");
  return {
    ok: true,
    size,
  };
