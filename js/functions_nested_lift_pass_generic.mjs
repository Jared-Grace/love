...
export async function functions_nested_lift_pass_generic(
  candidates_fn,
  lift_fn,
) {
  arguments_assert(arguments, 2);
  ("One walk down the work list handed in, moving out the piece it names inside each function that has one by the move handed in, each under a name worked out from the two names it has, and each committed under its own command before the next one starts.");
  ...
  ("Which list and which move are handed in because there are two of each and they pair up: the narrow list with the move that takes the name away, the wide list with the move that decides per piece. Everything between them - working out the new name, the two steppings-over, the refusal a name already spoken for, committing each move under its own command - was the same lines whichever pair was walking.");
  let candidates = await candidates_fn();
  let lifted = [];
  let skipped = [];
  for (let row of candidates) {
    ...
    await function_call_commit(lift_fn, [f_name, nested, f_name_new]);
    list_add(lifted, {
      name: f_name,
      nested,
      f_name_new,
