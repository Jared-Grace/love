...
export async function app_message_download_private() {
  "Brings every message down out of storage onto this machine's disk and keeps it, each file written under the folder held away from the repos at the same address it has in the bucket. Answers with the addresses written.";
  ...
  "Written where git cannot see it, and that is the point rather than a detail. This repo is public and is committed by commands that sweep whatever they find, so a copy anywhere under a working folder would be published by a run whose whole job is to commit - see the folder it lands in, which says so at the top.";
  ...
  async function lambda(item) {
    ...
    let f_path = folder_private_path(name);
