...
export async function g_arc_review_chapter_cards(chapter_code) {
  "$plain chapter_code";
  "Everything a reviewer needs about one chapter's arcs in a single answer: every person written for it, their turns with the Scripture each answer named, every note already standing against a line, and what has moved in each arc since it was last read.";
  ...
  "THE ARC AS IT WAS READ IS READ IN THE SAME BREATH AS THE ARC ITSELF, and that matters more here than for any of the others. The two are compared against each other, so a gap between the reads is a gap in which the live arc can be rewritten - and the answer would then show a difference against a version nobody ever saw beside a version nobody has read.";
  ...
  for (let entry of arcs) {
    ...
    let reviewed_arc = g_arc_chapter_person_or_null(reviewed, wanted);
