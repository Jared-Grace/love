...
export function app_g_conversation_sky_target(conversation_fraction) {
  "the sky PHASE a conversation drifts to at the given progress (0..1), aware of the day SLICE it belongs to. in a day session (slices_total > 0) the conversation advances the sky only through ITS slice — from slices_done / slices_total to (slices_done + 1) / slices_total of the working day, which g_day_sky_phase maps onto 6 AM → 7 PM — so N people fill the day one slice each and the real game just advances its own slice. the #day_conversation demo runs a 1-slice day, so one whole conversation spans 6 AM → 7 PM and the change is easy to SEE. with NO day session (an ordinary tapped-npc conversation) it keeps the standalone morning → night arc (fraction times the night phase index), so real play is untouched.";
