...
export async function bible_glyph_chapters_verse_marks_overdrawn_walked() {
  ...
  ("A MARK THAT STANDS FOR NO ORIGINAL WORD IS SKIPPED RATHER THAN COMPARED, the same way and for the same reason as in the chapter-wide reading next door. The name badge says the letters beside it are somebody's name, and nothing in the Hebrew or the Greek is under it, so every verse it ever stands in seats it nought times - which is the loudest fault this reading knows and the wrong answer in every single case. Skipping it before the walk count is counted keeps that count honest: it is how many marks were compared, and these are not compared.");
