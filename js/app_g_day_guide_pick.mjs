...
export function app_g_day_guide_pick(g, player, target, window_tiles) {
  ...
  ("The window arrives as the one thing it is rather than as four loose numbers, and whether a tile is in it is asked of the name that already answers that. It used to take the four edges apart and compare them here, which is the same test written a second time - and a second copy of a test that decides where the gold may land is a place for the two to disagree about the edges, which nothing would report.");
  ...
  for (let candidate of property_get(g, "coordinates")) {
    ...
    let in_window = g_coordinates_window_inside_is(candidate, window_tiles);
    if (not(in_window)) {
      continue;
    }
