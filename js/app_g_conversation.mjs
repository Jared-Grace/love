...
export async function app_g_conversation(
  ...
  function run_turn(turn) {
    ...
    async function on_correct() {
      "the RIGHT verse landed: this turn is done (dropped from remaining) and the day ticks forward. before the next screen, the NPC speaks the AFTER — its warm response to the Word (the fruit), generated by g_response for this turn's after_kind — carried as the pending intro so render_openers / render_pray say it in place of the usual continue-prompt: 'npc says the after, then asks what to talk about next'. one turn per opener in the demo, so the after always leads to the menu; a multi-turn thread would instead chain into the next turn's before.";
