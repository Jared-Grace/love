...
export async function app_g_conversation(
  ...
  function render_openers() {
    ...
    ("the turns carrying an order are separated out FIRST, and both the lowest order and the turn holding it are looked for among those alone. asking a turn for an order it has not got is an error here, not an empty answer, so a saved conversation from before orders existed died on the question instead of falling back - the fallback the line above promises was written but could never be reached. measured in the live game: every such conversation threw at the moment it drew its choices, leaving the player looking at a greeting and a 'one moment, please' that never went away.");
    ...
    let correct_turn = list_find_property(ordered, "order", order_min);
    if (not(correct_turn)) {
