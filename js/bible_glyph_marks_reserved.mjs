...
export function bible_glyph_marks_reserved() {
  ...
  "THE POOL IS AT ZERO USABLE AND RESERVING CANNOT FIX THAT (2026-08-25), which is what this list looks like once both of its entries are read. Two pictures reach no word edge and both are on this list already, so the next group that wants a safe half has nothing to be given. Reserving is a way of KEEPING a picture and not a way of making one; the only move left is buying.";
  "AND ANY BOUGHT PICTURE IS SAFE, WHICHEVER ONE IT IS. Reaching no word edge is a consequence of never being seated and not a property of the drawing, so there is no search to run and no candidate that fails on structure. That makes the whole of the choice a question about MEANING, which is worth saying plainly because the two questions have been mixed up once already and the structural one is the one that feels like work.";
  "THE STANDING PROPOSAL IS AN UP ARROW MEANING THIS ONE IS GOD'S, and it is written here as a proposal because the decision is not one this file can make. It was wanted for the burning heart, where God's jealousy and a man's envy share a mark and the second sense libels the first; and it would be wanted again by every word the tables seat where one sense belongs to God alone. One general qualifier is worth more than one more specific picture, which is the argument for buying this rather than buying a second heart.";
  "IT HAS TWO OBJECTIONS AND BOTH ARE REAL. This table ALREADY says God, with the fire, so a second divine mark is bought for its lack of edges rather than for what it means - and a reader who learned the fire in the first verse will want to know why the arrow is not the fire. And a reserved picture is never seen alone, so it can never be learned by pointing at the thing it draws, which is this Bible's whole method; the trays and the hands are learned without pointing, but they are learned in PAIRS, and a lone qualifier has no opposite to be told apart from.";
