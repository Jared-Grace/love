...
import { firebase_hosting_ignore_walked } from "./firebase_hosting_ignore_walked.mjs";
...
export async function firebase_hosting_ignore_gate_run() {
  ...
  "HOW MANY STAGES WERE ASKED ABOUT TRAVELS OUT WITH THE VERDICT. Nothing missing is what a good day says, and it is also what a run says whose list of stages to keep off the internet has quietly become empty - the same green, about a question nobody asked. The count is the part of the answer that falls when that happens, and the offenders are not: they are nothing on every run that passes anyway.";
  arguments_assert(arguments, 0);
  let told = await firebase_hosting_ignore_walked();
