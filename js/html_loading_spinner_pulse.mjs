...
export async function html_loading_spinner_pulse(pulse, milliseconds) {
  ...
  while (html_connected_is(pulse)) {
    ...
    let left = text_combine("scale(", smallest);
    let style_value = text_combine(left, ")");
    html_style_set(pulse, "transform", style_value);
    await sleep(milliseconds);
    let largest = random_range(1.06, 1.34);
    let left2 = text_combine("scale(", largest);
    let style_value2 = text_combine(left2, ")");
    html_style_set(pulse, "transform", style_value2);
    await sleep(milliseconds);
  }
}
