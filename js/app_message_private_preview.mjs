...
export async function app_message_private_preview() {
  ...
  "The time and the mark of whoever wrote it go above the bubble rather than inside it, in smaller letters, because a bubble is the words and nothing else. Inside it they would have to follow the words, which is not where a reader looks for them.";
  ...
  function each(record) {
    ...
    let line = html_p_text(listed, head);
