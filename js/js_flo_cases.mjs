...
export function js_flo_cases() {
  ...
  "Two of the cases stop rather than answer, and both are written down with the same one word, because what is being watched here is that the reading stops at all rather than what it says while stopping. A module with no export and a module with two are the two ways of not having one, and a reading that started answering about one of two would be picking, silently, which is the thing it was written not to do. The words it stops with are its own business and are left to it.";
