...
export async function app_music_references_fill(asked_list, song, texts_asked) {
  ...
  "THE FILE IS ALREADY ON ITS WAY WHEN THIS IS CALLED, and that is why what arrives is handed in rather than asked for here. This runs after the whole song has been drawn, and the set of passages was fixed before the page opened - so waiting until here to ask put the whole of the drawing between the page arriving and the request leaving, for nothing. It is a small saving on a good connection and it is not written down as more than that.";
