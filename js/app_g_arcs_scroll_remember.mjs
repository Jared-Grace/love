...
export function app_g_arcs_scroll_remember(panel, watched) {
  "Files how far down the arcs bench the reader has come, under the sheet they are reading, so a refresh puts them back rather than at the top.";
  ...
  "IT IS FILED UNDER THE SHEET, which is one person's arc in one chapter, because two of those are two different lengths of writing - and being put back a thousand pixels down a sheet that was never scrolled is worse than being put at the top of it. It was filed under the chapter while the bench drew a chapter's people end to end; the day the bench held one person at a time, a chapter stopped naming what was on screen.";
  ...
  let sheet_code = property_get(watched, "sheet_code");
  let unknown = null_is(sheet_code);
  if (unknown) {
    return;
  }
