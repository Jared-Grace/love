...
export function g_sermon_chapter_verses_text(passages) {
  ...
  "The colon also parts two runs of DIGITS - the chapter number and the verse number - where a space is the weakest boundary available at exactly the point a reader has to tell one from the other.";
  ("The whole citation is copied back as it stands, rather than taken apart into a chapter and a list of verses. Copying is the one thing that cannot be got wrong: a reference reproduced whole either matches a passage that was handed over or matches none, while a chapter and its verses answered separately can each be right and still name a passage nobody offered - verse 5 alone, where the passage was 5 and 6 together. So the taking apart happens here, where the passages are, and ",
    fn_name("g_arc_answer_passage"),
    " finds the passage by the citation ",
    fn_name("g_passage_reference"),
    " wrote.");
  ("The book is SPELLED OUT rather than coded: Romans 8, never ROM08. The code is the game's own shorthand and is a spelling of a book name that exists nowhere outside this repo, so a prompt written in it asks the writer to work in a private notation for the one thing it most needs recognised. The lookup costs nothing and cannot resolve a book the arc was never offered.");
