...
export function app_code_expression_node_left_operator_first(
  ...
  ("Parentheses are never written here, and whether the printed line carries any is not the same answer for every caller: a first operator that is the stronger of the two gathers its sides on its own and needs none, and one that is weaker can only mean what it says with parentheses round it. Both come out right because the helper that prints the line reads the need off the shape instead of being told, which is what lets this one builder serve a lesson that must have parentheses and a lesson that must not.");
