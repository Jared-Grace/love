...
export function g_coordinates_member_is(coordinates) {
  ...
  ("the index is built here rather than by the caller, because a caller that only wants the question answered should not have to know that the answer is kept in an object keyed by tile - and the ones that hand the same tiles to ",
    fn_name("list_filter"),
    " three times over were each building their own.");
