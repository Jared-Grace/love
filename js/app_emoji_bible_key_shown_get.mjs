...
export function app_emoji_bible_key_shown_get() {
  ...
  "That last rule is the shared one underneath rather than a rule of this app's own, so the twin beside this one - the English on its own - cannot quietly disagree with it.";
  let shown = storage_local_flag_get(
    app_emoji_bible_key_shown_get,
    "key_shown",
  );
  return shown;
}
