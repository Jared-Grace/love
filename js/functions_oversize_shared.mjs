...
export async function functions_oversize_shared() {
  ...
  "IT RAN AGAIN ON 2026-08-25 AND CAME BACK EMPTY. The oversize list had fallen from 130 to 28 and not one of those 28 held a run any other function holds, while the repo still had 43 shared runs in it. So the answer the first run gave has not merely held, it has gone all the way: the two readings are now disjoint, and an empty list here is a finding rather than a failure - both readings were asked the same day and both had plenty to say on their own.";
