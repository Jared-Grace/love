...
import { bless_block_roof } from "./bless_block_roof.mjs";
...
export function app_g_bless_buildings_raise(rows, buildings, block) {
  ...
  ("The roof is a DIFFERENT material from the wall, and the same one on every building of");
  ("this street. Seen from above a house is mostly roof with one row of wall showing, and");
  ("painting the two alike leaves a solid oblong of colour that reads as ground of an odd");
  ("shade rather than as something standing up - the doors then look like marks lying on");
  ("the floor. Giving the roof its own material puts an edge along the front of every");
  ("house, and that edge is what says the wall is a wall.");
  ("Common to every building on the street because a roof is not how a player tells one");
  ("house from another. The wall is, and it can only do that job while everything around");
  ("it holds still. Vary both along a row and the street is noise.");
  ("It varies from STREET to street, which is a different question and answered the other");
  ("way. Between blocks nothing has to hold still - the whole point of walking to the next");
  ("block is arriving somewhere else - and a roof is the second widest thing a block is");
  ("made of after the ground it stands on. So the roof is asked of the block, exactly as");
  ("the pavement and the set of fronts are, and for exactly the same reason.");
  ...
  ("the houses as grass. Grassed, it reads as the garden of the house beside it, and it joins");
  ("the row of grass that runs along the front of the whole street, which is the same green.");
  ("It was PAVED in the block pavement until the street grew a garden. That was right while");
  ("the only ground in front of a house was pavement - a leftover row had to be something,");
  ("and matching the alleys either side of it was the least it could be. Now there is grass");
  ("to match instead, and matching the grass is what makes a set-back house read as a house");
  ("standing further back in its own garden rather than as a house with a paved gap in front");
  ("of it.");
  ...
  let item_yard = bless_yard_grass();
