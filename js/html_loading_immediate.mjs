...
export async function html_loading_immediate(lambda) {
  ("like html_loading, but hides the overlay immediately on finish instead of lingering 300ms. use this for an in-place re-render that is usually served from memory: a fast run never paints the overlay so there is no flash, while a slow run still shows the spinner (never a blank white page) for as long as it actually waits");
