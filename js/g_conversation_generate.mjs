...
export function g_conversation_generate(pronouns) {
  "PLACEHOLDER generator for an unbeliever NPC's conversation object: { turns, converts }. one turn per opener kind (gospel objection, how-are-you struggle, believe doubt), each = { kind, concern, correct:{reference,text}, wrong:{reference,text}, after_kind, prayer_text }. concern is the BEFORE (the need the NPC voices); after_kind names the NPC's AFTER — the warm response once the RIGHT verse lands, generated fresh by g_response (comfort for a met struggle, ponder for a doubt / the gospel to consider). REAL per-chapter content will carry an authored `after` STRING per prompt; the demo generates it by kind so the before/match/after mechanic runs without new static text. prayer_text is what the closing prayer assembles into 'God, please <prayer_text>, Amen'; it names the person with their own GENDERED pronouns (from `pronouns`, per npc.gender) — 'open HIS heart', 'comfort HER' — not neutral they/them. converts=true for now. this is scaffolding so the flow runs";
