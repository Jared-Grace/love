...
export function app_code_lesson_expression_parentheses_minus_divide() {
  "the second parentheses lesson, and the one that answers a question the first leaves open. The first showed parentheses beating a STRONGER operator: (1 + 2) * 3 is 9 where 1 + 2 * 3 is 7. A learner can come away believing parentheses only matter when the operators differ in strength - that they are a way of saying do the weak one first. This lesson shows them changing an answer where both operators are the SAME strength: 12 - (3 + 4) is 5 where 12 - 3 + 4 is 13, and 12 / (2 * 3) is 2 where 12 / 2 * 3 is 18. What the parentheses override here is not strength but the left-to-right order the same-strength lessons taught.";
  ...
  function refill() {
    ...
    let v =
      app_code_lesson_expression_parentheses_minus_divide_outer_minus_line();
    let v2 =
      app_code_lesson_expression_parentheses_minus_divide_outer_divide_line();
    let v3 =
      app_code_lesson_expression_parentheses_minus_divide_outer_minus_line();
    let v4 =
      app_code_lesson_expression_parentheses_minus_divide_outer_divide_line();
