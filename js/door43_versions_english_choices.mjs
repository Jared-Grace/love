...
export function door43_versions_english_choices() {
  ...
  ("THE LANGUAGE IS ASKED FOR BY THE PLAIN NAME EACH ENTRY WRITES, not by its code. The two shelves do not agree on how many letters a language code gets - eBible's English is two and this catalogue's entries write three - so a comparison keyed on the code would quietly hold nothing. The plain name is the one word both sides already spell the same way, and using it needs no third spelling invented to sit between them.");
