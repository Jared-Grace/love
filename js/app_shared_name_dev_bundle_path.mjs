...
export async function app_shared_name_dev_bundle_path(a_name) {
  ...
  "The apps with a dev page are gathered from every repository here, and only some of them are in this one - so the folder an app's bundle is in is a fact about the app and cannot be worked out once and shared. Spelled relative, the word web/public/dev means whichever folder the reader happens to be standing in, which for an app living next door is a folder that will never hold its bundle.";
  ...
  "The repository is found by looking for the app's way in rather than for a name built out of the app's own, because an app may spell its way in either way round and only one of the two spellings is the function that is really there.";
  let app_name = await app_shared_name_main(a_name);
  ...
  let dev_relative = folder_web_dev();
