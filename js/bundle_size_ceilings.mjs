...
export function bundle_size_ceilings() {
  ...
  "reply_local came back over the two hundred, at a hundred and ninety three point nine on the twenty third of August and two hundred and one point six on the twenty fifth. Nothing arrived - the step gate saw no jump across those three days, which is a drift and so exactly what an accidental import does not look like. The reachable set was read again rather than trusted, and it holds no node half at all: nothing this app carries is a function a page cannot run, and the forty seven names sitting below an environment check are the loading splash, the clipboard and the browser side of fetching, which is what the page runs.";
  "The shape win was looked for a second time and is not there. Every carried function over two kilobytes was weighed by how many of its bytes go on the names of slots repeated down a list, the measure that found the languages list and the replacing app's rules, and the largest candidate spends eleven hundred bytes of source - the two that were worth reshaping spent tens of thousands. So there is nothing here to strip and no shape left to change.";
  "The one real win is written down already, in the reply app itself, on the line saying the legacy reply app keeps the authored list in its own bundle while the verses app fetches the list from firebase as data. That list is the biggest thing this page carries at twenty five kilobytes of source, and the fetching half is already built and already uploaded. Taking it is not a refactor - it puts a round trip in front of the first paint - so it is a person's to decide and a phone's to check, and it is named here so the next reader meets it instead of measuring for it again.";
  "So the choice at this ceiling is a person's, and it is between three things rather than the usual two. Raise it a fourth time, which buys about four days at the drift measured above. Or take this app off the list the way the two bible readers were taken off, for the same reason they were - it drifts because an authored list of verses and three hundred and forty languages grow, and the step gate catches a tree arriving without charging anybody for the growing. Or take the fetch above and stop carrying the verses at all. Nothing was done here, because raising a ceiling to fit is the one move that must never be made by whoever is trying to get past it.";
  "None of those three was taken and reply_local is under the ceiling anyway, at a hundred and forty point five against the two hundred, because the thing charging it was never its own. Its foot carries the contact button, whose plain import pulled in the whole writing screen and the firebase upload road behind it - twenty eight and a half KiB of source and forty nine functions exclusive to this page. Fetched by name at the press instead, that tree left twenty pages at once on the twenty sixth of August, and this one dropped sixty one KiB with nothing about its languages list touched. The lesson for the next ceiling is to ask what the FOOT of the page costs before arguing about what the page is for.";
  ("A large app being absent from here is no longer the same as being unwatched. ",
    fn_name("bundle_size_step_gate_run"),
    " measures every bundle there is against how big it was last seen, and refuses a jump of more than sixteen KiB in one step - so a tree arriving in an app too big to have a ceiling still fails loud, in that app, on the step it arrives on.");
  ("Which is why the two biggest bible readers were left off this list on purpose rather than added when a nine KiB win in them was measured. They drift about two KiB a day, so a ceiling with any useful headroom would have to be raised every week or two forever, and each raise costs a person a measurement. The step gate charges nobody anything for drift and still catches the thing a ceiling was wanted for.");
  ...
  ("The paragraph about the gear is wrong too, and this is now the THIRD reading of the same idea to end this way, so here is the thing that kills it wherever it is raised again. The list is not a menu. It is the dictionary that turns the reader's own chosen language code into a record, and every bible reader does that before it can show anything: the choice arrives in the url hash and goes through ",
    fn_name("ebible_languages_from_codes"),
    " to become a language, and the chapter cannot be fetched until ",
    fn_name("ebible_language_to_bible_folder"),
    " has found the folder to fetch it from. Both walk the whole list, both are on the path to the first paint, and neither has anything to do with the gear.");
  ("So there is no app anywhere in this repo that reaches the list only after a press. Answering the gear's question with a count would free nothing, because the readers underneath it want the list anyway. The way to stop paying for this list is to stop it being a list of records, which is what was done, and there is no timing left to fix.");
  ("The same shape was then looked for elsewhere and found once, much bigger. Every generated getter in the repo was weighed by how many of its bytes go on the names of slots repeated down a list, and only one is in the languages list's league: the replacing app's saved list of which rules each goal leant on, fourteen hundred and forty one rules spelling left, right and original once each. Reshaped into three words to a rule, the app's bundle went from a hundred and ninety eight KiB to a hundred and sixty three - thirty five KiB, or nearly a fifth of the whole page, and the expanded list is identical byte for byte before and after.");
  ("Nothing else is worth doing. Below those two the next candidates spend under eight KiB each on slot names, and most spend under two - so this is a finding rather than a sweep, and the way to notice the next one is to weigh generated getters again after a big one is written, not to go looking now.");
  ("The replacing app is deliberately not given a ceiling here either, for the reason the bible readers were not: adding an exercise adds its rules to that saved list, so the page drifts upward every time the course grows, and a ceiling would have to be raised by hand each time. The step gate catches a tree arriving without charging anybody for the course growing.");
