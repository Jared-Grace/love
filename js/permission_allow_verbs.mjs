...
import { permission_settings_path } from "./permission_settings_path.mjs";
...
export async function permission_allow_verbs() {
  ...
  "reading the file rather than the source that generates it, because what decides is what is on disk. the two are held together by a gate, and if they ever part this should say what is actually in force rather than what was meant to be.";
  ...
  let path = permission_settings_path();
  let rules = await permission_settings_allow_read(path);
  let verbs = set_new();
