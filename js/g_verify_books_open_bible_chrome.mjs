...
export async function g_verify_books_open_bible_chrome() {
  ("Open each active sermon-loop book's CURRENT verse (the latest-written one, awaiting approval) in its own tab, all in ONE new Chrome window. Reads the live loop state via g_verify_loop_check_line, deep-links the PROD bible app per book (bible.html#c=<chapter_code>,v=<verse> — the hash pairs are COMMA-separated, per html_hash_object_get), and spawns google-chrome --new-window with every URL. A one-command review shortcut to eyeball all in-flight passages at once.");
  let state = await g_verify_loop_check_line();
  let books = property_get(state, "books");
  let base = firebase_prod_asset_url("bible.html");
  let urls = books.map(function (book) {
