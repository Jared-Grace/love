...
export async function commons_picture_download(title, width, path) {
  ...
  "$plain width";
  "$plain path";
  "Fetches a scaled copy of one named Wikimedia Commons picture and puts it on disk at a named path, answering the path, or null when Commons will not say where the copy is.";
  ...
  "It answers null rather than throwing on a title Commons does not hold, because a walk over many scenes tries several candidate paintings per scene and a miss is an ordinary outcome rather than a fault.";
  ...
  await http_buffer_file_overwrite(url, path);
  return path;
}
