...
export function app_index_emojis() {
  ...
  "The page listing every app was left out for a while on the reasoning that these pictures are drawn ON it, so it has nothing there to be told apart from. That was answered by where else its name turns up: it is a card on the index, a link in the foot of nearly every app, and a line on its own page among the rest. Everywhere but one, it is one name among many, so it gets a picture like the others - a phone, which is what the other apps are on - the same picture the way back to them wears in the foot of every app.";
