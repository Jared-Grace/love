...
export function bless_block(x, y) {
  ...
  ("for.");
  ("The door is the pavement tile in front of a building's middle, and it is the whole of");
  ("what a building means to somebody living in it. Nobody goes inside - there is no inside");
  ("- so a home here is a doorstep, which is where a person stands when they are outside");
  ("their own house rather than out walking.");
  ...
  function building_tiles(index) {
    let at = building_x(index);
    let tiles = bless_tiles_rectangle(at, y, width, depth);
    return tiles;
  }
