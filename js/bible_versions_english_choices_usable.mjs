...
export async function bible_versions_english_choices_usable() {
  ...
  ("THE SWORD SHELF IS BUILT AND NOT YET JOINED ON HERE, and the reason is a rule this repo already gates: a translation is offered only once its words can be read. Its verses went up one file to a verse, which is the road the reading-aloud pages take; the road a comparison takes asks for a whole chapter in one file, and that upload has not been run. Joined on before it has, the sword translations are on the list and read as nothing, which is exactly the silent shortfall the sources gate exists to catch - and it caught it.");
