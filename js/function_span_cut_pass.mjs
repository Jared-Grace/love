...
export async function function_span_cut_pass(f_name) {
  ...
  ("A run that is stepped over is remembered by the word it ends on and never offered again, because a walk that re-asked the list would be handed the same unusable run for ever. The reasons asked beforehand are all about the name, and a name does not become spellable by being asked twice.");
