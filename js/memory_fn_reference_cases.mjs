export function memory_fn_reference_cases() {
  ...
  "The cases that answer with nothing carry as much weight as the ones that answer with a name. A reader that collected every snake case word would pass a corpus of pointers alone, and the shapes it would wrongly collect are all here: a name told as history, a call whose last two letters spell the marker's, and a marker whose letters are not the ones a name is made of.";
  "The call-shaped case is kept from the form this replaced. A marker cannot look like a call any more, which is the reason for the change, so the case now proves that rather than guarding it.";
  ...
  let cases = [
    ...
    },
    {
      ...
      tokens: [],
      why: "the shape the old bracket form sat one character away from, kept so the reason for moving off it stays visible: a real call ending in those two letters can no longer be confused for a marker at all",
    },
    {
