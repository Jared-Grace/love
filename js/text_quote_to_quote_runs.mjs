...
export function text_quote_to_quote_runs(t) {
  ...
  ("This is not a reading of the strings a file holds. A mark inside a comment, inside a written pattern, or standing in the middle of a longer sentence opens a stretch here like any other, and that is wanted: what this answers is whether some exact quoted spelling is written anywhere in a file, which is a question about the writing and not about the code. The neighbour that does read strings properly is ",
    fn_name("text_quote_end_index"),
    ", which minds escapes and which kind of mark opened the run.");
  ...
  while (greater_than_equal(opened, 0)) {
    let index_from = add(opened, 1);
    let closed = text_index_of_from_try(t, '"', index_from);
    if (less_than(closed, 0)) {
      return runs;
    }
    let sum = add(closed, 1);
    let run = t.slice(opened, sum);
    list_add(runs, run);
    opened = closed;
  }
  return runs;
