...
`The reading order is just the groups flattened — ${fn_name("examples_groups")} is the one source`;
"(so order and grouping can never drift). Files not listed there are appended at the";
`end by ${fn_name("examples_corpus_read")} (a new example still shows up).`;
