...
export function app_shared_path_dev_served(app_name) {
  "the path an app's dev page is FETCHED at, which is its built path with the public folder taken off the front - because the server's root IS that folder, so a page built at web/public/dev/code.html is asked for as /dev/code.html";
