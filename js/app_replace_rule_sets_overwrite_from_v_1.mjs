...
export async function app_replace_rule_sets_overwrite_from_v_1() {
  ...
  "It reads a list that stopped being added to long ago, so running it now would take the app from seventy-two exercises down to the forty-one it had then, and the thirty-one lost would go quietly - a list is still a list when it is short. The word generate invited exactly that, being the obvious thing to reach for on hearing the list had drifted, which is why the name says overwrite and where from.";
  error(
