...
export async function lyric_video_picture_scene_alternative_missing() {
  ...
  "★ THE FIRST RUN CAUGHT A POSSESSIVE NAMING AN OWNER WHO WAS NOT IN THE PICTURE - a shepherd's wooden staff, lying alone on a step with a crown. The staff was there and the shepherd never was. It now reads a long curved wooden staff, which is what it always showed. Worth knowing before writing a scene: the check reads the sentence, and a sentence can name a person it does not draw.";
  ...
  for (let held of read.pictures) {
    ...
    let said = lyric_video_picture_scene_words_said(
      picture.scene,
      words_people,
    );
