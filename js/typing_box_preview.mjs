...
export function typing_box_preview() {
  ...
  "That second half used to be answerable in one move. web/public/dev/typing.html held the same box with none of our running code around it, so the two read together divided the fault: failing in both meant the phone, failing only here meant something this page has and that one had not - the shell, the boot, the viewport. That case is not hypothetical. A shell measured in the height of the window reflows the moment the keyboard opens, which is the moment somebody is typing, and what they type reaches nothing.";
