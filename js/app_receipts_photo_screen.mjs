...
export function app_receipts_photo_screen(root, folder_code, on_change) {
  ...
  html_clear(root);
  ("The folder is a button so the way to change it is to press the thing that names it; on_change is handed the current code.");
  function on_folder() {
    on_change(folder_code);
  }
  app_shared_button_wide(root, "📁 " + folder_code + " ✏️", on_folder);
