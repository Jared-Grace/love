...
export async function file_builtin_calls_repair(f_path) {
  "Say the covered built-in calls in one file with the names the repo keeps for them, and bring those names in.";
  "One file rather than the whole repo, because the pass this belongs to normalizes twenty-four other things at the same time and a sweep of that over every file that reaches a built-in is a diff nobody can read and a collision with whoever is mid-edit. What moved here is only ever a covered built-in call.";
