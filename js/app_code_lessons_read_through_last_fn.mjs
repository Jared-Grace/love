...
export function app_code_lessons_read_through_last_fn() {
  ...
  "A learner meeting a lesson nobody has read through has no way to tell it from one that has been, so a run that stops at a known-good place is worth more to them than a longer one they have to judge for themselves. Every lesson after this one is still built, still opens on the working copies, and costs nothing to release - moving this name down the list is the whole of releasing them.";
  "This one, because the lessons around here come in twos: one that asks which of the two operators in a line goes first, and then one that solves a line of that shape. This is the second of such a pair, and the lesson after it opens the next pair. A cut between the asking and the doing would leave a learner having just been asked a question the track never answers.";
  "It is also the first place parentheses are put around a comparison joined by and or or, rather than around arithmetic. So the run now ends where the two halves a learner has been taught separately - working a sum out, and reading a true-or-false line - are finally written in one line together.";
