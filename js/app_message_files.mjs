...
export async function app_message_files() {
  ...
  "what people have written, and only that: the error reports and the daily opens land under the same opening because it is the only place a browser may write at all, so the folders they sit in are skipped here rather than read as somebody's message";
  ...
  function written_by_a_person(item) {
    ...
    let outside = outside_errors && outside_opens;
    return outside;
  }
