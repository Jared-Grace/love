...
import { app_en_learn_bible_gloss_urdu_verse_claims_wrong_baseline_growth_assert } from "./app_en_learn_bible_gloss_urdu_verse_claims_wrong_baseline_growth_assert.mjs";
...
export async function app_en_learn_bible_gloss_urdu_verse_claims_wrong_baseline_write() {
  "Rewrite the record of the Urdu explanations naming a verse that holds nothing related, from what the store holds right now. For seeding it once, and for banking each batch of settled sentences - never for widening it, which is the one thing the gate exists to refuse.";
  ...
  assert_json(stored, {
    hint: "the Urdu gloss store is not on the disk, so there is nothing to write the record from - mount the drive the store lives on and run this again",
    stored,
  });
  let known = await app_en_learn_bible_gloss_urdu_verse_claims_wrong_names();
  await app_en_learn_bible_gloss_urdu_verse_claims_wrong_baseline_growth_assert(
    known,
  );
