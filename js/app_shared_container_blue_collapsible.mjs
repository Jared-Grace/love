...
export function app_shared_container_blue_collapsible(
  ...
  ("THE ARROW IS ITS OWN SPAN, WHICH IS WHAT LETS IT BE COLOURED APART FROM THE TITLE. Arrow and title were one run of text, so the arrow could only ever wear whatever the words wore, and it read as the first character of the title rather than as a control - a reader looking for what to press had nothing to look for. Given a colour of its own it is the one blue thing on the line and the eye finds it without being told. Splitting it also makes the change cheaper than it was: only the arrow is rewritten when the card folds, where before the title was thrown away and written out again every time.");
  let caret_mark = html_span_text(title_button, caret_open);
  ...
  function collapsed_set(value) {
    ...
    html_text_set(caret_mark, caret);
