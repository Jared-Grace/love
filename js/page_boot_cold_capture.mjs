...
export async function page_boot_cold_capture(url) {
  ...
  ("the listener has to be hung BEFORE the address is opened, and ",
    fn_name("playwright_test_url"),
    " has already opened it by the time it hands the page over - so the page is sent to a blank one and then to the address again, and everything the opening throws is heard. going away to blank first is also what makes the page be built afresh rather than reused, the same thing the game's phone sweep does between screens.");
  let errors = [];
  let body_text = "";
