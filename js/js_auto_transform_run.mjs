...
export async function js_auto_transform_run(t, ast) {
  "Run one step of the normalize pipeline and, if it gives up, say which step it was. The pipeline runs every step in its list over the same tree, so a bare failure message names a predicate that was violated somewhere without naming the step that violated it, and finding the step meant bisecting the list by hand every time. How many steps there are is deliberately not written here: the list next door is the one place that knows, and a count copied into a sentence is wrong from the next step added onward with nothing to say so. This one said twenty-three while the list held twenty-five.";
