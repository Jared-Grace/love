...
export async function app_music_song_pictures_fetch(urls, status) {
  ...
  "THE COUNT MOVES WHILE IT RUNS, because a long fetch that says nothing reads as a button that did nothing, and the reader's next move is to press it again.";
  "WHETHER IT SHOULD RUN AT ALL IS SOMEBODY ELSE'S QUESTION. This one fetches what it is handed, every time it is called; the once-only answer is claimed before the call, where the buttons can see it in time to switch themselves off.";
  ...
  await list_map_unordered_async(urls, fetch_one);
