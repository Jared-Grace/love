...
export function bible_dream_hand_nearness(gap_squared, tolerance_squared) {
  ...
  ("It is a ratio of the two rather than a subtraction, so it needs no clamping and cannot go negative however far the hand wanders. Both are squared, because the gap arrives squared from ",
    fn_name("bible_dream_point_gap_squared"),
    " and taking a root here would only be undone by the caller.");
  let divided = divide(tolerance_squared, tolerance_squared + gap_squared);
  return divided;
}
