...
export function bless_building_family_tiles(building, index) {
  ...
  ("A downstairs family in a two-storey house owns ONE ROW and that is correct, not a bug to");
  ("be widened away. The street is drawn from the front and from above at once, so the");
  ("ceiling of the ground floor is the floor of the one above it and is hidden underneath it.");
  ("The only surface a downstairs family shows is the band of wall its door is in. The");
  ("family above gets the roof for the matching reason: nothing is drawn over a roof.");
  ("A one-row share that reads as a stripe rather than as a floor is a question for whoever");
  ("draws it. Giving the family a row of yard to stand on was tried and reverted - it made a");
  ("prayer light ground outside the house to make the mark thicker, which is a picture");
  ("saying something untrue in order to look better.");
