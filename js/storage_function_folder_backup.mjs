...
export async function storage_function_folder_backup(f_name) {
  ...
  "It copies with the spelling that only ever writes, never the one that clears the place out first. Nothing here would notice the difference, because the moment it is stamped with has never been written to before - but a command whose argument reaches a deletion cannot be approved in advance by anybody, and being approvable in advance is the entire reason this exists rather than a line of copying typed by hand.";
  ...
  let skipped = [];
  await folder_copy_over(source, backup, skipped);
