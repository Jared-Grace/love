...
export function app_code_line_swap_mirrored_is(code) {
  ...
  ("WHAT THIS TAKES FOR GRANTED, AND WHERE THAT IS NOT YET TRUE. The whole of this rests on a learner knowing the rule, and a learner is taught it in the swapping lessons. Four lessons standing before those - the ones about a line having two sides, and about a pair of comparisons - hand out a turned-round line among their others, and a learner meeting one there has no rule to answer it with and really does work both sides out. Priced by this, those lines come out at one step when they cost three.");
  ("It changes no reading today, and only for a reason that is luck. Each of those four lessons also hands out lines that are not turned round and that cost three, and a lesson is priced by the hardest line it can reach, so the wrong low price is covered by a right higher one. That is a fact about the questions those lessons happen to make up rather than anything promised here. Should one of them come to hand out turned-round lines alone, it would quietly read as the easiest lesson in that part of the course, and nothing would say so. The honest repair at that point is to ask this only of lessons standing at or after the first swapping lesson, which means handing a place in as well as a line, and so is not free.");
