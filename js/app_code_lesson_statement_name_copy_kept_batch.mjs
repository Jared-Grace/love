...
export function app_code_lesson_statement_name_copy_kept_batch() {
  ...
  ("Whether it is offered is not settled here. A question's wrong answers are drawn from batches of their own, so the word this program put in second may or may not be among them. What is settled here is that it is never the right one, which is the only thing this screen has to be sure of.");
