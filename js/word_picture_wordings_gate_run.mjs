export function word_picture_wordings_gate_run() {
  ...
  list_empty_is_assert_json(untaught, {
    hint: "a drawing wording is written for a word the game does not teach, so nothing will ever tap it; add it to words_game_taught_glosses or drop the wording",
  });
  list_empty_is_assert_json(claimed, {
    hint: "a drawing wording is written for a word whose only picture would be a claim about God; word_picture_wordings says why these four are never drawn",
  });
  let r = { words: list_size(words) };
  return r;
}
