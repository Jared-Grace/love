...
export async function gloss_explains_repeated_measure() {
  "How much of each gloss store hands one explanation to more than one word, given as shares out of every thousand explanations - one for a wording worn by genuinely different words, one for a wording handed to the same word met again, and one for the two of them together - beside the plain counts they were worked out from, and which stores could not be read at all.";
  ...
  "The first two are kept apart because only one of them is a fault. A wording worn by different words teaches the reader nothing about the word in front of them, and every one of them has to be written again. The same word told the same thing twice in a chapter is a different question altogether and may well be the right way to write it, so a single number covering both would go red at good writing and would hide the bad writing behind it.";
  "The third exists because the first two trade against each other. Repairing a class label worn by five different words gives each of those words its own sentence, and that sentence is then worn by every occurrence of its own word - so the fault falls and the harmless number rises by very nearly the same amount. Watching the harmless one alone would refuse the repair. Watching the two added together refuses the one thing that is left to fear, which is somebody writing a single sentence over more and more of the store, whichever of the two kinds it lands in.";
