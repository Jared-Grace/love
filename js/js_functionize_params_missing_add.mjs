...
export async function js_functionize_params_missing_add(
  ...
  ("One of those words is not provided at all once the function being cut binds a name of its own to it. Inside these lines the word means the local binding and the language's own cannot be reached, so a run moved out that reads it is handed nothing and quietly reads the language's instead - and nothing goes red, because the word is bound, so no name is left over and no import is missing. Measured 2026-09-01 on the lifting twin: a list of the moves a player had made was kept under the word a page uses for the pages it has visited, and the moved lines went on adding to the page's own.");
  ("Handing it over is right whichever binding the word turns out to mean, because the line left where the run used to stand reads it there, and so passes on exactly what the run itself would have found.");
  let shadowed = js_globals_shadowed_names(ast);
  let provided_free = list_difference(provided, shadowed);
  missing = list_difference(missing, provided_free);
