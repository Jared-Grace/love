...
export function app_shared_game_player_npc_swap_if(g, from, to) {
  ...
  "It is nearly always a STRANGER who is traded with: the way through is worked out with the player's own line left standing. The exception is a line with no trail left to back into, which would otherwise wall its own leader in for good.";
  ...
  "The trade is a SHOVE and not a walk. The person did not choose to go, and the tile they are handed is behind the player, so the trade turns them nearly right round; coming hard after the turn a crowd opening had already given them, that is a person spinning. So they keep their facing unless they have rested, exactly as somebody shuffled aside does.";
  ...
  app_shared_game_npc_shove(npc, tile, 0);
  return true;
}
