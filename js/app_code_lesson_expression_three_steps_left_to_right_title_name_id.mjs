...
export function app_code_lesson_expression_three_steps_left_to_right_title_name_id() {
  ...
  ("In one go is what tells it from the lesson it is the twin of, which walks the very same lines a press at a time and is titled Solve three steps left to right. Two lessons on one home list showing one line each and named alike would be one line read twice, with nothing on the screen to say which of them a learner had already done.");
  ...
  function paint(parent) {
    html_cycle_code(parent, ["Three steps in one go, left to right"]);
  }
