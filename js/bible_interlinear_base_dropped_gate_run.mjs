...
export async function bible_interlinear_base_dropped_gate_run() {
  ...
  assert(below_ceiling, {
    hint: "far more words were dropped than the edition marks account for, so the filter is reading a span wrongly and deleting scripture - read bible_interlinear_base_dropped_report and look for one verse losing a whole clause",
    words_dropped,
    most,
  });
  let r = {
