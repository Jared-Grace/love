export function bible_glyph_artwork_source() {
  ...
  "THE SET ALSO ANSWERS WHAT IT HOLDS, which is why the listing address is kept here beside the fetching one. Every name in the bridge table is a guess about what this set calls a picture, and a guess that can be checked against the set's own list stops being a guess. The two addresses are different services on the same repository - one hands over a file, the other hands over the names of the folders - so they are recorded together rather than one of them being worked out from the other.";
