...
export async function functions_locals_unread_repair_some(how_many) {
  ...
  "Everything about how a few are taken, committed one at a time and counted afterwards is shared with the operators family next door, which is where to read why each of those steps is the way it is";
  let r = await functions_repair_some_generic(
    functions_locals_unread,
    "name",
    function_declarations_unused_clear,
    functions_locals_unread_baseline_write,
    how_many,
  );
  return r;
}
