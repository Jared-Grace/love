import { app_original_bible_gloss_explains_write_file } from "./app_original_bible_gloss_explains_write_file.mjs";
import { app_original_bible_gloss_generate } from "./app_original_bible_gloss_generate.mjs";
import { each_async } from "./each_async.mjs";
import { gloss_explains_write_files_verse_keys } from "./gloss_explains_write_files_verse_keys.mjs";
import { list_add } from "./list_add.mjs";
export async function app_original_bible_gloss_explains_write_chapter(
  chapter_code,
) {
  "Write into one chapter of the original-language gloss every piece of new wording waiting for it, a passage at a time, and answer with the passages mended.";
  "$plain chapter_code";
  ...
  let verse_keys = await gloss_explains_write_files_verse_keys(
    chapter_code,
    fn,
  );
  let mended = [];
  async function verse_key_write(verse_key) {
    await app_original_bible_gloss_explains_write_file(chapter_code, verse_key);
