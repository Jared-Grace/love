...
export async function functions_imports_missing_scoped() {
  ...
  "The names to look for are gathered into a set once, before the walk, and both readings cross against that one set. Crossing against the list instead searched all nine thousand names afresh for every name in every file, and the second reading then gathered its own set per file on top of that - the same names, over and over, for seven thousand files running.";
