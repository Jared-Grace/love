export function song_image_review_couplets() {
  ...
  "A COUPLET TAKEN OFF THIS LIST STOPS HAVING ITS NOTES MARKED DONE, WHICH IS HOW FIVE OF THEM WENT UNANSWERED. The command that closes a round walks this list and marks every note on every couplet in it; a couplet lifted off before its last note was answered keeps that note open for ever and nothing ever looks at it again. Couplets 9, 14, 25, 32 and 33 were carrying eighteen unanswered notes between them when this was found, and 14's whole file had never been touched. Before lifting a couplet off, read its note file to the end.";
  ...
  "A NEW CLAUSE CAN CROWD OUT AN OLD ONE, AND THE DRAWING IS WHERE THAT SHOWS - now confirmed rather than suspected. Couplet 11 was given a clause making its three field sheets single unbroken panes, and every drawing for five rounds came back with the field right and the thicket, the crown of thorns and the halo all gone. The clause was cut and the very next batch brought all three back together in one picture. Nothing in the file went red at any point; the two clauses simply compete for the same attention, so when a picture starts losing a thing it used to have, suspect the clause that was added last rather than the drawing.";
  "AND A CLAUSE THAT ONLY REPEATS THE SHARED SHEET CROWDS JUST AS HARD AS AN INVENTED ONE. Couplet 15 was asked to keep the lead lines off the sword; the shared sheet already says the field's lead lines stop where they meet the symbol, so the couplet was given its own copy of that rule. Three drawings followed, all three kept the lines off the sword and all three lost the flame from the blade altogether. The copy was taken back out and the wording returned to what it had been. A rule already in force elsewhere buys nothing when it is repeated and costs a place in the list exactly like a new one.";
