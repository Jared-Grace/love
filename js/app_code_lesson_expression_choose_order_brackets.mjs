...
export function app_code_lesson_expression_choose_order_brackets() {
  ...
  ("brackets round the || of a line that also holds an && , taken a press at a time: false && (true || true), choose the || , choose what it comes to, then choose the && and what that comes to");
  ...
  ("Nothing has to refuse the wrong press. The && cannot go first because its right side is not a value yet - it is the || , still waiting - so the rule is read off the line rather than enforced against the learner, which is the same reading every lesson in this run has asked for.");
