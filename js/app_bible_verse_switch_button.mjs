...
export function app_bible_verse_switch_button(
  ...
  ("chapter_code is not read here and stays all the same - this function is handed to ",
    fn_name("app_shared_bible_read"),
    " as a value, so the four places belong to that caller and dropping one would slide the verse number into the slot before it");
