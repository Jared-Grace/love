...
export function gloss_chapters_claims_wrong_read(chapters) {
  ...
  "★ THE WORD THE CLAIM WAS ABOUT SITS BESIDE THE NAME, AND IT IS THE FIRST THING TO READ ON THE ROW. The name carries the word the entry explains, which is not always the word the sentence claimed a verse for: an entry explaining Jesus may say that Christ stood in verse two, and then the name says Jesus, the verses beside it are Christ's verses, and a reader with only those two would think the row contradicted itself. Where the claim was about the entry's own word the two read the same, and that sameness is itself the fastest thing on the row to check.";
  ...
  function chapter_read(chapter) {
    ...
    function finding_read(finding) {
      ...
      let claimed_word = property_get(finding, "claimed_word");
