...
export async function claude_sessions_restart(minutes) {
  ...
  await command_line_code_ignore(
    text_combine_multiple(["tmux kill-session -t ", session]),
  );
  let restored = await claude_sessions_restore(minutes);
  await command_line_interactive(
    text_combine_multiple(["tmux attach -t ", session]),
  );
  return restored;
  ...
  async function claude_session_refill(session, pane, minutes) {
    ...
    await command_line_code_ignore(
      text_combine_multiple(["tmux kill-window -a -t ", pane]),
    );
