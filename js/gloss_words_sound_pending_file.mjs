...
export async function gloss_words_sound_pending_file(pending, folder, voice) {
  ...
  "The voice's folder is made here, once, before any of the filing starts. The filing runs every note at the same time, so a folder made down there would be several hundred pieces of work all asking for the same folder at the same moment.";
  arguments_assert(arguments, 3);
  let spoken = gloss_word_sound_voice_folder(folder, voice);
  await folder_exists_ensure(spoken);
