...
export function lyric_timing_screen_buttons_tapping(parent, desk) {
  ...
  ("The first button stays even though choosing a chapter now loads it by itself, because the passage the screen arrives already naming was never chosen and so has never been loaded, and because a passage whose document has been written since it was loaded is fetched again from here.");
