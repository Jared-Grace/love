...
export function app_code_lesson_expression_choose_order_both_sides_expression(
  ...
  ("A true line is refused while the two sides come out written the same way. Written the same, the line answers itself: a learner who has seen that both sides say 3 + 4 knows the answer without solving either, and the lesson has asked them nothing.");
  ...
  let left_node = app_code_expression_node(
    left_parts.left,
    left_parts.symbol,
    left_parts.right,
  );
  let left_code = app_code_expression_code(left_node);
