...
export function app_code_division_formula_recall_words(what, tree_of) {
  ...
  ("The worked numbers are the same two on all four screens, so a learner following the run meets one example rather than four, and the formula they were shown is the formula they are asked for.");
  let dividend = 14;
  let divisor = 4;
