...
export function bless_index_household(index) {
  ...
  ("A building always holds nine, so the building is a division; inside those nine its two,");
  ("three or four families are two to five people each, so which of them is a walk. At most");
  ("four steps, whether the person is on the first street or a billion streets away.");
  ...
  ("the same question is what stops this needing to know how many of them there are.");
  ("It walks the full numbering STRIDE - four steps - and not the doors this building really");
  ("has. A building with two of them runs out of families after the second boundary, and");
  ("every step past that asks about a boundary that is the end of the building again and is");
  ("answered no again, so the count is right without this having to look the doors up.");
