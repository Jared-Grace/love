...
export function g_coordinates_walk_distances(g, starts) {
  "how many STEPS of walkable land lie between the NEAREST of `starts` and every tile reachable from them — one breadth-first sweep outward, answered as a map from g_coordinates_key to that number. a tile that is water, an npc's own tile, or cut off behind them is simply absent from the map, so a lookup that finds nothing means unreachable. one sweep answers for the whole map, which is what a shortest-path-per-candidate loop cannot do: choosing the best tile out of a windowful asks the same question of every one of them, and asking it once from the other end costs a single sweep.";
