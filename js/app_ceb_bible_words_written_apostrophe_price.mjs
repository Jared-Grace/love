...
import { text_words_dash_apostrophe_kept } from "./text_words_dash_apostrophe_kept.mjs";
...
export async function app_ceb_bible_words_written_apostrophe_price() {
  ...
  "★ THE READER CEBUANO NEEDS ALREADY EXISTS IN THIS REPO AND NOTHING ASKS FOR IT. Cebuano keeps a dash inside a word and an apostrophe inside a word, and the reader chosen for it today keeps only the first of those; the one that keeps both was written for something else entirely and fits this exactly. So the question here is not how to build a fix. It is what the fix would cost, which is the one thing nobody had counted.";
  ...
  function verse_read(verse) {
    ...
    let both_words = text_words_dash_apostrophe_kept(text);
