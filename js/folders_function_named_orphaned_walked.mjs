...
export async function folders_function_named_orphaned_walked() {
  arguments_assert(arguments, 0);
  ("Every folder of stored data named after a function that no function answers to any more, and how many folders were looked at to say so.");
  ...
  ("How many folders were looked at travels out beside them, because the two runs this cannot otherwise tell apart are a clean disk and a disk that was never reached. Both report nothing left behind. The count is what falls to nothing on the second one, and the gate above says so out loud rather than reading a quiet pass.");
