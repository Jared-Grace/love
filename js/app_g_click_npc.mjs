...
export async function app_g_click_npc(div_map, npcs_matched, player_img_c) {
  ...
  let npc_img = app_shared_game_npc_img_get(npc);
  let direction_npc = g_direction(npc, player);
  await app_shared_game_character_turn(npc, npc_img, direction_npc);
  let direction_player = g_direction(player, npc);
  await app_shared_game_character_turn(player, player_img_c, direction_player);
