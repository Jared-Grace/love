...
export function bless_building_shape() {
  ...
  ("The depth is THREE, and it is the same three whether a building has one floor or two. What varies is how much of it is wall and how much is roof: a two-storey house shows two bands of wall with a single strip of roof behind them, and a one-storey house shows one band of wall with a broad roof behind it. Seen from above that is the whole of what makes one house look taller than the next.");
  ("It was FOUR for a few hours, to give a downstairs family a row of yard in front of its door so that its share of the street was not one square deep. That was a wrong answer to a real question. The view is front-and-top: the ceiling of the first floor is the floor of the second and is covered by it, so the only surface a downstairs family shows IS the single row its door is in. One row is the truth about a house seen this way, and a share that reads badly is a drawing problem rather than a geometry one.");
