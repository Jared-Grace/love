...
export async function binisaya_words_gather(words) {
  ...
  "Which words are still to be asked about, and the lowering of them that has to happen before anything is looked for, are the shared question, asked in the one place so that a reading of what the next run will cost cannot drift from what the run actually does.";
  let asked = await binisaya_words_unasked(words);
  let known = property_get(asked, "known");
