...
export async function daemon_ensure(f_name) {
  ...
  let text = daemon_unit_text(f_name);
  let current = await daemon_unit_text_current(f_name);
  let changed = equal_not(current, text);
  let unit = daemon_unit_name(f_name);
  if (changed) {
    await file_overwrite_uncached(daemon_unit_path(f_name), text);
