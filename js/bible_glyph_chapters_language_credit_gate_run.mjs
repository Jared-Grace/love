...
export async function bible_glyph_chapters_language_credit_gate_run() {
  ...
  "IT REFUSES TO PASS ON AN EMPTY SET, which is how a gate like this dies. Finding the languages is a search for a sentence, so a reworded sentence answers with nothing rather than with an error, and a gate that only checked what it found would go green the moment it lost the ability to find anything and would stay green.";
  ...
  ("THE SENTENCE THIS IS FOUND BY IS NAMED INSIDE THE HINT AND NEVER BESIDE IT, because a failed gate's words are read back afterwards for function names and every name found is taken as an accusation.");
