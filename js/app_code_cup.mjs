...
import { app_code_note_name_tag } from "./app_code_note_name_tag.mjs";
import { not } from "./not.mjs";
import { html_font_color_set } from "./html_font_color_set.mjs";
import { html_bold } from "./html_bold.mjs";
import { app_shared_spaced_small_gap } from "./app_shared_spaced_small_gap.mjs";
export function app_code_cup(parent, inside, name, names) {
  ...
  ("The box is drawn whether anything has been put in the cup or not, and stands empty when nothing has. The name is written under it, so a box that came and went would carry the name up and down the glass with it - and the story this picture tells is one cup being filled, where the label stays exactly where somebody wrote it. Its height is set rather than left to its contents, so an empty one takes the same room as a full one.");
  ...
  ("★ THE LETTER IS WRITTEN IN THE NAME'S OWN COLOUR, RINGED IN THE BLACK THE CODE CHIP IS FILLED WITH. So the cup and the code box show the same two colours doing the same two jobs - a coloured letter read against black - and the only thing that differs is what the black is cut to. There is no colour invented here at all: the name's colour is handed in, and the black is the chip's own background asked for by name, so neither can drift from its twin further down the screen.");
  ("Something dark has to touch the letter because this card is pale. A colour bright enough to be read as ink on a black chip is not readable on this card at all - the green measures about 1.2 to 1 against it, against a floor of 4.5 - so the letter needs a ground of its own, and taking the chip's ground is what makes the cup a second drawing of the code rather than a second arrangement of the colour.");
  ("★ THE BLACK FOLLOWS THE LETTER RATHER THAN FILLING A BOX BEHIND IT, AND THAT IS ABOUT THE PICTURE AND NOT ABOUT READABILITY. The two measure the same, because what helps a reader is the colour touching the letter and not the shape the rest of it is cut to. A filled box inside a drawn cup reads as something that was put in the cup - it was narrowed twice trying to stop it reading that way, which is how it became clear that the shape was wrong and not the size.");
  ("The reading rejected last: ring the letter in the name's own colour scaled down dark, so the cup carries one hue and nothing else. It reads - 4.9 to 1 for the letter against its ring, where the black gives 7.1 - and it was preferred for a while because it looked like the stricter reading of one colour. That was the mistake. The black was never a rival colour to the name; it is the colour the name is already read against in every run of code on the page, and scaling the hue down made the cup its own arrangement again.");
  ("The reading rejected here: putting the colour behind the letter as a bright patch and writing the letter dark, which is what this did before and what the code chip still does. It is legible and it is one colour, and it costs the correspondence - the cup then shows a green block where the program shows a green letter, and a reader comparing them across the screen is comparing two different arrangements of one colour rather than the same mark twice.");
  ("The reading rejected beside that: a plain outline round the letter with the card showing through and the letter left bright. Nothing darker then touches the letter, so the measurement is the one that started all of this - bright green on this card is 1.2 to 1. An outline held a little way off the letter cannot make the letter readable; only what is against it can.");
  ("The reading rejected before that: remaking the colour darker by hand for this card. That is a second colour however carefully it is chosen, and it drifted - a reader looking at the cup and the code said the green here was not the green down there, and they were right. Asking for a colour that already exists elsewhere, rather than making one, is what stops that happening again.");
  ...
  if (named) {
    ...
    if (lent) {
      app_code_note_name_tag(written, lent);
    }
    if (not(lent)) {
