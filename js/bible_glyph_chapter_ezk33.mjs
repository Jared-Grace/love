export function bible_glyph_chapter_ezk33() {
  ...
  "TWO SENTENCES HERE WERE DRAWN WITH A NEGATION TOO MANY, AND THE CHAPTER NEXT DOOR IS WHAT PROVED IT. The thirteenth verse said that none of his righteous works will not be remembered, and the sixteenth that none of the sins he has sinned will not be held against him. In English the extra word would be a stumble a reader steps over; drawn mark for word it is arithmetic, and both verses came out saying the opposite of what Ezekiel says. Nothing could have gone red over either one, for the same reason nothing went red over the doubled prohibition in John ten: a gate that ratchets against a baseline can only see a pattern recur, never a pattern appear.";
  "WHAT FOUND THEM IS THAT THIS CHAPTER HAS A TWIN. The eighteenth chapter says both sentences again in almost the same words, and it draws both correctly - the righteous acts he did will not be remembered, none of the transgressions he committed will be held against him. So the two readings could be set beside each other and only one of them could be right. A twin passage is the cheapest check this Bible has: it needs no gate, no baseline and no interlinear, only the two pages open at once. Where a book repeats itself, draw the repetitions together and read them together.";
  "AND THE BEASTS OF THE FIELD IN THE TWENTY SEVENTH VERSE WERE DRAWN AS LIFE. Chayyah, a living thing, is the same root as the chayah this chapter alternates with muth twenty times, so the seating followed the root and gave a verse in which men are handed over to the life of the field to be devoured. The root is not the sense. Where one root carries two senses and only one of them is seated, the other is left in English, which costs a reader nothing and is the one move that can never be wrong.";
  let chapter = {
    chapter_code: "EZK33",
