...
export async function qa_gates_repo_lookup_reaching() {
  "The gates that can reach the spelling of a folder as the public folder joined onto whichever repo this machine is currently pointed at, however many files away.";
  "Not merely the lookup underneath it. Measured 2026-08-26: eighteen gates could reach that lookup and not one of them reached it this way - every one of them reached it to find out which project to ask over the wire, which is a real question with a real answer and no folder in it. Held to the wider target the check refused honest work, and the first new gate it ever met was one of those. So the target is the join, which is the thing that was actually going wrong.";
