...
export function app_next_copy_button(parent, reading_text) {
  ...
  "So the same copying is offered again as something to press. Under a thumb no browser refuses it, and it copies exactly what the page copied for itself - the same words that are on the screen, the reading and the link on from it together.";
  "It stands inside the card holding the reading rather than among the ways onward, because it is not a way onward: it hands over what is already here. Standing last inside it also says where the copying stops - everything above this button is taken, and everything below it is the page rather than the message.";
