...
export async function app_g_dev_if(div_map) {
  "dev-only: if the URL hash names a route in app_g_dev_routes, run it; #index shows the clickable directory of all routes; anything else (incl. no hash) is a no-op so normal play is untouched. div_map is threaded through so a route may draw on the map (e.g. #day_unbelievers markers)";
