...
import { usfm_marker_rest } from "./usfm_marker_rest.mjs";
import { property_get } from "./property_get.mjs";
import { usfm_marker_layout } from "./usfm_marker_layout.mjs";
import { not } from "./not.mjs";
import { usfm_verse_numbers_inlined } from "./usfm_verse_numbers_inlined.mjs";
...
export function usfm_lines_laid_out(usfm_lines, verse_numbers_shown) {
  arguments_assert(arguments, 2);
  ("$plain usfm_lines");
  ("$plain verse_numbers_shown");
  ("Lines of usfm turned into lines of plain writing - the notes gone, the marks gone, each line stepped in as far as its mark asks, and a blank line wherever the printing breaks the passage.");
  ...
  ("A line whose words all turn out to be a footnote leaves nothing behind rather than an empty line. That happens wherever the printing hangs a note on a heading, and a reader would see a hole in the poem and take it for a fault.");
  let out = [];
  ...
  for (let usfm_line of usfm_lines) {
    ...
    let split = usfm_marker_rest(unreferenced);
    let marker = property_get(split, "marker");
    let rest = property_get(split, "rest");
    let layout = usfm_marker_layout(marker);
    ...
    if (laid_out) {
      let numbered = usfm_verse_numbers_inlined(rest, verse_numbers_shown);
      ...
      if (not(silent)) {
        let heading = equal(kind, "heading");
        let paragraph = equal(kind, "paragraph");
        let parted = or(heading, paragraph);
        if (parted) {
          blank_line_add();
        }
