...
export async function bible_glyph_chapters_bands_missing_repair() {
  ...
  "IT FINDS ITS OWN SET BECAUSE THE SET IS THE THING THAT KEEPS CHANGING. Picture chapters are authored in bursts - nine landed between one gate run and the next - and each one reds bible_glyph_chapters_rosetta_lines_gate_run and bible_glyph_chapters_language_gate_run until its bands are written. Repairing that by hand means calling the single-chapter writer once per code, which leaves nothing behind and cannot be replayed; asking the two lists what disagrees costs nothing and cannot drift from what is actually broken.";
