...
export async function bible_glyph_chapters_bands_missing_repair() {
  "Finds every picture Bible chapter whose Rosetta bands are not named beside it, writes and registers the ones that are short, then rebuilds every reveal language once.";
  ...
  ("IT REBUILDS EVERY REVEAL LANGUAGE ONCE, AT THE END. The writers take no argument and each rewrites its whole file from the picture chapter list, so running them per chapter would do the same whole-file work N times over and land the same answer. They run after the loop rather than inside it because the last run is the only one whose output survives.");
