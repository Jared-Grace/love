...
export async function app_index_gloss_coverage() {
  ...
  "The two are asked at the same time rather than one after the other. They are the only thing the first screen waits on, so asking them in turn would spend two round trips of a visitor's time to learn what one round trip's worth of waiting already knows. Measured 2026-09-06 from a terminal: one listing alone and both together took the same nine seconds, of which eight is a spread this repo's own node fetching sleeps and a browser does not - the request itself answered in under a second.";
