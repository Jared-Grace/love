...
export function app_code_lesson_expression_both_sides_any_comparison() {
  ...
  ("★ SO A CARD ABOVE IT SAYS THE TELLING IS THE ONE THEY ALREADY READ, AND IT HAS TO NAME WHICH LESSON. Its twin is not the lesson immediately before this one - the lesson on arithmetic on both sides of an === sits between the two - so the previous lesson would be a sentence that is simply false here, and a learner sent back to check would find a screen that says something else. Naming the twin by its own home title is the one wording that points at the screen the words actually came from.");
