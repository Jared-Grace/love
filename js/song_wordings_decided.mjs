...
export async function song_wordings_decided(
  ...
  ("Reads every English translation against the lines a song sings at each of its passages, writes both the whole reading and what it decided into files named after the song, and hands back where it put them and how much fell into each of the four heaps.");
  ...
  ("HOW MANY PASSAGES CAME BACK QUIET IS THE ONE NUMBER THAT SAYS WHETHER THIS WAS WORTH RUNNING. The other three heaps report what was found; that one reports how much of the song was never heard at all, and a song that comes back mostly quiet has been read and has no preferences to record - which is an answer, and a different answer from a song that was never read. Measured 2026-09-02 across both songs on the music page, more than half of every reading taken came to a single shared word, so this is the usual outcome rather than a rare one and hiding it would make every song look settled.");
  ...
  let quiet = property_get(decided, "quiet");
  let filed = {
    path,
    reading_path,
