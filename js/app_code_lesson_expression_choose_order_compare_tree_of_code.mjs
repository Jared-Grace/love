...
export function app_code_lesson_expression_choose_order_compare_tree_of_code(
  ...
  ("Five pieces whichever end the comparison landed on, and the two operators simply swap places between the two shapes - the parenthesis travels with its own operator, so a parenthesis anywhere in the line is what says the comparison is on the right.");
  ...
  let parenthesised_is = text_includes(code, open);
  let comparison_left = not(parenthesised_is);
  let second = list_get(pieces, 1);
  let fourth = list_get(pieces, 3);
  let inner_symbol = ternary(parenthesised_is, fourth, second);
  let outer_symbol = ternary(parenthesised_is, second, fourth);
