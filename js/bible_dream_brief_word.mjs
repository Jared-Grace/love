...
export function bible_dream_brief_word() {
  ...
  "★ THE PICTURES ARE NOT NEW ART - THE REPO ALREADY HAS THAT VOCABULARY. app_emoji_bible renders whole verses as glyphs with a key beneath them, and bible_glyph_referents holds what each glyph refers to. Drawing a fresh set for this step would be a second vocabulary for the same Bible, and the two would disagree within a month. Whoever takes this step should start by reading what those already say and asking what is missing, not by choosing icons.";
  ...
  let brief = {
    ...
    open:
      "whether a picture stands for a word or for a phrase; what a wrong order costs and whether it can be undone; how a long speech is broken up; whether the reading aloud can be heard again on request",
    forbidden:
