...
export async function app_ceb_bible_gloss_root_word_apart_shaped() {
  ...
  async function chapter_read(chapter_code) {
    ...
    function entry_read(entry) {
      ...
      let before = property_get_or_null(apart_seen, key);
      let first = null_is(before);
      if (first) {
        ...
        property_set(apart_seen, key, row);
        list_add(apart_rows, row);
        return;
      }
      let seen = property_get(before, "sightings");
      let after = add(seen, 1);
