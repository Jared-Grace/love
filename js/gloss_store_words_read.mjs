...
export function gloss_store_words_read(fn) {
  ...
  "The Cebuano store is read with the dash kept, because Cebuano spells the catch in the throat with one, inside the word - panan-aw, nag-ingon, kabubut-on. Every other store keeps the reading it is given today, which cuts there. That is deliberate and not a measurement: this is a chooser and not a repair, so adopting it changes one store's answer and no other's, and whoever later measures a remaining store can move it here on that store's own evidence.";
