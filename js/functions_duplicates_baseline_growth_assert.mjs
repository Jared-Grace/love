...
export async function functions_duplicates_baseline_growth_assert(known) {
  ...
  let recorded = await duplicates_baseline_read();
  let change = names_versus_baseline(known, recorded);
  let added = property_get(change, "added");
  list_empty_is_assert_json(added, {
    hint: "these functions share their work with another name now and did not before - collapse the pair onto one name rather than recording it as known",
    added,
  });
}
