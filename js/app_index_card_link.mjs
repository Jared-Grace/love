...
export function app_index_card_link(root, label, text, url) {
  ...
  "The line under the button is handed back, for the one caller that writes it again once a number it was waiting on arrives. Nothing else looks at it.";
  let card = app_index_card_frame(root, text);
  app_shared_button_wide_link(card, label, url);
  let caption = app_index_card_caption(card, text);
  return caption;
}
