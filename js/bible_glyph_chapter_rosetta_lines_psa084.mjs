export function bible_glyph_chapter_rosetta_lines_psa084() {
  "One chapter of the picture Bible as its two known Rosetta bands: each verse in the language it was written in, and the same verse in English.";
  "THIS FILE IS WRITTEN BY A COMMAND AND NOT BY HAND. It is the interlinear read once, at authoring time, for the verses the hand-written picture chapter has reached.";
  "IT IS COMMITTED RATHER THAN FETCHED because the browser is the one place these lines are read and the thing that builds them cannot run there. Authored Bible text already lives as committed functions here, and this is authored Bible text.";
  "Both bands are text anybody may be shown: the original is the public-domain base text, and the English is the Berean wording, which this repo already publishes as one of its own translations and reads as public domain off the publisher own licence page. The English stands in English order, because the column it is built from is that translation cut into chunks and hung on the original words - chunks that straddle the words they hang on, so read in the original order they come out as neither language.";
  let chapter = {
    chapter_code: "PSA084",
    verses: [
      {
        verse_number: 1,
        original:
          "לַמְנַצֵּ֥חַ עַֽל־ הַגִּתִּ֑ית לִבְנֵי־ קֹ֥רַח מִזְמֽוֹר׃ מַה־ יְּדִיד֥וֹת מִשְׁכְּנוֹתֶ֗יךָ יְהוָ֥ה צְבָאֽוֹת׃",
        english:
          "For the choirmaster According to Gittith A Psalm of the sons of Korah How lovely is Your dwelling place O LORD of Hosts",
      },
      {
        verse_number: 2,
        original:
          "נִכְסְפָ֬ה וְגַם־ כָּלְתָ֨ה׀ נַפְשִׁי֮ לְחַצְר֪וֹת יְה֫וָ֥ה לִבִּ֥י וּבְשָׂרִ֑י יְ֝רַנְּנ֗וּ אֶ֣ל אֵֽל־ חָֽי׃",
        english:
          "My soul longs even faints for the courts of the LORD my heart and my flesh cry out for the living God",
      },
      {
        verse_number: 3,
        original:
          "גַּם־ צִפּ֨וֹר׀ מָ֪צְאָה בַ֡יִת וּדְר֤וֹר׀ קֵ֥ן לָהּ֮ אֲשֶׁר־ שָׁ֪תָה אֶפְרֹ֫חֶ֥יהָ אֶֽת־ מִ֭זְבְּחוֹתֶיךָ יְהוָ֣ה צְבָא֑וֹת מַ֝לְכִּ֗י וֵאלֹהָֽי׃",
        english:
          "Even the sparrow has found a home and the swallow a nest for herself where she places her young near Your altars O LORD of Hosts my King and my God",
      },
      {
        verse_number: 4,
        original: "אַ֭שְׁרֵי יוֹשְׁבֵ֣י בֵיתֶ֑ךָ ע֝֗וֹד יְֽהַלְל֥וּךָ סֶּֽלָה׃",
        english:
          "How blessed are those who dwell in Your house They are ever praising You Selah",
      },
      {
        verse_number: 5,
        original: "אַשְׁרֵ֣י אָ֭דָם עֽוֹז־ ל֥וֹ בָ֑ךְ מְ֝סִלּ֗וֹת בִּלְבָבָֽם׃",
        english:
          "Blessed are those whose strength is in You whose hearts are set on pilgrimage",
      },
      {
        verse_number: 6,
        original:
          "עֹבְרֵ֤י׀ בְּעֵ֣מֶק הַ֭בָּכָא מַעְיָ֣ן יְשִׁית֑וּהוּ גַּם־ בְּ֝רָכ֗וֹת יַעְטֶ֥ה מוֹרֶֽה׃",
        english:
          "As they pass through the Valley of Baca they make it a place of springs even the autumn rain covers it with pools",
      },
      {
        verse_number: 7,
        original:
          "יֵ֭לְכוּ מֵחַ֣יִל אֶל־ חָ֑יִל יֵרָאֶ֖ה אֶל־ אֱלֹהִ֣ים בְּצִיּֽוֹן׃",
        english:
          "They go from strength to strength until each appears before God in Zion",
      },
      {
        verse_number: 8,
        original:
          "יְה֘וָ֤ה אֱלֹהִ֣ים צְ֭בָאוֹת שִׁמְעָ֣ה תְפִלָּתִ֑י הַאֲזִ֨ינָה אֱלֹהֵ֖י יַעֲקֹ֣ב סֶֽלָה׃",
        english:
          "O LORD God of Hosts hear my prayer give ear O God of Jacob Selah",
      },
      {
        verse_number: 9,
        original:
          "מָ֭גִנֵּנוּ רְאֵ֣ה אֱלֹהִ֑ים וְ֝הַבֵּ֗ט פְּנֵ֣י מְשִׁיחֶֽךָ׃",
        english:
          "Take notice of our shield O God and look with favor on the face of Your anointed",
      },
      {
        verse_number: 10,
        original:
          "כִּ֤י טֽוֹב־ י֥וֹם בַּחֲצֵרֶ֗יךָ מֵ֫אָ֥לֶף בָּחַ֗רְתִּי הִ֭סְתּוֹפֵף בְּבֵ֣ית אֱלֹהַ֑י מִ֝דּ֗וּר בְּאָהֳלֵי־ רֶֽשַׁע׃",
        english:
          "For better is one day in Your courts than a thousand elsewhere I would rather be a doorkeeper in the house of my God than dwell in the tents of the wicked",
      },
      {
        verse_number: 11,
        original:
          "כִּ֤י שֶׁ֨מֶשׁ׀ וּמָגֵן֮ יְהוָ֪ה אֱלֹ֫הִ֥ים חֵ֣ן וְ֭כָבוֹד יִתֵּ֣ן יְהוָ֑ה לֹ֥א יִמְנַע־ ט֝֗וֹב לַֽהֹלְכִ֥ים בְּתָמִֽים׃",
        english:
          "For the LORD God is a sun and a shield the LORD gives grace and glory He withholds no good thing from those who walk with integrity",
      },
      {
        verse_number: 12,
        original: "יְהוָ֥ה צְבָא֑וֹת אַֽשְׁרֵ֥י אָ֝דָ֗ם בֹּטֵ֥חַ בָּֽךְ׃",
        english: "O LORD of Hosts how blessed is the man who trusts in You",
      },
    ],
  };
  return chapter;
}
