...
export function app_code_quiz_variation_buildable_is(
  ...
  "It is worth its own name because two callers have to agree on it exactly. The quiz drops the unbuildable orderings so the learner cannot start one and dead-end - a swap across a non-commutative neighbour introduces a parenthesis the answer never had, and there is no parenthesis button for it. A gate asking whether the quiz is too lenient has to drop the same ones, or it reports faults in orderings nobody could ever have pressed.";
