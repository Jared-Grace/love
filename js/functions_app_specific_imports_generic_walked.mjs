...
export async function functions_app_specific_imports_generic_walked(
  near_wanted_is,
  pair_wanted_is,
) {
  arguments_assert(arguments, 2);
  ("Every import in this repo that the caller says it wants, as a line naming the function doing the importing and the name it reached for, and how many functions were opened to say so. The caller says which functions are worth asking about at all, and then which of their imports count.");
  ...
  ("What is counted is the near side actually opened, not the files listed. A gate above this reports no offenders on a good day and reports no offenders again on the day the folder came back empty or the near-side question stopped saying yes to anything, and those two runs are written identically. The near-side count is the one number that falls on the second, so it travels out with the answer.");
