...
import { app_ceb_bible_gloss_verse_claims_wrong_baseline_growth_assert } from "./app_ceb_bible_gloss_verse_claims_wrong_baseline_growth_assert.mjs";
...
export async function app_ceb_bible_gloss_verse_claims_wrong_baseline_write() {
  "Rewrite the record of the Cebuano explanations naming a verse that holds nothing related, from what the store holds right now. For seeding it once, and for banking each batch of settled sentences - never for widening it, which is the one thing the gate exists to refuse.";
  ...
  let known = await app_ceb_bible_gloss_verse_claims_wrong_names();
  await app_ceb_bible_gloss_verse_claims_wrong_baseline_growth_assert(known);
