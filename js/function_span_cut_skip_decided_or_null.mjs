...
export function function_span_cut_skip_decided_or_null(
  f_name,
  address_from,
  address_to,
  opening_is,
  ...
  ("Why a run of lines is being stepped over rather than cut out, or nothing at all when there is no reason to step over it, decided from the two words alone once somebody else has asked the repo the three questions only the repo can answer.");
  ("Every reason lives here and nothing here reaches the repo, which is the whole point of it standing apart from the one that does. The three answers only a repository can give - whether the run starts on the first line of work, whether the word it ends on is one the repo already answers to, and whether the name it would take is spoken for - arrive already decided, so this can be handed a run that exists nowhere and still say what it would do with it. That is what lets every reason be written out in a corpus and checked. Asked through the lookups instead, a run has to be a real function in a real repository before anything can be asked about it at all, and these reasons went unchecked by anything for exactly that long.");
  ...
  ("They are asked in this order because each rests on the one before. The first ones ask about the word itself and can be asked straight away; the last ones ask about the name built out of it, and there is no name to ask about until the word is one this repo can spell a name from. Two of them can hold at once, so moving one is not the same as leaving it alone.");
  ...
  ("The name the run would take is worked out here rather than handed in, so nobody can write down a run beside a name its own word would never have given. Whoever asks the repo whether that name is spoken for has to work the same name out in order to ask, which is one small reading done twice on purpose - the other cutting move already does exactly that beside the same call.");
  if (opening_is) {
    let preamble = {
      about: "start",
      address_from,
