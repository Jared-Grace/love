...
export async function functions_gate_walk_escapes_gate_run() {
  ...
  "How many functions the reading reached goes out beside the verdict, so that a run which found nothing wrong can be told from a run which opened nothing. This one is a gate about gates that stop early, and a gate that has quietly stopped looking is the same fault wearing the same green word.";
  "The verdict and the count are put together one name along rather than here, because the other ratchet that has been given a count ends in exactly the same run of work and a duplicate ending is a helper waiting to be written.";
