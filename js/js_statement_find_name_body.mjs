...
export function js_statement_find_name_body(ast, name) {
  ...
  ("The first line of the function's own body that mentions a name, however deep inside that line the name is written.");
  ("The address a span of work is chosen by. Asking for the line that calls a named function was the first try and it could not reach the line a span most often starts on, because that line is an accumulator being opened - a list or an object with nothing in it and so no call in it either. A name covers both: the accumulator is named by the name it introduces, and a loop is named by anything called inside it.");
  ("A name rather than any word at all. Only the names are searched, so a word standing in a piece of prose is not an address however plainly it is written there.");
