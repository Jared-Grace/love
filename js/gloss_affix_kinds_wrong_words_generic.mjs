...
export async function gloss_affix_kinds_wrong_words_generic(fn, known) {
  ...
  "The chapter view answers which chapter to sit down with. This answers how much writing there actually is, which is a different number: the same word is wrong wherever it was met, and the store's twelve hundred wrong claims are far fewer words than that.";
  ...
  function chapters_count(gathered) {
    let chapters = property_get(gathered, "chapters");
    let count = list_size(chapters);
    return count;
  }
  let r = list_sort_number_mapper_reverse(gathered_all, chapters_count);
  return r;
}
