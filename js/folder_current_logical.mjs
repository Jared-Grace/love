...
export function folder_current_logical() {
  ...
  "A person or an editor is told a folder, stands in the one they were told, and means the spelling they were given: stepping out of a folder reached through a link should land in the folder that link sits in, not in the folder it points through to. The kernel forgets the given spelling when it follows a link, so the shell's own record of it - the PWD value it passes on - is the only thing that still holds it, and trusting that record costs a check that it still names the same place. When the check fails the shell spelling is stale or foreign, and the folder as it is answers.";
  "A page in a browser was started in no folder, so it answers with an empty piece, which a path join passes over - a path built on it comes out exactly as it was. The file system is fetched while running rather than imported, so a bundler cannot see it and no page downloads it in order never to use it.";
  if (browser_is()) {
    let r = "";
    return r;
  }
