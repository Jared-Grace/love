...
export async function app_code_html_update_latest_promote_deploy() {
  ...
  "Both halves come back separately, and the copy happens after the check that can refuse to publish, so a refused run leaves the published folder as it found it - read the answer rather than the exit code, because either half can fail on its own.";
