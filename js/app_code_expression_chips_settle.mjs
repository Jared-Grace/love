...
export async function app_code_expression_chips_settle(line, spans) {
  ...
  ("take the chips off a run of operators in two beats: first they go plain where they stand, and only then the room they were holding goes and the rest of the line slides along into it");
  ("Both beats at once is one change too many to follow. The pale square leaving and the whole line shifting sideways land in the same frame, and a learner looking for the operator they did not press finds it somewhere else, plain, with no way to tell whether it moved because it stopped being pressable or because something was worked out.");
  ("Apart, each beat says one thing. The colour going says these are not to be pressed any more. The closing up says the line is now this much shorter - which is the same thing every other step of the line says, and it is said the same way, so it costs nothing new to read.");
  ("The colour is given in one frame rather than faded, because a chip is pale lettered dark and a plain operator is dark lettered pale: faded, the two pass through each other and there is a moment in the middle where the fill and the lettering are the same grey and the operator cannot be read at all. Nothing moves in that beat, so a frame is enough - the pause after it is what makes it a beat of its own.");
