...
export function app_shared_game_npc_turn_note(npc, direction) {
  ...
  ("Writes down on a person that they are about to turn to face a new way - when, and how big a turn - so the rest before their next turn is measured from it - and the way they faced before it, so two turns can be measured together.");
  ...
  if (turning) {
    ...
    property_set(npc, "turn_from", facing);
