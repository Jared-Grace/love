...
export function qa_commit_named_path() {
  ...
  "What is kept for good is the names and not the hundreds of lines that carried them. The names are what a later question needs, they are bounded where the printed reasons are not, and a fresh asking gives the same reasons again. The lines are held for a while all the same, so that a reader corrected today can redo its reading of a judgement made yesterday instead of spending a quarter of an hour asking the gates again; they are let go once the judgement they belong to has fallen too far behind to authorise anything, which is what keeps a file that is committed on every write from growing without end.";
