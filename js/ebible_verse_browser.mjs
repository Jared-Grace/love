...
import { ebible_verse_chapter_browser } from "./ebible_verse_chapter_browser.mjs";
export async function ebible_verse_browser(
  ...
  "What the remembering is filed under is the name of the reader underneath rather than this function's, and that is deliberate. This function's own name is already spoken for by the settling below - both of them write where the page keeps things by the name of the function they are given, so filing under the same name would have each of them find the other's value sitting there. That name is also the honest one for what is kept: these are the answers that reader gave, filed under the arguments it was asked with.";
  async function get() {
    let verse_get = global_function_initialize(
      ebible_verse_browser,
      ebible_verse_chapter_browser,
    );
