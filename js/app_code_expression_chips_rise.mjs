...
export async function app_code_expression_chips_rise(line, spans) {
  ...
  (
    "put the chips on a run of operators in two beats: first the room they stand in opens and the rest of the line slides along to make it, and only then do they take their colour"
  );
  (
    "The mirror of taking them off, and it runs the beats in the mirrored order. Coming off, the colour goes first and the line closes up after it. Going on, the room opens first and the colour lands on operators already standing where they belong."
  );
  (
    "Both ways round, the rule is that the beat which MOVES the line is watched on its own. A step that opened the room and coloured it in one frame showed a learner an operator that had moved and changed at the same moment, and the two together read as the line having been redrawn behind their back rather than as an operator becoming something to press."
  );
  (
  ...
  (
    "No pause is kept between the beats, because the sliding IS the pause - it is a beat the learner watches, and it ends the moment before the colour lands. Coming off, the colour goes in a single frame with nothing moving under it, so there a pause has to be added or the beat is never seen."
  );
  (
