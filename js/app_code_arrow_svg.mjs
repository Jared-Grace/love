export function app_code_arrow_svg() {
  "the derivation arrow DRAWN as inline SVG instead of typed as a glyph, so the head and the shaft are sized independently - a big triangle on a short line, which no font character can give (scaling a glyph squashes the head and the shaft together). Fills with currentColor and has exact edges, so it needs none of the baseline nudging or negative margins a text arrow needed. Head is 16 of the 20 units tall; the shaft is a thin 8-unit stub";
