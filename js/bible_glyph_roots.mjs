export function bible_glyph_roots() {
  ...
  "SO THE HEBREW SPLIT IS NOT A MODEL TO COPY ACROSS, which is the thing a later reader of these two tables would most naturally get wrong. Over there 7067 takes the burning heart with the crown because the word is only ever said of God; here the mark stays single because the words are not. The shared vocabulary of pictures means a reader carries a mark from one testament to the other, and what they carry across is the burning zeal - the crown is the extra claim, and it is only ever made where the Hebrew makes it.";
