...
export async function bible_glyph_chapter_rosetta_lines_fetched_gate_run() {
  ...
  ("THE REFUSAL IS CAUGHT HERE RATHER THAN LET OUT, WHICH IS A CHANGE OF HOW AND NOT OF WHETHER. A refusal let out still failed the gate, so the line above stayed true - but it ended the walk where it happened, and the gate then reported one chapter and asked about nothing after it. Measured on the first of September 2026 exactly one chapter was missing its bands and it happened to be first in the list, so the escaping refusal named the right chapter by luck. A chapter missing further down would have been reported alone, with every chapter after it never asked.");
  ("THE REFUSAL IS CARRIED BACK IN ITS OWN WORDS RATHER THAN COUNTED. A catch that keeps only the fact of a refusal turns every reason into the same reason, and this walk would then report bands that were written but will not parse as bands nobody has written yet - a repair aimed at the wrong place, told with confidence. So what the fetching said is written down beside the code, and this gate claims only that the bands would not come.");
  ...
  for (let chapter of references) {
    ...
    let answered = await catch_message_async(bands_sent_for);
