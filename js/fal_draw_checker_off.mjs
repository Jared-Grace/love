...
export async function fal_draw_checker_off(model, prompt, width, height) {
  ...
  ("Only the ask is built here. Spelling the address, sending it and digging the picture out of the answer are the same on all three roads into fal and are done in one place, which is also where the waiting is explained.");
  let body = {
    prompt,
    image_size: {
      width,
