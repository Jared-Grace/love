...
export function qa_gates_machine() {
  ...
  "One of them asks a machine that is not this one at all - the file store, over the network - and it belongs here for the same reason as the rest: no copy of these files holds the answer, and the answer can change while not a line of them does";
  let gates = [
    cors_gate_run,
    daemons_gate_run,
    guard_gate_run,
