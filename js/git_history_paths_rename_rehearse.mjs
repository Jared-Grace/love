...
export async function git_history_paths_rename_rehearse(
  ...
  "BOTH HALVES ARE COMPARED AS SORTED LISTS OF NAMES, AND NEVER AS THE TWO COLLECTIONS THEMSELVES. The repository's way of asking whether two collections are the same writes each one out as text and compares the text, so the order the names were put in counts as part of the answer - and a rename reorders them by construction, because a path that moves to a new folder sorts in a new place. Measured 2026-09-03 on a rename of four files out of sixty-four, every blob identical and every name exactly where it was meant to be: both halves of the proof failed. Sorting first is what makes the question asked here the question meant - is this the same set - rather than is this the same set written down the same way.";
  ...
  let list = object_property_names(tracked_before.blob_names);
  let blobs_before = list_sort_text(list);
  let list2 = object_property_names(tracked_after.blob_names);
  let blobs_after = list_sort_text(list2);
  ...
  let list3 = object_property_names(paths_expected);
  let names_expected = list_sort_text(list3);
  let list4 = object_property_names(tracked_after.paths);
  let names_after = list_sort_text(list4);
