...
import { bible_glyph_word_parse_group } from "./bible_glyph_word_parse_group.mjs";
import { list_add } from "./list_add.mjs";
import { less_than } from "./less_than.mjs";
import { not } from "./not.mjs";
import { bible_glyph_name_character_is } from "./bible_glyph_name_character_is.mjs";
import { bible_glyph_word_parse_parts_word } from "./bible_glyph_word_parse_parts_word.mjs";
export function bible_glyph_word_parse(text) {
  ...
  "TWO PARTS OF THIS ARE READ NEXT DOOR, and both are the parts that need to remember nothing. Splitting a group on its pluses is handed the gathered run and hands back the names. Deciding what a finished list of pieces stores is handed the pieces and hands back the word. What stays here is the one thing that cannot be handed anywhere - walking the characters while holding where the reading is up to, whether it stands inside a group, and what has been gathered so far.";
  let parts = [];
  let buffer = "";
  ...
  function buffer_flush() {
    ...
    if (inside) {
      let group = bible_glyph_word_parse_group(buffer);
  ...
  buffer_flush();
  let r = bible_glyph_word_parse_parts_word(parts);
  return r;
}
