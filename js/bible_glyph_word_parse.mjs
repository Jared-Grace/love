...
export function bible_glyph_word_parse(text) {
  ...
  "THE NAMES OF A GROUP ARE READ APART NEXT DOOR, because splitting a run on its pluses is the one part of this that needs nothing else to be remembered - not where the reading is up to, not whether it stands inside a group, not what has been gathered so far. It is handed the run and hands back the names.";
  let parts = [];
  let buffer = "";
