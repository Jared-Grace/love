...
export async function functions_arguments_assert_mismatches() {
  ...
  ("Nothing else here asks this. The neighbour comparing calls against parameters is asking what callers do; a function disagreeing with itself is already wrong with no caller involved. There is a piece that puts the two back together, but only one thing ever reached it - the command that changes a parameter list - so a parameter added any other way left the disagreement standing.");
  ("Both ways of counting are asked about, the bare number and the list of tests, because a function carries one or the other and either can be left behind. The list of tests is guarded once already, where it is written, but never again afterwards.");
  ...
  ("Each one found is handed back as the name and the two numbers together with the sentence saying it, and with which of the two kinds of check it carries. The sentence is for whoever reads the failure; the rest is for the command that repairs them, which can put a bare number right on its own and cannot invent a test.");
  ...
  for (let f_name of names) {
    ...
    let asserted = uncounted ? listed : counted;
