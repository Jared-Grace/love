...
export function app_code_lesson_console_log_chained_generic(params) {
  "a reusable console.log lesson for chaining THREE numbers with one operator (x op y op z); the caller passes the operator symbol, a word for the id, an example triple for the intro, and triples_get which returns four DISTINCT [x,y,z] triples per refill so no two examples come out the same";
  ...
  function title_name_id() {
    "the home title shows the operator twice, like console.log + +, to signal three chained numbers";
  ...
  function above(root) {
    ...
    html_div_cycle_code(c, ["We can use more than two numbers"]);
