...
| `function_prose_remove <fn> <position>` | **take a line out** — the third of the three things that can be done to a line of an account, and with it the whole family is `add` / `set` / `remove`. A line can go stale rather than wrong, pointing at a choice nobody makes here any more or a neighbour that has gone, and then taking it out is the honest repair rather than writing something else in its place. Same numbering, counting from 1; every line after the one removed moves up by one, so **read the account again between two removals**. Reaches a line anywhere in the body, not only the account at the top |
...
| `functions_new_baseline_ratchet <lister> <slug> <hint>` | **write a whole shrink-only ratchet in one command** — you write the one function that lists what offends; this writes the four around it (`<slug>_baseline_path`, `_baseline_growth_assert`, `_baseline_write`, `<slug>_gate_run`), seeds `data/<slug>_baseline.json` from what offends today, and joins `qa_gates` through `function_list_name_add`. So the repo it leaves is **green**, and green in both ways a new gate can be red: the record is seeded, and the gate is wired (`functions_gate_run_unwired_gate_run` fails on the second). 25 ratchets are written here and each one cost four near-identical files after the interesting part — deciding what counts as an offence — was already done. `<hint>` is the sentence a failing gate says; it is a whole argument, so **a comma is fine**, and it is quoted before it is written. Both names are read through `js_identifier_expression` and the lister is checked against the functions that exist |
| `function_new_code <fn> <parameters> <async_is> <lines>` | the atom the row above is built on — write a whole function from its name, the names it receives, whether it waits, and the lines it is made of, then canonicalize. **Not for running from a command line**: `<lines>` is written code, so it is for a command that shapes its own lines from names it has already checked. Whether it waits is said rather than inferred, because the word has to be in place before a line holding `await` can be read at all |
