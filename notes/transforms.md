...
| `js_selects_code_add_after <code>` / `_before` | write one line of **code beside a chosen line**, the code half of the pair above and the same working-out asked to parse rather than to quote. Until these, written code could only land at the *top or bottom* of a block (`js_block_body_add_code` / `_first`), so a line that must sit **between two particular lines** — a value worked out after the line above it and read by the line below it — was a hand edit every time. Takes the prompting `_code` command, so reach for `function_select_apply_code_auto`; one statement only, because the address names one place |
